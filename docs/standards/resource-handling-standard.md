...
> 原路径：`docs/wakeflow-typescript-resource-handling-standard-2026-08-26.md`，2026-09-03 迁入开发文档系统；目录职责与权威顺序见 [docs/README.md](../README.md)。

> 创建日期：2026-08-26
> 当前状态：`confirmed-direction / RH-1-implemented / RH-2-implemented / RH-3-ledger-publication-and-catalog-implemented`
> 上位计划：[Wakeflow TypeScript 全新项目能力重构开发计划](../plan/typescript-reimplementation-plan.md)
> 基础服务边界：[Wakeflow 全局基础服务需求](../archive/wakeflow-foundation-services-requirement-2026-08-11.md)
...
- Event Sourced aggregate 以 event stream 为可变状态唯一 authority，snapshot 只能是可删除、可重建的 checkpoint；按 [ADR-0005](../decisions/0005-demand-event-stream-read-path.md)，snapshot 在每次 apply 成功后刷新，operation 读取默认走 snapshot 加尾部，刷新失败不改变 event stream 的 authority；
