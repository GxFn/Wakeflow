...
async function captureObservation(workspaceRoot, selection, source, policy, signal) {
    const read = await readHostHookObservationRecord(workspaceRoot, source.hostId, source.recordId, signal === undefined ? {} : { signal });
    if (read === null)
        fail("source");
...
export class ManagedEvidenceCapturePlanningService {
    ...
    async preview(demandIdValue, selectionValue, optionsValue = {}) {
        ...
        try {
            ...
            const worktreePaths = (await listPodWorktreeReceiptsAnyHost(this.#workspaceRoot, context.loaded.identity.podId, options.signal === undefined ? {} : { signal: options.signal })).map((receipt) => receipt.path);
