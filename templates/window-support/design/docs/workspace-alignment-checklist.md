# Design 与 Wakeflow 能力对齐检查

状态：长期规则

本文件用于确认 `Design` 的文档和配置能完整承接 Wakeflow 的需求设计前置能力。它不是执行计划，也不替代总控规则。

## 角色边界

| 总控能力 | Design 对应产物 | 必须保留的边界 |
| --- | --- | --- |
...
| 测试交接 | `requirement-design` 的验证策略和 `workspace-handoff` 的验证需求 | 不直接创建 `Test` 测试 card / 任务包，不跑真实项目测试；总控接收后再决定是否用 `wakeflow-intake.mjs test-card`。 |
