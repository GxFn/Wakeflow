 ...
 */
export const WAKEFLOW_TARGET_TASK_PLANNING_PUBLIC_TOOL_NAME = "wakeflow_plan_target_task";
export const WAKEFLOW_TARGET_TASK_PLANNING_PUBLIC_SCHEMA_VERSION = 1;
const validateRequest = createRuntimeJsonSchemaValidator(WAKEFLOW_TARGET_TASK_PLANNING_REQUEST_SCHEMA);
const validateResult = createRuntimeJsonSchemaValidator(WAKEFLOW_TARGET_TASK_PLANNING_RESULT_SCHEMA);
...
export function parseTargetTaskPlanningRequest(value) {
    const result = validateRequest(requestJson(value));
    if (!result.ok)
        fail("invalid-request", "schema", `$request${result.path.slice(1)}`);
    return result.value;
}
export function admitTargetTaskPlanningResult(value) {
