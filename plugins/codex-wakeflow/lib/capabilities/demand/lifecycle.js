...
import { createFileAtomically, replaceFileAtomically, } from "../../foundation/filesystem/durable-atomic-file-write.js";
...
import { createDemandCompletion, DemandCompletionError, } from "../../governance/lifecycle/demand-completion.js";
import { buildDemandPostAcceptanceRoute, DemandPostAcceptanceRouteError, } from "../../governance/review/demand-post-acceptance-route.js";
...
import { DEMAND_LIFECYCLE_JOURNALS_ROOT_REF, demandArchiveRef, demandLifecycleJournalRef, } from "../../kernel/layout.js";
...
async function readClaim(context, requirementId) {
    return readRequirementClaimState(context.root, requirementId, context.signal);
}
function packageReceipt(source, lineage) {
    return Object.freeze({
...
// ---- 步骤日志 ------------------------------------------------------------
async function readJournal(context, demandId) {
    try {
        const read = await readDeterministicJsonFile(context.root, demandLifecycleJournalRef(demandId), {
        ...
        if (kind !== JOURNAL_KIND)
            fail("recovery-required", "journal-invalid", "$journal");
        return value;
    }
    catch (error) {
        const reason = typeof error === "object" && error !== null && "reason" in error ? error.reason : null;
        if (reason === "not-found" || reason === "resource-not-found")
            return null;
...
async function writeJournal(context, journal) {
    ...
    const bytes = encodeUtf8(renderDeterministicJsonDocument(parseJsonValue(journal, "$journal"), "$journal"), "$journal");
    try {
...
async function deleteJournal(context, demandId) {
...
async function appendLifecycleEvent(context, handle, command, commitId, expectedStreamRevision) {
    try {
        const result = await executeDemandEventSourcingCommand(new DemandEventSourcingRepository(handle.demandRoot), command, { commitId: commitIdOf(commitId), expectedStreamRevision, ...signalOptions(context.signal) });
        return Object.freeze({
...
async function findEventReceipt(context, handle, commitId) {
    let commit;
    try {
        commit = await new DemandEventSourcingRepository(handle.demandRoot).findCommitById(commitId, signalOptions(context.signal));
    }
    catch (error) {
        mapCommandError(error);
    }
    if (commit === null)
        fail("precondition-failed", "terminal-commit-mismatch", "$demandRoot");
    return Object.freeze({
...
// ---- 完成与取消：计划 -------------------------------------------------------
async function postAcceptanceStage(handle, signal) {
    try {
...
async function planTerminal(context, input, facts) {
    const demandId = parseDemandId(input.demandId);
    const handle = await openDemandHandle(context, demandId);
    if (handle === null)
        return terminalBlockedWithoutRoot(context, demandId);
    const { loaded } = handle;
    ...
    const existingArchive = await findLatestDemandArchive(context.ledgerRoot, demandId, context.signal);
...
// ---- 完成与取消：应用 -------------------------------------------------------
async function completionCommand(context, handle, plan, claim, at) {
    const route = await postAcceptanceStage(handle, context.signal);
    if (route === null ||
...
function cancellationCommand(plan, at) {
    if (plan.reason === null)
        fail("unexpected", "cancel-reason", "$plan");
    return Object.freeze({
...
async function terminalEvent(context, handle, plan, claim) {
    ...
    const command = plan.action === "complete"
...
async function worktreeMember(context, scope) {
    if (scope.pod.placement !== "worktree")
        return null;
    const receipts = await listPodWorktreeReceiptsAnyHost(context.root, scope.pod.podId, signalOptions(context.signal));
    return {
...
async function sealArchive(context, handle, plan, receipt, verify) {
    ...
    const sealed = await sealDemandArchive(context.ledgerRoot, {
        demandId: plan.demandId,
        streamRevision: receipt.streamRevision,
        payload,
        verifyReport: parseJsonValue(verify, "$verify"),
        manifest: (payloadSummary, reportDigest) => createArchiveManifest({
...
function archiveReceipt(archiveRef, manifest) {
    return Object.freeze({
        archiveRef,
...
function manifestLineage(manifest) {
    return Object.freeze({
...
async function settlePackage(context, plan, lineage) {
    ...
    const at = now(context);
    const next = plan.action === "complete"
...
async function releaseClaims(context, plan) {
    ...
    for (const windowId of plan.windowIds) {
        ...
        if (inspected.claim === null)
            continue;
        if (inspected.claim.holder.demandId !== plan.demandId)
            continue;
...
async function applyTerminal(context, plan, verify, disposition) {
    ...
    if (handle === null) {
        ...
        if (handle === null)
            fail("recovery-required", "demand-root-vanished", "$demandRoot");
...
async function recoverTerminal(context, action, operationId) {
    ...
    if (journal !== null) {
        ...
        if (journal.verify === null)
            fail("recovery-required", "journal-verify-missing", "$journal");
    ...
    const located = await findLatestDemandArchive(context.ledgerRoot, demandId, context.signal);
    if (located === null)
        fail("not-found", "demand-unknown", "$request.operationId");
    const expected = action === "complete" ? "completed" : "cancelled";
    if (located.manifest.outcome !== expected) {
        fail("precondition-failed", `archive-outcome:${located.manifest.outcome}`, "$request.operationId");
    }
...
async function executeTerminal(action, tool, parse, admit, value, options) {
    ...
    return runPublicationTransaction({
        ...
        plan: (context, input) => {
            ...
            return planTerminal(context, { action, demandId: input.demandId, reason: "reason" in input ? input.reason : null }, facts);
        },
        apply: (context, _input, plan) => {
            if (facts.verify === null)
                fail("unexpected", "verify-missing", "$plan");
            const verify = facts.verify;
            return afterMutationRefresh(context.root, context.signal, () => applyTerminal(context, plan, verify, "apply"));
        },
...
export async function executeDemandCompletionRequest(value, options = {}) {
    return executeTerminal("complete", WAKEFLOW_DEMAND_COMPLETION_PUBLIC_TOOL_NAME, parseDemandCompletionRequest, admitDemandCompletionResult, value, options);
}
...
async function planContinue(context, input) {
    ...
    if (blockers.length > 0 || archive === null || claim === null)
        return blockedPlan(blockers);
    const ids = deriveLifecycleIds(demandId, "continue", archive.manifest.terminalEvent.streamRevision);
    return readyPlan(Object.freeze({
        action: "continue",
        demandId,
        archiveRef: archive.archiveRef,
        archiveManifestDigest: manifestDigest(archive.manifest.manifestDigest, "$manifest.manifestDigest"),
        payloadTreeDigest: manifestDigest(archive.manifest.payload.treeDigest, "$manifest.payload.treeDigest"),
...
async function planDecision(context, input) {
    const demandId = parseDemandId(input.demandId);
    const handle = await openDemandHandle(context, demandId);
    ...
    if (blockers.length > 0 || handle === null || state?.awaitingDecision === undefined)
        return blockedPlan(blockers);
    const ids = deriveLifecycleIds(demandId, "record-decision", handle.loaded.aggregate.streamRevision);
    return readyPlan(Object.freeze({
...
async function reclaimPackage(context, plan) {
    const source = await readClaim(context, plan.package.requirementId);
    if (source === null)
        fail("recovery-required", "claim-state-vanished", "$board");
    const lineage = { recordRef: plan.package.recordRef, recordDigest: plan.package.recordDigest };
...
async function applyContinue(context, plan, disposition) {
    ...
    await releaseDemandHandle(context);
    await restoreDemandRoot(context.root, plan.demandId, await restoredPayload(context, plan), context.signal);
    const handle = await openDemandHandle(context, demandId, { reload: true });
    if (handle === null)
        fail("recovery-required", "demand-root-restore", "$demandRoot");
    let event;
    let committed = false;
    if (handle.loaded.aggregate.state.lifecycle === "completed") {
        const appended = await appendLifecycleEvent(context, handle, Object.freeze({
        ...
        }), plan.commitId, plan.expectedStreamRevision);
...
async function applyDecision(context, plan) {
    const demandId = parseDemandId(plan.demandId);
    const handle = await openDemandHandle(context, demandId, { reload: true });
    if (handle === null)
        fail("precondition-failed", "demand-root-absent", "$demandRoot");
    const appended = await appendLifecycleEvent(context, handle, Object.freeze({
...
async function recoverContinuation(context, operationId) {
    const demandId = parseDemandId(operationId, "$request.operationId");
    const journal = await readJournal(context, demandId);
...
export async function executeDemandContinuationRequest(value, options = {}) {
    return runPublicationTransaction({
        ...
        apply: (context, _input, plan) => afterMutationRefresh(context.root, context.signal, () => plan.action === "continue"
        ...
        next: async (context, phase) => phase.mode === "preview"
            ? previewNext(WAKEFLOW_DEMAND_CONTINUATION_PUBLIC_TOOL_NAME, "demand-continuation-apply", phase.planned)
            : nextAfterMutation(context, phase.outcome.demandId),
