...
const identitySchemaRelativePath =
  "foundation/wakeflow-durable-id-kind.schema.json";
const identityGeneratedRelativePath =
  "foundation/wakeflow-durable-id-kind.generated.ts";
...
test("Schema catalog rejects an external reference outside the new catalog", () => {
  const fixtureRoot = mkdtempSync(path.join(tmpdir(), "wakeflow-schema-catalog-"));
  const schemaRoot = path.join(fixtureRoot, "src/contracts/schemas");
  mkdirSync(schemaRoot, { recursive: true });
  writeFileSync(path.join(schemaRoot, "unknown.schema.json"), `${JSON.stringify({
