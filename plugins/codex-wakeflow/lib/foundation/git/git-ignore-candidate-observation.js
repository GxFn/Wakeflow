...
import { createFileCandidateDurably, DurableFileCandidateError, } from "../filesystem/durable-file-candidate.js";
import { parsePortableResourcePath, } from "../filesystem/portable-resource-path.js";
...
import { GitIgnoreObservationError, observeGitIgnorePathsInWorkTree, } from "./git-ignore-observation.js";
/**
 ...
 */
const GIT_IGNORE_CANDIDATE_MAXIMUM_BYTES = parseByteCount(2 * 1024 * 1024, "$candidateBytes");
...
export class GitIgnoreCandidateObservationError extends Error {
    name = "GitIgnoreCandidateObservationError";
    code = "wakeflow-git-ignore-candidate-observation";
    reason;
    path;
...
function snapshotCandidate(value) {
    ...
    try {
        ...
        if (error instanceof Sha256Error)
            fail("hash-failure", "$candidateBytes");
        throw error;
    }
    return Object.freeze({
        bytes,
...
async function assertRepositoryRootCurrent(root) {
    try {
...
async function createTemporaryWorkTree() {
    let createdPath;
    try {
        createdPath = await mkdtemp(nodePath.join(os.tmpdir(), TEMPORARY_WORK_TREE_PREFIX));
    }
    catch {
        fail("isolation-failure", "$candidateWorkTree");
    }
    try {
        const root = await RootedDirectory.open(createdPath, "$candidateWorkTree");
...
async function materializeCandidate(workTreeRoot, candidate, signal) {
    try {
        await createFileCandidateDurably(workTreeRoot, GITIGNORE_RESOURCE_PATH, candidate.bytes, {
            mode: 0o600,
...
async function observeCandidate(repositoryRoot, workTreeRoot, probePaths, signal) {
    try {
        return await observeGitIgnorePathsInWorkTree(repositoryRoot, workTreeRoot, probePaths, signal === undefined ? undefined : { signal });
    }
    catch (error) {
 ...
 */
export async function observeGitIgnoreCandidate(repositoryRootValue, candidateBytesValue, probePathValues, optionsValue) {
    ...
    try {
        ...
        const observation = await observeCandidate(repositoryRootValue, temporary.root, probePathValues, options.signal);
    ...
    try {
        ...
        if (operationError === undefined) {
            operationError = new GitIgnoreCandidateObservationError("isolation-failure", "$candidateWorkTree");
        }
    }
    if (!safeToRemove)
        fail("cleanup-failure", "$candidateWorkTree");
