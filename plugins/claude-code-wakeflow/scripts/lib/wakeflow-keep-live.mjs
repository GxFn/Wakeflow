...
export function createKeepLiveManager({
  ...
  function keepLiveCommand() {
    return getValue(
      "--keep-live-command",
      process.env[hostProfile.keepLiveEnv.command] || "caffeinate",
    );
  }
  ...
  function keepLiveArgs() {
    ...
    const jsonArgs = process.env[hostProfile.keepLiveEnv.argsJson];
    if (jsonArgs) {
      try {
  ...
  function keepLiveEnabled() {
    ...
    if (process.env[hostProfile.keepLiveEnv.disable] === "0") return false;
    return true;
  }
