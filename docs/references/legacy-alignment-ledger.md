...
| 同上 | 配置词汇 | `governance.audit.preservedReviewAfterDays`、`governance.validation.runtimeResidue`、`repositories[].validation.residueExceptions` | schema 与解析器保留，两个实现里都没有行为消费者（旧消费者 preservation/legacy transform 已放弃；runtimeResidue 旧实现亦无消费者；residueExceptions 旧只做计数） | dropped | 2026-09-21 用户裁决删除（gate-log §13.113）：schema 去掉三条词汇与只有它们引用的定义，`governance` 保留为空对象 |
...
| `wakeflow-reconcile.mjs` | `planWakeflowReconcileBackbone` | 自动修复：local 静态目录、支撑面目录、ledger 目录与索引、`.gitignore` 与记忆托管块、active 布局与 TODO 板、投影、窗口投影、Claude settings；只报告：窗口投影 unsafe/unavailable、配置非 current、托管块手改 | 本轮 **G4** 后：活动布局与看板、ledger 根与容器、宿主 capability 目录、支撑面根与 scaffold、支撑面与工作区 `.gitignore`、程序/外部指令、支撑面记忆、共享协调布局；Claude settings/statusline 由宿主 contribution；只报告：`*-conflict`、`*-envelope`、维护协议根 busy / recovery-required / conflict；宿主运行时根、维护协议根乃至整个 `.wakeflow-local` 缺失自 §13.114 起也重建 | recut | 窗口投影 stale/missing 由 G5（§13.108）重建、G6（§13.111）报告 |
| `wakeflow-reconfigure.mjs` | `diffWakeflowConfigV3Topology` | 拓扑差异 | 预览 `sameSemanticSection`：topology/storage 任一变化即 `reconfigure-layout-change-unsupported`，pods 变化即 `reconfigure-pods-change-unsupported`；hosts 启动偏好可改 | recut | 旧版允许支撑面移动等；新版按能力卡 1 拒绝布局改动。hosts 曾被误并入布局检查，§13.116 D1 按能力卡 1 §1.3 改回可改 |
...
| 同上 | `inspectWindowRuntimeProjectionsForLayout` | 维护候选模型（fresh / reconfigure / reconcile）与观察的 layout 视角盘点；旧 `wakeflow_status` / `wakeflow_verify` 的 `window-runtime` 域（health、`window-runtime-projection-not-current`）由它供数 | 维护：fresh `publish-unregistered-window-runtime`，reconcile G5；观察：`wakeflow_status.windows[].projection` 与 `domains.windowRuntime`，verify 门 `window-runtime-projection`（同一只读缝 `wakeflow-window-runtime-projection-inspection.ts`） | recut | 曾为 **G6**（C 组核出，F 组修，gate-log §13.111） |
