...
| L2 业务场景与联合 | 从初始化到归档的端到端场景跑通 | 场景联合：初始化、窗口、TODO、Demand、派发、结果、评审、完成、归档；场景验收全量运行；skills 与 commands 文本随场景重写；Agent 执行的派发在两宿主完成一次投递并交回回读证据 | 无场景价值的旧测试 | Controller Route 不再返回任何 not-implemented blocker；矩阵每行都有 TS owner 或放弃记录；场景验收覆盖全部保留场景；治理测试墙钟低于 3 分钟 |
...
- 新测试始终轻量：共享一个 v3 基线 fixture 按需复制，替代各自建工作区的 fixture；单元测试用内存值不落盘；每个场景只保留一条端到端链；发现重复证据即删除；治理测试墙钟目标低于 3 分钟（ADR-0007）。
...
| E2 能力重新实现 | `in-progress` | 已完成 Artifact Tree Identity、Config Authority Snapshot、RH-1 TODO Intake Aggregate 与经根本审查重构的 RH-2 Demand Core Aggregate；Demand 已收敛为 immutable Identity/Authority + append-commit Event Stream + immutable derived snapshots，不保留 JSONL、state/event 双 authority、stream lock 或 pure append journal。至 `c0098e2`：23 个公共工具，Ledger、TODO、Demand、Route、Task、Delivery、Host Effect 握手、Result、Review、Completion、Managed Evidence 纵切闭合；公共入口已按四组拆分。已知偏离见 [reviews/2026-09-03](../reviews/2026-09-03-typescript-checkpoint-review.md)：快照只写一次、读路径全量重放、样板与脱敏漂移、宿主效果层缺席，按 §8.1 的 P1 到 P4 处理 |
| E3 能力覆盖与场景验收 | `pending` | 按 ADR-0008 从新旧对比改为能力覆盖与场景验收；场景验收骨架从 P0 开始建立并随每个纵切运行；旧实现只读保留，旧门已退出 `npm test` |
