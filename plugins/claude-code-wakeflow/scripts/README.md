...
- `wakeflow-delivery.mjs`: state-root-backed local transport manager. It
  ...
  automation jobs, send host session messages, verify target truth, or accept results. Runtime
...
- `wakeflow-setup.mjs`: sibling-directory installation helper. It
  ...
  `wakeflow.config.json` generation, root `CLAUDE.md` unpacking, child-window
