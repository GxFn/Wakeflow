...
## 13.126 逐模块对齐第五轮：需求包与 Design / Test 技能——对照、技能文本四处移植、看板视图带撤回原因、两道静态门与一条并发回归（2026-09-25）

**背景。** 第五轮对照旧实现的需求入口与两份角色技能：TODO 板与认领（`wakeflow-todo-{service,table}.mjs`）、ledger 记录与投影（`wakeflow-ledger-{records,materialization,projector}.mjs`）、需求发布事务（`wakeflow-demand-publication-service.mjs`、`wakeflow-demand-document-builder.mjs`）、模板资产（`wakeflow-template-{builder,source}.mjs`），以及 `core/skills/` 里的 `wakeflow-design`（SKILL 加五份 references、两份 assets）、`wakeflow-test`（SKILL 加四份 references）、`wakeflow-target-craft` 与 governance 的 `testing-validation.md`、`todo-backlog.md`；对照单位仍是旧实现的测试名（`wakeflow-todo-service`、`wakeflow-todo-table`、`wakeflow-ledger-v3`、`wakeflow-demand-v3-layout`、`wakeflow-demand-core-v3`、`wakeflow-skill-surface`、`wakeflow-template-{builder,source}`、`wakeflow-thin-doc-contract` 九个文件）。新实现对应 `src/capabilities/requirement/`、`src/kernel/requirement-board.ts`、`src/governance/ledger/`、`src/governance/{tasking,testing,result,review}/` 的测试合同与测试报告，以及 `assets/agent-text/skills/{wakeflow-design,wakeflow-test,wakeflow-target}/` 与 `tests/artifacts/agent-text-honesty.test.ts`。

**行为对照。**

| 旧实现已验证的行为（测试名摘要） | 新实现 | 状态 |
|---|---|---|
| 候选服务拥有唯一确定的空板；TODO 权威准入拒绝 final-file 模式、链接数与体积漂移 | 每包一份 0600 状态文件在 0700 的 `board/` 下，人读索引确定性重写，空索引摘要绑进初始化权威 | 同形（重切为每包一文件） |
| 严格扫描拒绝未知、损坏、非规范与重复行；已损坏的板上任何变更零写入 | 读不出或与文件名不符的条目计入 `skipped`，`wakeflow_verify` 的 `board-consistency` 门与 `inspect_board.truncated` 都能看到；一份损坏的兄弟状态不阻塞另一包的变更（每文件各自权威） | 有依据的重切 |
| append 有锁、认板 CAS、重复安全、不动无关行 | 每包一把锁加独占创建：同记录重放 `current`，异记录占同标识 `claim-state-exists` | 同形 |
| claim 要求精确的 intake 快照与结构化的 Demand 挂载；同一快照的并发认领恰有一次提交 | Demand 发布计划记下 `expectedClaimStateDigest`，锁内重读加节点与摘要 CAS；**本轮补上并发回归**（两个 Demand 凭同一快照认领恰有一个成功，同一状态并发创建恰有一个 `created`） | 同形（补测试） |
| claim 检查只读；恢复在同一把锁内提交 pending 或接受精确的 committed | `exactClaimedPackage`：同一 Demand 已认领即 current，恢复重跑同一认领 | 同形 |
| archive 只凭业务回执删除一条精确的 claimed 行；谱系能从 claimed 行反推 intake 快照 | claimed → archived 保留状态文件与修订链（`previousStateDigest`），`archive.demandId` 必须等于 `claim.demandId`，重开只允许归档时的那个 Demand | 有依据的重切（行保留不删） |
| 并发 append 串行化；文件系统入口要求固定路径、已有父目录、非符号链接的板 | rooted directory 加 stable read；每包锁；并发创建回归 | 同形 |
| ledger：严格 create/load 规范、只创建、拒未知字段与摘要漂移；成员路径、引用、未知残留与符号链接失败关闭 | `LedgerAuthorityStore` 发布与加载；读取器要求目录条目集合恰等于期望集合，多一个文件即失败 | 同形 |
| 四索引投影字节确定；投影要求 0644 单链接；对账只消费安全的原子阶段 | 一份看板索引确定性重写；与并发写者相撞重列重写四次，仍撞报可重试的 `board-index-contended`，权威已提交不受影响 | 同形（一份索引） |
| 需求发布：先 sidecar 与完整 stage 再 root-first rename，认领 TODO 在根健康之前；同意图幂等、异 Demand 不能复用同一挂载；不可变归档永久保留 Demand 身份；同意图并发发布串行到一个根 | 根先建后认领 CAS（ADR-0011 D7）；`claim-reclaim-other-demand`；同 Demand 重放为 current；没有 sidecar 日志，靠 CAS 与重跑恢复 | 同形（部分重切） |
| Design 技能：只草稿直到显式确认；`wakeflow_deliver` 恰好 13 列的行加板 CAS；parked 与 Auto Claim；六种就绪状态；澄清、方案比较、切片、需求设计、交接五个方法；两份草稿模板 | 预览摘要即确认点 1，记录不可变；**移植**：看板动作程序（supersedes / parked / activate / withdraw，凭 `inspect_board` package 视图的 `stateDigest` 做 CAS）、澄清方法的压缩版、`requirement.md` 与 `landing.md` 的骨架（能力卡 3 Q7 的遗留项）；放弃方案比较、切片、13 列行与 Auto Claim（ADR-0011 单一交接） | 同形 + 移植 |
| Test 技能：三条铁律（只执行冻结的问题 / 环境 / 计划 / 方法；产品源码只读；测试证据不是验收）；两条入口门；Test 自有 harness / fixture 资产；结果证据精确形状（test-step 按序覆盖每一步）；风险策略、调试分诊、回归建议、自证评审四个方法 | 冻结合同（步骤绑验收标准、允许的技能、setup、尝试预算、停止条件）；逐步报告，整体 verdict 由 Wakeflow 派生；**移植**：Test 自有资产边界（`harnesses/`、`fixtures/`）、线上词汇（四种 verdict、七种失败分类与 owner）、判定产品缺陷前的反馈回路、导入前自证清单，以及 **Test 窗口自己记录 `test-output` 证据**（见下）；放弃回归建议与独立的风险策略方法 | 同形 + 移植 |
| Target Craft：七项实践各挣一份证据；锚点 RED / GREEN 计划；返工逐条回应；重复返工止损 | `wakeflow-target` 加 `craft.md`：先诊断、边界、逐锚点作答、报告写给没看过程的人；证据种类不做机器词汇（能力卡 5）；needs-review 常规结尾（§13.123 D7） | 同形（重切） |
| 技能面测试：精确文件面；frontmatter 只有 name 与 description；链接不出根、可解析、无孤儿；内容断言；制品逐字节匹配 | 诚实性门（白名单、反向覆盖、退役词汇、路径闭合、命令闭合、归属表、体积预算）；制品匹配是 `build:check`；**移植**：frontmatter 恰为 name 与 description 且 name 等于目录名，每份 reference 被自己的 SKILL.md 点名、点名的 reference 都存在 | 同形 + 移植 |
| governance 测试校验：实现校验未完不派 Test；`controllerSelfChecks`；冻结问题、边界、成功 / 失败含义、无效结论、停止条件；回传的 test-step 映射与派单合同比对 | `deriveTestPlanningBlockers`：real-environment、全部实现目标已接受、至多一个未终结 test 目标、复测谱系；合同带 question / objectBoundary / steps / environment / allowedSkills / setupPolicy / maxAttempts / stopConditions；报告步骤按合同 id 与本次尝试范围核对 | 同形 |
| TODO / Backlog 参考：入板条件、滚动 TODO、派单用法、归档 | 无：看板即需求包认领状态（ADR-0011） | 放弃 |
| 模板 bundle 与 builder / source / thin-doc 合同 | 无：模板是技能内容，不是运行时资产 | 放弃 |

**移植一：Design 的看板动作与骨架。** 旧技能的交接程序讲清了 parked、Auto Claim 与板 CAS；新技能的 description 提到 activate 与 withdraw，正文却一个字没有——Design 只会发布。现在 `wakeflow-design/SKILL.md` 多一节"Correcting, parking and withdrawing"：每个后续动作都从 `wakeflow_inspect_board` 的 package 视图取 `stateDigest` 作 `expectedStateDigest`；更正 = 带 `supersedes` 的新包，同一次 apply 把仍在 pending / parked 的旧包撤回为 `superseded-by`，已认领的包不从这里替代；搁置 = 带 `parked.trigger` 发布，条件成立后 `activate`；撤回 = `withdraw` 带原因，已认领的包只能由 Controller 取消 Demand。`references/requirement-package.md` 多了旧澄清方法的压缩版（把"改进一下"修成可观察的结果；只问会改变目标、范围、验收或测试决策的问题，一次一个并带推荐答案；事实、建议、决定、未决分开）和 `requirement.md` / `landing.md` 的骨架（标题即别名表认得的标题）——能力卡 3 Q7 "两份草稿模板移入 skills" 至此落地。

**移植二：Test 的线上词汇、自有资产与自证。** 代码接受七种失败分类（`product-defect`、`harness-defect`、`environment`、`flaky`、`missing-evidence`、`out-of-scope`、`needs-decision`）、四种 owner 与四种 verdict，技能只教了五种分类、没提 verdict 的词——Test 窗口按文本导入就会撞 schema。现在 SKILL.md 与 `test-execution.md` 写全了词汇、派生规则（任一 fail 即 fail，其次 blocked，其次 cannot-conclude）、`out-of-scope` 与 `needs-decision` 的用法；旧技能的 Test 自有资产边界（Test 面的 `harnesses/` 与 `fixtures/`，脚手架早已创建、记忆文件也提了，技能却没告诉 Test 在哪）、判定产品缺陷前的反馈回路顺序（公共接缝的既有检查 → 显式期望输出的 CLI / API / UI 动作 → 有界请求或 fixture 的回放 → 合同允许时的 Test 自有 harness → 重复运行；绝不改产品源码来制造可观察性）、导入前的自证清单（每步恰一次、定位符可解析、未通过的步骤都有分类、未跑的步骤是 blocked 而不是省略、没有秘密与本地路径）都进了文本。

**移植三：Test 窗口自己记录证据。** 真实宿主暴露的缺口：测试决定 `accept` 只接受 `completed` 的结果，`completed` 要求每一步都引用本 Demand 的受管证据，而 Test 的运行没有第二个观察者——按 §13.123 D7 的实现侧规则（目标以 needs-review 导入、Controller 记证据后 accept），Test 结果根本无法被接受。裁决：实现目标仍不记证据，Test 窗口把每一步的原样输出写到 Test 面下（例如 `fixtures/<demandId>/<stepId>.txt`），用 `wakeflow_record_evidence`（kind `test-output`，managed-path 来源指向该支撑面；preview 看隐私扫描，apply 拿定位符与摘要）记成本 Demand 的受管证据，再以 `completed` 逐步导入；Controller 的独立检查仍是 accept 的前提。`wakeflow_record_evidence` 因此被两份技能教到（D4 b 不受影响，D5 的归属表不含证据记录），Controller 的 `evidence.md` 写明了这条分工。

**移植四：看板视图带撤回原因。** 现场里 Design 两次如实报告"看板查询不显示撤回原因，这句原因我没法从看板上读回来核对"——状态文件里有 `withdrawal{reason, withdrawnAt}`，`inspect_board` 的条目却没投影它。现在 list 与 package 视图的条目多一个 `withdrawal`（`superseded-by:<后继>`、`demand-cancelled:<demandId>` 或撤回请求里的原因；未撤回为 null），schema 与生成类型同步。

**回归。** 内核看板测试新增并发用例；诚实性门新增两条（frontmatter 键、reference 闭合）；requirement 切片测试断言被替代包与用户撤回包的 `withdrawal`；技能文本在预算内（design SKILL 107 行、reference 180 行，test SKILL 99 行、reference 154 行，controller evidence 95 行）。焦点集 17/17（诚实性 10、内核 3、requirement 4）。

**门。** `npm test` 全绿：typecheck、architecture、Biome lint（714 文件）与 format（245 文件）、knip、TypeScript 测试 1020/1020（含二十个端到端场景）、schema 漂移检查、`build:check` 与已提交制品一致；`npm run smoke:artifacts` 通过；`git diff --check` 干净。

**现场。** 第六轮联合运行，仍是 WakeflowTestWorkspace 的八个窗口（Design、Test 先经 close + launch + replace + mark 重开以载入新技能文本；Controller 未重开，这一点后面成了现场的发现）。**Design（Opus）**：按新技能起草并预览一份 real-environment 的需求包（10 条验收标准，1–5 交真实环境、6–10 由 Controller 核对；`parked.trigger` 为"用户确认第六轮可以开始"），preview 只因 `user-confirmation-missing` 阻塞，摘要原样给我确认后补记确认时间、再预览、apply 为 parked（`requirement_92349d0c…` rev 1）；触发条件成立后它先只读复核基线，再从 package 视图取 `stateDigest` 做 `activate`（rev 2，pending）；更正（第二行改为 `round six verified`，44 字节）以 `supersedes` 发布，同一次 apply 把原包撤回为 `superseded-by:requirement_2bb96620…`（rev 3），新包 `requirement_2bb96620…` pending；占位 research 包以 parked 发布后立刻 `withdraw`（原因"占位包，撤回路径验证完成"，rev 2）。四种看板动作都按技能新写的程序走，`expectedStateDigest` 每次都取自动作前的 package 视图。**Controller（同一会话 `80580357…`，逐步 `/wakeflow:next`）**：认领为 `demand_e70f27e0…`（04:31:01Z）；规划实现任务包（10 个锚点对应 ac-1 到 ac-10）；准备投递、助手 deliver 一次、`landing: observed` → accepted；AlembicPlugin 3m55s 落地文件（44 字节、`87366ae7…`）并以 needs-review 导入、回调落地；Controller 只读复核 10 个锚点、登记 6 条证据、accept 并绑 `anchorEvidence`；规划测试任务包（冻结合同 5 步 ts-1…ts-5、`reuse-existing`、允许技能 test 与 target、`maxAttempts: 2`、4 条停止条件；环境成员由 Wakeflow 从 landing 派生），准备、投递到 Test 窗口（landing observed → accepted）。**Test（Opus，新技能）**：`wakeflow_status` 确认落地在 main 的 `7b2c53a`，五步只读执行，每步原样输出写到 `Test/fixtures/demand_e70f27e0…/ts-N.txt`，逐条 preview + apply 登记为 `test-output` 证据（5 条），以 `completed` 逐步导入（5 步全 pass，Wakeflow 派生 verdict pass，stream rev 21），回调经助手发回（landing observed）；报告带覆盖范围与限制段（ac-6–10 不在合同内、macOS `wc -c` 的前导空格、每步只跑一次），没有 tool 错误，产品检出未动、`harnesses/` 为空。**Controller 测试评审**：等 Test 窗口结束，逐步核对五条证据摘要，自己再跑一遍探针并登记为第 12 条证据，记 accept（评估 satisfied / sufficient，独立检查全过），stream rev 23。**完成前的发现**：`wakeflow_verify` 报 `host-settings-assets: claude-code:tmux.mjs:drift`；Controller 自己查清原因——磁盘上的助手与当前构建逐字节一致（`e40f36af…`），报 drift 的是本会话 23:03Z 启动、早于第四轮更新的 MCP 服务进程——并明确建议不要用旧服务去 reconcile（会把助手改回旧版），请用户重连 MCP 或 `claude --resume` 本会话。我用第四轮的路径处理：`resume --window` 关掉 %18、在 %27 里续同一会话、`relocate`、`mark`，新服务进程载入当前代码，verify 14/14；再一次 `/wakeflow:next`：Controller 确认重连生效、重新 preview（planDigest 不变）并 apply，Demand 归档到 `archives/demand_e70f27e0…/0000000024`（manifest 52 个文件、220,943 字节，含 12 条证据），需求包 archived。终态：board archived 5 / withdrawn 3、status idle、verify 14/14，AlembicPlugin 五个未跟踪冒烟文件、HEAD 未变，八个 pane 全活。**宿主传输错误两次**：Design 的更正 apply 与 Controller 的测试评审各遇一次 "API Error: Connection lost mid-response"；两个窗口都先查状态再决定——Design 确认第一次 apply 没落地后用同一个 planDigest 重发，Controller 确认决定已记录且未重复后补上报告——preview 摘要加 CAS 的一次性提交在这两次中断里都没有产生重复或丢失。从 Design 第一条 prompt（04:07Z）到归档（05:20:55Z）共 74 分钟，其中含一次代码修改与我的等待。

**残留。** D8 与 D9 仍待用户裁决。窗口的 MCP 服务进程早于制品更新时，`verify` 会把助手资产报成 drift，而 reconcile 建议会指向旧代码：这是宿主生命周期的固有约束，本轮由 Controller 现场诊断并指引，还没有机器可读的"服务进程版本"信号（候选：`wakeflow_status` 报告加载中的制品 manifest 摘要，与磁盘上的比对）。Test 记录自己的 `test-output` 证据是本轮的裁决，与实现目标的 §13.123 D7 路径分工不同，已写进两份技能与 Controller 的 evidence 参考；Codex 宿主没有真实会话测试。旧实现的方案比较、切片、回归建议等方法按 ADR-0011 放弃。

## 13.127 逐模块对齐第六轮：宿主中断、制品更新与窗口刷新——投递幂等、runtime-artifact 门、四份技能的中断规则（2026-09-25）

**背景。** 用户在第五轮之后要求把 "API Error: Connection lost mid-response" 这类宿主错误纳入 Wakeflow 运行时可能遇到的问题并给出规避方案，同时按需更新 tmux 各窗口。第五轮现场恰好提供了三个样本：Design 的更正 apply 与 Controller 的测试评审各被连接中断切断一次；Controller 在完成前发现自己的 MCP 服务进程早于第四轮的制品更新，`verify` 把磁盘上正确的助手报成 drift，而 reconcile 的建议会用旧代码把它改回去。本轮对照的是旧实现的传输与生命周期模块里与"中断后再来一次"有关的已验证行为（`wakeflow-claude-transport.mjs` 的 M4-T08 "paste uncertainty records ambiguous once, retains the lease, and never resends"、M3-T07 的 settled-replay，`wakeflow-delivery-orchestration.mjs` 的 run-first / replay），以及新实现每个效果工具的重放语义。

**行为对照。**

| 旧实现已验证的行为 | 新实现 | 状态 |
|---|---|---|
| 粘贴不确定时记一次 ambiguous、保留 lease、**从不重发**（M4-T08） | `record_delivery_outcome` 的 indeterminate 保留声明、从不重发；本轮再加助手侧的落地幂等：同一段 prompt 已有 `user-prompt-submit` 记录即拒 `already-landed` | 同形 + 移植 |
| 已 settled 的投递重放只回既有结果，不产生第二次运行（M3-T07） | `prepare_delivery` 同 key 同许可、`import_target_result` 结果 id 由声明围栏派生、评审决定同 key 重放 | 同形 |
| run-first：先落不可变 run 再提交 settlement，恢复只前向完成 | 效果工具 preview 摘要加 CAS 的一次性提交；`recover` 只前向完成（ledger、Demand、证据） | 同形 |
| 死 pane / 会话续接：resume 复用句柄、换定位器代际 | §13.125 的 `resume` + `relocate`，`resume-exited` 时 launch + replace | 同形（上轮移植） |
| 插件更新后旧进程继续跑 | 旧实现没有信号（runtime-meta 已放弃）；**移植**：hook 记录带制品 manifest 摘要，status / verify 判 stale 与 outdated | 移植（新） |
| 中断后的 Agent 规则 | 旧技能没有专门段落；**移植**：四份技能各一段 "After an interruption"，Controller 参考一节 "After a plugin update" | 移植（新） |

**移植一：助手 `deliver` 的落地幂等。** 宿主发送是 Wakeflow 唯一不能替 Agent 重放的效果：一轮在 `deliver` 之后、登记结果之前被切断，重来时 Agent 只知道"许可还在"。现在助手在 pane authority 之后、粘贴之前先按绑定会话与 prompt 摘要查 `user-prompt-submit` 记录，已有即拒绝为 `already-landed`（`attempt: failed-before-send`）并交回 `landing{recordId, recordedAt}` 与提示，`--force` 才照发；Controller 用交回的落地登记结果，而不是第二次投递。

**移植二：`runtime-artifact` 门与窗口的制品状态。** hook 记录多一个可选键 `artifactManifestDigest`（观察脚本从自身所在制品的 `artifact-manifest.json` 读字节摘要；旧记录读成 null；键集校验改为必需键加可选键，schemaVersion 不变）。MCP 服务进程启动时固定一次自己的制品身份（`src/entrypoints/wakeflow-artifact-identity.ts`），观察门面带上它：`wakeflow_status` 顶层 `runtime{artifactManifestDigest, artifactOnDisk: same | changed | unknown}`，每个窗口 `artifact: current | stale | unknown`（绑定会话最近一次 session-start 记录里的摘要与本进程的比对），`nextActions` 新增 `runtime-artifact-outdated`（owner user：本窗口的服务要由用户重连）与 `window-artifact-stale`（owner controller，subject 为 windowId：用助手 resume）；`wakeflow_verify` 第 15 门 `runtime-artifact`（`server-outdated` / `windows-stale:<n>`，没有 manifest 的运行记 not-applicable 而 pass）。hook 观察入口的闭包仍只有 foundation 与 kernel（manifest 用 URL 相对解析读取，不引入 node:url）。

**移植三：中断规则进技能。** Controller SKILL "After an interruption"（先看再做：status 与工具自己的 inspect 视图；已落就报告，没落就用同一 idempotencyKey / planDigest 重放；宿主发送先查落地）与参考 "After a plugin update"（其他窗口用助手 resume、`resume-exited` 时 launch + replace；本窗口只能由用户重连 `/mcp` 或 resume 会话；不要从过期的服务跑维护）；delivery 参考写明 `already-landed`；Design / Target / Test 各一条 bounded expectation（看板 / status 先看，再重放；证据 apply 重放为 already-recorded；不因为报告丢了就重跑步骤）。能力卡 10 新增 10.6 "宿主中断与制品更新"的规避矩阵，能力卡 6、9 与能力图同步。

**回归。** 内核 hook 记录测试新增可选键往返（旧记录 null、新记录原样）；观察脚本测试新增 launcher 摘要透传；决定测试新增 `runtime-artifact` 四种判定与两种 nextAction；切片测试新增带制品身份的门面（unknown → current → stale、`artifactOnDisk: changed`、门 `server-outdated,windows-stale:1`）；助手测试的落地用例改为先拒 `already-landed`、`--force` 才发；门数 14 → 15 的期望在决定、切片与场景测试同步。观察脚本 `observe` 的认知复杂度被 `??` 推到 16，改在 `recordInput` 里归一。焦点集 59/59（内核 12、脚本 8、决定 20、切片 13、助手 10 的合计）与诚实性加制品 16/16。

**门。** `npm test` 全绿：typecheck、architecture、Biome lint（715 文件，四条既有的 warning 不是错误）与 format（246 文件）、knip、TypeScript 测试 1024/1024（含二十个端到端场景）、schema 漂移检查（95 份 schema）、`build:check` 与已提交制品一致；`npm run smoke:artifacts` 通过；`git diff --check` 干净。第一次跑门时观察脚本的 `observe` 因新增的 `??` 达到认知复杂度 16 被 lint 拒绝、场景测试两处门数期望还是 14，修正后重跑才全绿。

**现场。** 仍是 WakeflowTestWorkspace 的八个窗口。**窗口刷新。** 刷新前用当前构建的制品副本看 status：`runtime.artifactOnDisk: same`，八个窗口的 `artifact` 全是 `unknown`（它们的 session-start 记录早于新字段）。先把插件缓存刷成本轮构建（hook 从缓存跑，session-start 才带摘要），再用脚本对每个已登记窗口做 close → `resume`（`resume-exited` 时改 launch）→ `relocate` / `replace` → `mark`：Controller、Design、Test、AlembicPlugin 四个有对话的会话都 `resume` 成功（hook observed，`relocated`，新 pane %28 / %29 / %30 / %39）；Alembic、AlembicAgent、AlembicCore、AlembicDashboard 四个从未有过对话的会话都被 `resume-exited` 拒绝后 launch 成功（hook observed，`replaced`，%32 / %34 / %36 / %38）；八个窗口 mark 成功，全程 3 分 12 秒。刷新后 status：八个窗口 `artifact: current`，`nextActions` 没有制品项；verify 只剩一门 `host-settings-assets: claude-code:tmux.mjs:drift`——工作区里的助手还是第五轮的，本轮改了它（`already-landed`），于是 reconcile 一步 `asset:claude-code:tmux` 部署新助手，verify 15/15。**投递幂等。** 对刚启动的 Alembic 窗口用工作区里的助手连发同一段良性 prompt：第一次 `sent`、`landing: observed`（记录 `05d76a81…`），第二次 `already-landed`、`attempt: failed-before-send`、交回同一条落地记录与提示；hook 目录里这段 prompt 的记录恰好一条。**制品信号的两种故障（用制品副本模拟，工作区零写）。** (a) 副本的 manifest 末尾加一个换行再起服务——相当于插件更新后旧窗口继续跑：status 八个窗口全 `stale`，`nextActions` 八条 `window-artifact-stale`（owner controller），verify 门 `windows-stale:8`；(b) 服务运行中再改一次 manifest：`runtime.artifactOnDisk: changed`，`nextActions` 多一条 `runtime-artifact-outdated`（owner user），verify 门 `server-outdated,windows-stale:8`。**上一轮的两次连接中断**（Design 的 apply、Controller 的评审决定）与 Controller 的手工诊断（`ps` 找到自己 23:03Z 启动的旧服务进程、建议用户重连而不是 reconcile）是本轮规则与信号的直接来源；本轮没有再遇到连接中断。

**残留。** D8 与 D9 仍待用户裁决。制品信号只在 hook 与 MCP 服务来自同一份制品时才能判 current（正式安装如此；本仓库的开发布局里 hook 走插件缓存、服务走仓库制品，两者要一起刷新，否则新会话也会被判 stale）。"本窗口的服务要重连"仍是用户动作（`/mcp` 或 resume 会话），Controller 只能提示；Claude Code 的 `/mcp` 重连是否总能换到新代码尚未在现场验证（本轮用助手 resume 代替）。Codex 宿主的 hook 同样会写摘要，但 Codex 真实会话测试仍未执行。下一轮：pod 与 worktree。

## 13.128 逐模块对齐第七轮：pod 与 worktree——对照、检出占用守卫、拉起顺序，第一个真实 pod（2026-09-25）

**背景。** 第七轮对照旧实现的 Pod 领域：`wakeflow-pod-service.mjs`（7,062 行：inventory、materialization、Test access、close 四段）、`wakeflow-pod-records.mjs`（九类证据 codec）、Claude 与 Codex 的 pod 宿主适配器，以及六个测试文件（`wakeflow-pod-service-v3`、`wakeflow-pod-evidence-v3`、`wakeflow-pod`、`wakeflow-claude-pod-host`、`wakeflow-codex-pod-host`、`wakeflow-pod-design-v3`）。新实现是 ADR-0010 的 pod 模型：`src/capabilities/pod/`（`wakeflow_pod` 的 create 与两段 close 走配置事务，recover 对账回执）、`src/kernel/pod-worktree-receipts.ts`（porcelain 解析、检出准入、回执存储）、`src/governance/pod/`（状态派生、处置引导）、端点切片里 worktree 观察的准入与 Test 窗口的附加目录，以及 Controller 技能的 "Pods" 一节。此前七次联合运行都在 main pod 里跑，pod 从未在真实宿主上创建过。

**行为对照。**

| 旧实现已验证的行为（测试名摘要） | 新实现 | 状态 |
|---|---|---|
| 一条严格的 append-only 物化链，脱敏瞬态宿主标识；失败的尝试保留、重试要显式授权 | 没有物化链：pod 记录在 `wakeflow.config.json` 的 `pods[]`，状态由配置 lifecycle 加窗口绑定与 worktree 回执派生，不落盘（ADR-0010 D6） | 有依据的重切 |
| 只登记已定稿的 Pod 身份，先建核心回执再绑状态；创建回执在每个 state journal 边界前向恢复 | create 是一次配置 CAS 事务；同键重放 `already-created`，零写；恢复只做回执对账 | 同形（重切） |
| 只重探一个非主产品 worktree，回执提交后才激活 | 产品窗口 register 必须带 porcelain 与 common-dir 原文，会话 cwd 必须是配置仓库的一个非主检出，指针互相印证；准入后同一互斥门内写回执；全部窗口绑定且回执与绑定同代、检出仍在才 ready | 同形 |
| **在身份登记前拒绝被另一个当前 Pod 占用的 worktree** | 无：另一个 pod 的回执还指着同一检出也能登记 | **移植** |
| Test access：冻结一份私有根计划，接受产品 HEAD 变化，只在每个冻结的产品 worktree 都绑定后才准备访问计划；Git 不匹配由原始观察派生，拒绝调用方自报的判定 | Test 窗口不建 worktree，启动意图把已有回执的 worktree 列为 `--add-dir` 附加目录；判定只来自 porcelain 与文件系统事实 | 同形（**技能文本补拉起顺序**） |
| close：intent 与回执分离；only unmaterialized member may be not-found；released 不臆造 worktree 归属；Codex 归档留在人工门后 | 两段关闭：第一段记分支处置（无活动 Demand、每个已登记 worktree 都有处置），第二段要求窗口全部退役、检出全部不在；Wakeflow 从不删 worktree，只给 `git worktree remove` 建议与宿主备选 | 同形 |
| Pod Design 请求 / 交接、product append 的四段事务 | 放弃：不做 Design 跨 pod 交接（ADR-0010 D6） | 放弃 |
| state-selected evidence 丢失是权威损坏，重试从不重建 | 回执丢失让 pod 回到 creating；同句柄重放 register 会按当前观察补写回执 | 有依据的重切 |
| Claude 宿主：先查唯一 correlation 再同步 create；恢复复用同一会话、从不 create | 启动意图 + 助手 `launch --worktree`；重放 register 为 replayed；resume 走 §13.125 | 同形（ADR-0009） |
| Codex 宿主：saved-project 根、有界列表预览、只定稿唯一 correlation；thread id 瞬态 | Codex 产品窗口即 Worktree 线程，detached HEAD 在导入前必须建分支；Codex 真实会话未测 | 同形（未验证） |
| 公共面：state-first 的 Pod 操作族，拒绝退役的扁平动作 | 一个工具 `wakeflow_pod`（preview / apply / recover）替代 `pod_open` / `pod_bind` / `pod_plan` / `pod_record` | 同形（重切） |

**移植一：检出占用守卫。** 旧实现 T03 在身份登记前拒绝被另一个当前 Pod 占用的 worktree；新实现的准入只核对会话 cwd 是配置仓库的一个非主检出，另一个 pod 的回执还指着同一路径也会登记成功并造出两个 pod 共享一个检出。现在内核多一个 `findPodWorktreeReceiptByPath`（本宿主上除本 pod 外、回执路径等于该检出的第一份回执），端点在准入之后、写回执之前查它，命中即 `precondition-failed / worktree-occupied`，`details.podId` 指出占用者。回归：内核回执测试的占用查找（命中、排除自己、别的路径不命中），pod 生命周期测试里第二个 pod 拿同一检出登记被拒，随后两段关闭这个没有登记与回执的 pod（不需要分支处置，直接 closing → closed）。

**移植二：拉起顺序进技能。** Test 窗口的 `--add-dir` 列表来自产品窗口登记时写的回执，先起 Test 窗口它就看不到任何 worktree——旧实现 T04 "只在每个冻结的产品 worktree 都绑定后才准备 Test 访问计划"讲的就是这个顺序。Controller 参考的 Pods 一节现在写明：Controller、Design → 每个产品窗口带 worktree 观察登记 → 最后 Test；并写明 `worktree-occupied`。

**移植三（现场逼出的）：请求隐私扫描对 worktree 观察的豁免。** 第一个真实 pod 拉起时 5 个产品窗口全部登记失败：命令外壳对每个请求做隐私扫描（工作区根与 home 的路径一律拒绝），而 worktree 登记又要求 `git worktree list --porcelain` 原文，真实宿主上仓库在工作区根之下、检出又在仓库里，porcelain 必然含根路径——单元与场景测试没有发现，因为它们的夹具把产品仓库和检出放在工作区根旁边。现在 `CommandShellSpec` 多一个 `requestPrivacyExemptPaths`（点分字段路径，只给"内核准入要原文、只进私有回执、从不回到公共结果"的字段），绑定工具豁免 `observation.worktree`；结果侧的公共边界不变。回归：命令外壳测试（豁免字段里的根路径放行、别处的仍拒绝、无豁免即拒绝），pod 生命周期测试把检出改到工作区根之下（`.wakeflow-test-worktrees/`），让夹具和真实宿主同形。

**移植四（现场逼出的）：绑定变更后重算活动投影。** pod 的 8 个窗口登记完毕后，`wakeflow_verify` 报 `active-projection: stale,stale`，而 reconcile 的预览是 ready 且零步骤——修不了。原因：活动投影的指纹含每个 pod 的 worktree 事实与生命周期，产品窗口登记写回执就改变了它，但端点切片的变更（register / replace / relocate / decommission / release-claim）是唯一没有在提交后调用 `afterMutationRefresh` 的变更执行器（pod、Demand、投递、评审、证据、维护都调）。现在端点的两条变更路径都包在 `afterMutationRefresh` 里；pod 生命周期测试在产品窗口登记后立刻 verify，要求 `active-projection` 门 pass（此前会 stale）。

**移植五（现场逼出的）：关闭步骤的锁与登录过期。** 关闭时 status 建议的 `git worktree remove` 对 Claude Code 加过锁的检出会失败（窗口退役后锁仍在），Controller 参考的关闭第 3 步写明先 `git worktree unlock` 再 remove 与 prune、Wakeflow 从不删检出；main Controller 在退役到第六个窗口时遇到 "Login expired · Please run /login"，Controller SKILL 的 "After an interruption" 补一句：只有用户能登录，登录后宿主可能自动续上被切断的一轮，续上后仍先看再做。能力卡 10.6 的中断矩阵加登录过期一行，ADR-0010 补充记锁的事实。

**回归。** 焦点集 9/9（内核回执 3、pod 决定 4、pod 切片 2）加端点切片；诚实性门通过；参考 208 行、SKILL 180 行，都在预算内。

**门。** `npm test` 全绿：typecheck、architecture、Biome lint 与 format、knip、TypeScript 测试 1025/1025（含二十个端到端场景，246 秒）、schema 漂移检查（95 份 schema）、`build:check` 与已提交制品一致（claude-code 929 文件、codex 902 文件）；`npm run smoke:artifacts` 两宿主 idle / verify ok / hook 落地；`git diff --check` 干净。门跑了两次：第一次在 pod 关闭完成后（1025/1025），第二次在移植五的技能文本之后（含冒烟），提交的是第二次的字节。

**现场。** 第一个真实 pod，全部由 main 的 Controller 按技能驱动（我只发 `/wakeflow:pod create round7`、确认与 `/wakeflow:next`）。**创建。** preview 一步给出 8 个窗口、5 个 worktree 意图与 podId，Controller 把计划与它随后要做的宿主动作（建 worktree、按 Controller → Design → 产品 → Test 的顺序起窗口、可能的信任对话框）讲清后才 apply；apply 只是一次配置事务（`pods[]` 多一项、窗口 16 个、状态 creating）。**拉起与第一次失败。** Controller 用 `claude --worktree wakeflow-round7` 起了 7 个窗口（Test 按新写的顺序留到最后），Controller 与 Design 登记成功，5 个产品窗口全部被 `privacy-violation` 拒绝（`observation.worktree.porcelain` 含工作区根路径）——移植三的缺口；它没有改写 porcelain 绕过检查，而是停下报告并给出修法建议。同时它报告了两件事：`claude --worktree` 从 origin/main 建检出而不是本地 HEAD（本地 main 落后 origin 12 到 38 个提交），以及 5 个主检出多出一行 `?? .claude/worktrees/`。**修复后继续。** 我加上请求隐私豁免、重建制品并用助手 resume 了 main Controller（同一会话，新服务进程）；它用保存的观察结果登记了 4 个产品窗口；按我的要求对 AlembicAgent 做实验：关窗口、解锁并删除 worktree 与分支，用 `git worktree add -b worktree-wakeflow-round7 .claude/worktrees/wakeflow-round7 HEAD` 从本地 HEAD 重建，再用原意图（带 `--worktree wakeflow-round7`）launch——session-start 的 cwd 正是那个检出、HEAD 等于本地 main，claude 复用了已有检出只加锁；随后登记成功。它在 5 个仓库的 `.git/info/exclude` 各加一行 `.claude/worktrees/`，`git status` 回到原行数（四个 0 行，AlembicPlugin 5 行冒烟文件）。最后起 Test 窗口：启动意图列出 5 个 worktree 作 `--add-dir`。pod `ready`，8/8 绑定，5 份回执。**第二个缺口。** 此时 `wakeflow_verify` 报 `active-projection: stale,stale`（外加预期中的 `runtime-artifact: windows-stale:13`），reconcile 预览 ready 且零步骤——移植四的缺口；修复后重建制品，用脚本把 16 个窗口全部换到当前制品（4 个有对话的 resume、12 个新会话 launch，pod 产品窗口的 `--worktree` 复用检出，9 分钟），status 16 个 `current`，verify 15/15（replace 变更把投影重算回来）。 **pod 里的 Demand。** main 的 Design 发布一份包（AlembicPlugin，在 round7 worktree 分支上新增并提交 `docs/wakeflow-pod-round7.md`，48 字节，controller-only），`requirement_a732b942…` pending。round7 的 Controller（新会话，凭 tmux 窗口选项认出自己是 pod Controller、不是 CLAUDE.md 点名的 main Controller）逐步 `/wakeflow:next`：认领为 `demand_ca50b2fa…`（pod-busy 只对本 pod 生效，main 仍可认领别的包）；规划实现任务（第一次 `authority-reference-unknown`：它把裸文件名当成需求文档引用，零写，改用记录里的引用重试成功；10 个锚点；停止条件"起点提交变了就 blocked"）；准备、投递到 round7 · AlembicPlugin（landing observed，记录 `f2278065…`）；产品窗口在 worktree 分支上落地并只提交这一个文件（`b17d52c`），以 needs-review 导入、回调落地（约 5 分钟）；pod Controller 评审时遇到两次 "API Error: Connection lost mid-response"，都发生在只读调用之后——按"After an interruption"先看再做，没有重复或遗漏写入——登记证据、accept；再一次 `/wakeflow:next`：verify 15/15 加 Demand 7 门、完成预检 8 门、归档 `archives/demand_ca50b2fa…/0000000010`（19 个文件、107,024 字节），需求包 archived；主检出 HEAD 仍是 `7b2c53a`。 **关闭。** 我发 `/wakeflow:pod close round7` 并确认五个分支都按 `abandoned` 记录（round7 只是联合测试，AlembicPlugin 的 `b17d52c` 不合并回主线，其余四个分支没有独有提交），要求 Controller 按参考顺序一路做完。它 preview → apply 记下 5 条处置（`closing`，08:46:34Z），然后逐个退役窗口：助手 `close` 加 `decommission`，AlembicPlugin、Alembic、AlembicAgent、AlembicCore、AlembicDashboard、Test 六个（窗格 16→10，六次 `decommissioned`，每次的关闭证据它都存了文件）；其间两次 "API Error: Connection lost mid-response"（08:49、08:59）与一次上下文压缩（08:55），每次我发一句"继续完成"，它按 "After an interruption" 先 `panes` 与 status 再动手，没有重复退役；退到第六个之后宿主报 "Login expired · Please run /login"，会话停住。**登录过期与外部续做。** `/login` 只有用户能做。等待期间我用脚本从外部续做：它关掉并退役了 round7 · Controller 与 round7 · Design（两条 session-end 记录 09:11:55Z、09:11:57Z，绑定与定位器文件在同一秒被退役），随后在已退役的 Test 窗口上因我自己的脚本缺陷停下——inspect 对未绑定窗口返回的是 `binding: {status: "unregistered"}` 而不是 null，脚本把它当成已绑定，发出了没有 `expectedBindingId` 的 decommission，被输入校验拒在 `$`；worktree 没有动。用户在 09:25:51Z 于 Controller 窗口执行 `/login`（Login successful）后，Claude Code 自动续上被切断的那一轮：Controller 先 `panes`、status、inspect 两个窗口，发现它们已是 unregistered，翻 hook 记录与 pod Controller 的会话记录确认"不是本会话退役的、结果已落地、不重放"并如实报告；核对五个 worktree 干净、锁都是已退出的 claude 进程加的，逐仓库 `git worktree unlock` → `git worktree remove` → `git branch -D`（status 建议的 `git worktree remove` 对加锁检出会失败，见残留）；第二段 preview `close-complete` 零阻塞（8 个窗口解绑、5 个 worktree `checkout-missing`），apply `closed`，退役 5 份回执；status `idle`、pods 只剩 main 8/8、窗口 16→8、verify 15/15，五个主检出 HEAD 不变、status 行数与创建前相同（四个 0 行，AlembicPlugin 5 行冒烟文件），5 分 55 秒。我随后用独立服务进程复核：status 与 verify 同值，board archived 6 / withdrawn 3，`unmergedAccepted` 空；每个仓库只剩主检出与 `main`，空的 `.claude/worktrees/` 目录与 exclude 行保留。**收尾。** 这一轮结束时制品又构建了一次（技能文本改动），插件缓存与八个 main 窗口随后刷新到新制品（见门）。

**残留。** D8 与 D9 仍待用户裁决。本轮的宿主中断：pod Controller 评审两次 API Error，main Controller 关闭 pod 时两次 API Error 加一次上下文压缩，三个窗口都按第六轮的规则先看再做，没有一次重复或遗漏写入；但被切断的一轮不会自己续上，每次都要人再发一句"继续"——这是宿主的边界，Wakeflow 只能保证续上时的状态可判定。新看到的一种中断是登录过期（"Login expired · Please run /login"）：只有用户能 `/login`，Claude Code 在登录成功后自动续上被切断的那一轮，Wakeflow 不需要新机制，Controller 技能的中断一节补了这一句。status 建议的 `git worktree remove` 对 Claude Code 加过锁的检出会失败，要先 `git worktree unlock`；本轮只把这一步写进技能与 ADR-0010 补充，候选做法是用回执里的 `locked` 标志给建议命令加 unlock 前缀，但关闭时的锁状态 Wakeflow 并不观察，所以暂不做。`unmergedAccepted` 只从活动 Demand 派生：Demand 归档后，它接受过的 worktree 分支即使没合并也不再出现在 status 里，pod 关闭的第一段仍会逼一次处置，但两次关闭之间的窗口期里 status 沉默——候选做法是从 pod 的 worktree 回执加已归档结果补这一列。`claude --worktree` 从远端默认分支建检出这一行为只在这一个 Claude Code 版本上核实过；`basePolicy: local-head` 现在靠"先 git worktree add 再 --worktree 复用"的配方实现，而不是宿主保证。一个 pod 8 个窗口意味着一台机器上 16 个 Claude 会话，拉起（连修复）与关闭各花了十几分钟，主要是每个窗口的启动等待与登记；Codex 宿主的 pod 路径没有真实会话测试。

## 13.129 逐模块对齐第八轮：观察与 verify——对照、投影覆盖提示、维护协议进 status、零写与隐私回归、冒烟的私有路径门（2026-09-25）

**背景。** 第八轮对照旧实现的观察域：`wakeflow-observability-v3.mjs`（2,475 行：一次观察、config / storage / status / verification 四份投影、15 个门、下一步动作）、`wakeflow-local-layout-inspection.mjs`（1,720 行）、活动投影器（`wakeflow-active-projection-service` 27 个测试、`-maintenance` 5 个）、公共面（`wakeflow-mcp-verify-surface` 5 个）、宿主激活门（`wakeflow-host-activation-gate` 7 个）与候选制品门（`wakeflow-candidate-artifact-gates` 4 个）。新实现是 `src/capabilities/observation/`（`wakeflow_status` 与 `wakeflow_verify` 走同一个命令外壳、各观察一次）、`src/governance/observation/`（逐域隔离失败的一次观察、投影事实、刷新）、`src/kernel/active-projection.ts`（页面、标记、指纹、四类目标、投影锁）与 `tooling/artifacts/smoke-plugin-artifacts.ts`。这是逐模块对齐计划的最后一轮。

**行为对照。**

| 旧实现已验证的行为（测试名摘要） | 新实现 | 状态 |
|---|---|---|
| 未初始化 / 旧版 / 无效配置：不回退、不写、不泄露配置内容，status 报 uninitialized / migration-required / degraded | 没有配置即 `precondition-failed / config-authority` 错误信封（脱敏、不回显根），没有迁移分支（配置只有 v1）；未初始化的路是维护工具的 fresh preview | 有依据的重切（**补回归**：非工作区目录零写） |
| 投影确定、有界、私有、只读、只认签发的观察对象 | 结果经 Schema 准入有界；命令外壳对结果扫根路径、句柄、hook 会话、worktree 路径；observedAt 由注入时钟固定；没有跨调用令牌，观察对象不出进程 | 同形（**补回归**：零写目录树快照、两次同字节、配置里的 tmux 会话名与 socket 名不出结果） |
| settings / assets 适配器按宿主精确适用 | 组合根固定提供每个宿主的 profile 与状态栏资产声明 | 同形 |
| 观察模块没有退役的读写依赖 | knip 与依赖方向规则 | 同形 |
| 公共面：每个工具一个闭合信封与精确操作集；只读 / 破坏性 annotation 矩阵；`wakeflow_status` 是唯一的活状态；旧扁平请求被拒 | 十八个闭合 Schema 工具，目录测试逐工具比对 annotations；`wakeflow_view` 放弃（ADR-0006） | 同形 |
| verify 15 门（config-authority、local-layout、active-authority、ledger-authority、transport-authority、coordination-leases、pod-evidence、active-projection、window-identity、window-runtime-projection、maintenance-gate、owner-contract、managed-drift、repository-roots、storage-inventory），`ok` 要全 pass，unavailable 分开计 | 15 门（config-authority、local-layout、ledger-layout、board-consistency、demand-root-audit、append-candidates-clear、evidence-integrity、work-claims、host-hook-channel、window-identity、window-runtime-projection、pod-execution-location、host-settings-assets、active-projection、runtime-artifact）；transport 进 Demand 事件流由 demand-root-audit 覆盖，owner-contract / managed-drift / storage-inventory 并入 local-layout 的对账预览，maintenance-gate 经预览的核心布局检查报 `maintenance-protocol-<状态>` | 同形（重切；**工具描述仍写"thirteen gates"，移植一**） |
| status 的 `maintenance` 域（state、lockPresent、operationCount）与 overall 的 maintenance 优先 | 无：overall 只看活动布局；被打断的维护 apply 只在 verify 的 local-layout 里可见，status 报 idle | **移植三** |
| status 的 git 观察（dirty / ahead / behind）与 `repository-git-dirty` 动作 | §13.94 D4：不 spawn git，只读仓库指针文件 | 有依据的重切 |
| 下一步动作去重排序上限 256、每项带 sourceRefs | 上限 64，`{owner, tool, reason, subject}`；维护 > 活动 pod 未登记窗口 > Demand 前沿 > 待认领 | 同形（重切） |
| 活动投影：确定、0600、对 mtime / TODO / 本地运行时不敏感；多行标签折叠；路径百分号编码 | 指纹只随绑定事实变化；0600；`inline()` 去控制字符压空白有界 | 同形 |
| 活动投影：读不出的 current 根报"覆盖未知"，创建锁在时绝不发布假 idle；损坏的根保留旧页 | 退休证据只在看全时退休（§13.94 D5）；但工作区页把读不出的 Demand 直接丢掉，看起来像 idle | **移植二** |
| 活动投影：符号链接、多重硬链接、手写目标整轮零写；stale / missing 窄重建；两个投影进程经共享锁收敛；只改四类投影文件 | 内核测试逐项覆盖（unsafe 零写、手写零写、锁争用 projection-contended、退休证据） | 同形 |
| 活动投影维护参与者（T08 五条） | 维护只建容器，投影由变更后的刷新与 status 自愈 | 有依据的重切 |
| 宿主激活门（M6-T11 七条：无人值守权威） | 放弃：没有无人值守模式（能力卡 10 疑点 3） | 放弃 |
| 候选制品门：验证 / 冒烟 / setup / CLI；冒烟的零写证据；冒烟拒绝解析输出里的私有路径；临时根创建与清理失败结构化 | `build:check`、`smoke:artifacts` / `smoke:candidate`；冒烟没有对工具结果做私有路径扫描 | **移植四** |

**移植一：verify 工具描述的门数。** 公共描述自 §13.111 起就落后于实现："thirteen gates" 列了 12 个名字，而门早已是 15 个（§13.111 补回 window-runtime-projection，§13.127 加 runtime-artifact）。描述改为十五个门的完整清单（516 → 5xx 字节，仍在 640 内），`deriveWorkspaceGates` 的注释与测试名同步改成十五。

**移植二：投影的 Demand 覆盖提示。** 旧实现 T09 的两条断言——"读不出的 current 根报覆盖未知"、"创建锁在时绝不发布假 idle"——在新实现里只有一半：退休证据保证读不出时不删 Demand 页，但工作区两页会把读不出的 Demand 从列表里丢掉，页面看起来像没有活动 Demand。现在投影事实多一个 `demandCoverage`：`complete`（这一轮看全）、`incomplete`（至少一个读不出：两份工作区页在 Demand 列表前加一条"可能缺项"的提示，并进工作区指纹）、`unobserved`（fresh 初始化那条没有观察过的路：不加提示、与 complete 同字节，初始化后的第一次 status 才不会把页面判成 stale）。回归：内核渲染测试的指纹表加 `demandCoverage`，新测试断言提示位置、Demand 页不提覆盖、complete 与 unobserved 同字节。

**移植三：维护协议进 status。** 旧实现的 status 有 `maintenance` 域且 overall 以 maintenance 优先；新实现 overall 只看活动布局，一次被打断的维护 apply（事务残留、失活的锁）只在 verify 的 local-layout 里以 `maintenance-protocol-recovery-required` 可见，status 照报 idle。现在 status 顶层多一段 `maintenance{status, protocol}`，protocol 来自维护工具预览用的同一份核心布局检查（idle / busy / recovery-required / conflict / absent / bootstrap-prefix），非 idle 即 `overall: maintenance`、`next` 指向 `workspace-maintenance`；检查读不出即 unavailable / unknown，不猜 idle。Controller SKILL 的 "Checking the workspace" 补一句：这种 overall 要先用 recover 收尾。第一版曾在 local-layout 门里另加 `maintenance-transactions:<n>` 码，读到预览已经报同一件事后撤掉，避免一个门两套码。回归：切片测试往 transactions 目录放一个残留文件，status 报 maintenance / recovery-required 且 next 是维护、verify 的 local-layout fail 并带 `maintenance-protocol-recovery-required`，清掉后恢复；决定测试补预览阻塞码的用例。

**移植四：冒烟的私有路径门。** 旧实现 T10 让冒烟拒绝解析输出里的私有 sibling-ledger 路径；新冒烟只信命令外壳。现在冒烟的 `callTool` 对每个结果的 JSON 扫一次性目录的根（工作区、ledger 与产品仓库都在它下面），命中即失败——脱敏边界失守时不靠运行时的自查。

**回归（旧实现 T08 的三条不变量）。** 切片测试补：status 与 verify 前后工作区与产品仓库的目录树逐节点（类型、模式、大小、mtime、ctime）相同，同一时钟下两次结果逐字节相同；配置里的 tmux 会话名与 socket 名、绑定句柄、一次性目录根都不进结果（夹具的 fresh 选择带上 `hosts.claude-code.tmux`）；非工作区目录的 status 是 `precondition-failed / config-authority` 且零写。手搭的完成夹具（`.wakeflow-local` 0755）现在被 status 判成 maintenance，归档测试的下一步断言相应放宽。

**门。** `npm test` 全绿：typecheck、architecture、Biome lint（715 文件，四条既有的 warning 不是错误）与 format（246 文件）、knip、TypeScript 测试 1031/1031（含二十个端到端场景；本轮新增 6 条）、schema 漂移检查（95 份 schema）、`build:check` 与已提交制品一致；`npm run smoke:artifacts` 两宿主 idle / verify ok / hook 落地，跑在与提交相同的制品字节上；`git diff --check` 干净。门跑了三次：第一次被 Biome 的格式检查挡下（三个测试文件的长行、两处 import 顺序，`biome check --write` 修正后制品字节变了一次），第二次被 status Schema 里一行过长的 enum 挡下（只改格式，制品字节不变），第三次全绿。

**现场。** 仍是 WakeflowTestWorkspace 的八个 main 窗口。**刷新。** 插件缓存刷到本轮制品（manifest `166f580c…`），八个窗口用脚本换到新制品：五个有对话的会话 resume 加 relocate（Controller、Design、Test、Alembic、AlembicPlugin），三个从未有过对话的产品窗口 `resume-exited` 后 launch 加 replace（AlembicAgent、AlembicCore、AlembicDashboard），2 分 33 秒，status 八个 `current`，verify 15/15。**基线。** 独立服务进程的 status：`overall: idle`，新段 `maintenance{status: observed, protocol: idle}`，`runtime.artifactOnDisk: same`，投影 `current`，`next` 无前沿，看板 archived 6 / withdrawn 3；verify 15/15；`workspace-current-status.md` 616 字节，没有覆盖提示（活动 Demand 看全了）。**故障注入。** 往 `.wakeflow-local/runtime/maintenance/transactions/` 放一个 3 字节的假 intent 文件（文件名编号全零、内容 `{}`）：status 立刻变 `overall: maintenance`、`maintenance.protocol: recovery-required`、`next` 是 `workspace-maintenance`（owner controller，`wakeflow_maintain_workspace`）；verify 14/1，只有 `local-layout` 失败，码 `local:blocked,maintenance-protocol-recovery-required`——与切片测试同形。**Controller 的报告。** 此时向 main 的 Controller 发 `/wakeflow:status`（2 分 12 秒）：它报出 maintenance 与失败的门和码，按 "Checking the workspace" 只读地看了事务目录，指出那个文件不是 Wakeflow 自己写的（合法意图文件叫 `maintenance_operation_<uuid>.intent.json` 且带完整计划，这个缺前缀、编号全零、内容为空）、维护锁没被占着，推断 recover 按编号找不到它、清不掉，明确说这一点没验证"因为这次只是查看状态"，按 CLAUDE.md 没碰 `.wakeflow-local`；其余状态（看板、Demand、pod、声明、仓库）如常，还如实报出"10:47:59Z 到 10:50:21Z 之间 7 个窗口重启了会话、3 个换了 binding，这些不是本会话做的"；下一步先问用户"这个文件是你放的吗"，给出 recover 试一次 / 用户自己删 / 当缺陷处理三个选项，没有自作主张。**恢复。** 删掉注入文件后 status 回到 `idle`、protocol `idle`、无下一步，verify 15/15；告诉 Controller 这是故障注入并已删除后，它 11 秒内再看一次 status 确认 idle，"没有再跑 verify，也没有执行 recover"，停下。

**残留。** D8 与 D9 仍待用户裁决。verify 工具描述的门数从 §13.111 起停在 "thirteen" 两轮没人发现——公共描述里的数字现在由决定测试对着门集合核对，但其他工具描述里的枚举（如 Design 的看板动作、Test 的判定词汇）还没有同类静态门，只靠人读。status 与 verify 报 `recovery-required` 时不点名残留文件、也不说文件名是否是合法的操作编号；recover 只认 `maintenance_operation_<uuid>`，格式错误的残留只能由人删——现场的 Controller 自己看出并指出了这一点，本轮记为残留（候选：`maintenance` 段加残留条目数与"可否 recover"）。`overall: maintenance` 现在也覆盖维护协议非 idle 的所有情形，包括另一次维护正在进行（busy）：并发的两个窗口同时看 status 会在那几秒里都看到 maintenance，这是如实的，不是缺陷。投影的覆盖提示只在工作区两页，Demand 页不提；提示进指纹意味着一次瞬时读失败会让页面变化一次、下一次刷新或变更再变回，与此前"读不出的 Demand 掉出列表再回来"的动态相同；本轮现场没有制造读不出的 Demand，提示只在内核测试里核实。旧实现的 storage 视图（4,096 项存储清单、forbiddenConclusions）与 view 工具按 ADR-0006 仍是放弃，不补。逐模块对齐八轮到此结束：投递与落地、评审与证据、维护与对账、窗口与宿主、需求包与 Design / Test 技能、宿主中断、pod 与 worktree、观察与 verify；Codex 宿主全程没有真实会话测试。

## 13.130 残留清理：八轮对齐后的全部残留汇总、逐项方案与实现（2026-09-25）

**背景。** 八轮逐模块对齐（§13.122–§13.129）各自在"残留"里留下了没做的事。用户要求"全部汇总并分别进行方案设计和实现"。汇总共 18 项：12 项实现，1 项验证缺口（本轮现场没有补），5 项按宿主边界或既有裁决记录不做（表中写明理由）。实现分两批，之后三个只读审查对照代码报 15 条，全部核实修复。本轮先后用 Fable 5.1 与 Opus 5.5 执行：Fable 5.1 启动的两个多代理工作流在第一批只完成了一个分组（D），另两个分组（H、O）与 D8 / D9 的设计小组都因"agent stalled on all 6 attempts"失败（单次调用里构建锁最多等 15 分钟，加上 auto mode 的安全检查间歇不可用），额度用尽后由 Opus 5.5 接手：H 的代码与测试已在树里，补完文本；O 从头做；D8、D9、O2 重开只读设计小组，锁等待改为单次最多 60 秒。

**汇总与处置。**

| # | 问题 | 来源 | 处置 | 归属 |
|---|---|---|---|---|
| 1 | D8：私有目录的安全 mode 漂移（0750 / 0755 等）一律报 conflict，只能手工修；门与阻塞码只给码不给节点 | §13.124 | 实现：foundation 分类与逐节点 CAS 收敛，workspace 普查私有两棵树；reconcile 的计划就是收敛，不安全节点只报告；verify 的 local-layout 门按区域点名 | 第二批 D8 |
| 2 | D9：改拓扑的 reconfigure 整体被拒，"往运行中的工作区加一个产品仓库"做不到 | §13.124 | 实现"先新增"：允许新增仓库与它在 primary pod 的恰好一个产品窗口；删除与改动仍拒绝 | 第二批 D9 |
| 3 | 回读在 Claude Code 上永远 pending（粘贴块折叠成 "[Pasted text #N +L lines]"） | §13.122 | 助手识别粘贴指示（行数吻合或首行可见）→ readback confirmed | 第一批 H1 |
| 4 | 证据文件里出现 hook 记录 id（裸 UUID）被隐私扫描拦下，Controller 只能删掉重登记 | §13.122 | 技能文本：证据与报告正文不写 hook 记录 id / 会话 id / 裸 UUID | 第一批 D2 |
| 5 | `resume` 在 hook 记录迟到时报 pending，Controller 只能稍后再 relocate | §13.125 | 核实 `--wait` 已存在，技能文本写明用更长的等待 | 第一批 H2 |
| 6 | 被切断的一轮不会自己续上，每次要人再发"继续" | §13.128 | 助手新命令 `nudge`：只在 pane 显示 API Error 且空闲时粘一句续做；技能：投递后沉默且看到 API Error 时用 nudge 而不是二次投递 | 第一批 H3 |
| 7 | `claude --worktree` 从远端默认分支建检出；local-head 靠手工配方 | §13.128 | 助手 `launch --worktree` 自己从本地 HEAD 建检出并写 exclude | 第一批 H4 |
| 8 | pod 8 个窗口逐个等待，拉起十几分钟 | §13.128 | 技能：产品窗口 `--wait 0` 批量起、事后登记 | 第一批 H4 |
| 9 | status 建议的 `git worktree remove` 对加锁检出失败 | §13.128 | 回执的 `locked` 进建议命令：`git worktree unlock … && git worktree remove …` | 第一批 O1 |
| 10 | `unmergedAccepted` 只从活动 Demand 派生，归档后到 pod 关闭之间 status 沉默 | §13.128 | 全作用域另从仍在配置里的 worktree pod 的归档（完成与撤销）Demand 派生，条目带 source: active / archived | 第二批 O2 |
| 11 | status / verify 报 recovery-required 时不点名残留文件、不说 recover 能否处理 | §13.129 | 核心布局检查暴露 residues[]，status.maintenance.residues[{name, kind, recoverable}] | 第一批 O3 |
| 12 | 工具描述里的枚举没有静态门（verify 的 "thirteen" 两轮没人发现） | §13.129 | 新测试：每个工具的 mode / action / operation / intent.kind 枚举值必须出现在描述里；Test 技能的判定词汇与 Schema 逐值核对 | 第一批 D1 |
| 13 | blocked 再决定、continue_demand 重开、证据三种来源、controller-confirmed 审阅只有切片测试没有现场 | §13.123 | 验证缺口，不是缺陷：本批现场没有补（见"现场"），仍由切片测试覆盖 | 未执行 |
| 14 | 登录过期只有用户能 `/login` | §13.128 | 宿主边界，技能已写；不做 | 记录 |
| 15 | 制品信号要 hook 与服务同一制品；`/mcp` 重连未现场核实 | §13.127 | 开发布局的固有约束，正式安装不受影响；不做 | 记录 |
| 16 | Codex 宿主全程没有真实会话测试 | 各轮 | 需要 Codex 会话（用户侧）；不做 | 记录 |
| 17 | 旧实现 storage 视图、view 工具、方案比较等 | ADR-0006 / ADR-0011 | 既有裁决放弃；不做 | 记录 |
| 18 | 投影覆盖提示只在内核测试里核实 | §13.129 | 现场制造读不出的 Demand 有风险，保留为测试覆盖 | 记录 |

**第一批（H、O1、O3、D）。**

- **回读（H1）。** Claude Code 把多行粘贴折叠成 `[Pasted text #N +M lines]`，助手的首行子串检查永远看不到，回读一直 pending（§13.122）。现在 `deliver` 也认这个折叠指示（审查之后只认屏幕上最后一个、且在输入框里的那个）：M 等于 prompt 去首尾空白后的行数减一，或等于原样粘贴的换行数；一行的 prompt 只认不带计数的指示。回读仍只是屏幕摘要，落地只看 hook 记录。回归：计数吻合 confirmed、计数不同 pending、首行可见 confirmed、什么都看不见 pending、一行 prompt 的无计数指示 confirmed。
- **resume 的等待（H2）。** `--wait` 早就在，只是文本没说；窗口启动文本现在写明 `launch` 与 `resume` 都等 `--wait <秒>`（默认 20，上限 120），`hook: pending` 而 pane 活着是记录迟到，保留打印的观察，记录到了再登记或 relocate。
- **续做（H3）。** 被 "API Error: Connection lost mid-response" 切断的一轮不会自己续上（§13.128）。助手新命令 `nudge --window <windowId> [--text <短语>]`：按 `deliver` 的同一套 pane 核对定位，截屏一次，只在 `API Error` 行是输入框上方最后一条对话行（审查之后收紧，见下）、没有工作中标记（`esc to interrupt`、`running … hooks`）且输入框为空时粘一句（默认 `Continue.`）并回车一次；否则报 `not-needed` / `busy`，定位失败报 `pane-missing`，一次调用最多推一次。投递参考写明：落地已观察、回调没来、窗口停着时 nudge 一次，它不是第二次投递；仍沉默就按 status 的下一步。回归覆盖五种状态、`--text 继续`、非法短语与粘贴失败。
- **worktree 由助手准备（H4）。** `basePolicy: local-head` 不再靠 Controller 手工配方（§13.128）：`launch` 在意图带 `local-head` worktree 意图且参数里有 `--worktree <name>` 时，先在仓库主检出 `git worktree add -b worktree-<name> .claude/worktrees/<name> HEAD`（检出已由本仓库登记则复用；审查之后，分支已在而检出不在、路径被别的东西占着、落点不是仓库顶层都拒绝，见下），并保证 `.git/info/exclude` 有 `.claude/worktrees/` 一行；结果多 `worktreePrepared: created | reused | not-requested`。`git worktree add` 失败在开 tmux 窗口之前拒绝。pod 拉起步骤补一句：产品窗口可以 `--wait 0` 一起起，留着观察事后登记。回归用真实临时仓库：新建、复用、未请求、add 失败，以及审查补的分支已存在、路径被占、子目录落点与继承的 `GIT_DIR`。
- **加锁检出的处置（O1）。** 关闭时的建议命令对登记时已加锁的检出改为 `git worktree unlock <path>; git worktree remove <path>`（`;` 而不是 `&&`：锁在登记后已释放时 unlock 失败，remove 仍要做）；两处路径同一个截断结果，整条仍在 512 字符内。两个调用点（pod 关闭结果、status 的 pod 段）都传回执里的 `locked`。
- **点名维护残留（O3）。** §13.129 的 `maintenance{status, protocol}` 只说 recovery-required，不说是哪个文件、recover 能不能处理。核心布局检查现在返回 `transactions` 目录的条目，按命名约定分 intent / journal / unknown（operation ID 必须是 `maintenance_operation_<uuid>`），status 的 `maintenance` 多 `residues[{name, kind, operationId, recoverable}]`（至多 64 条，名字单行化有界）与 `residuesOmitted`。现场 Controller 那次故障注入里的假文件现在会被报成 `kind: unknown, recoverable: false`。
- **工具描述的静态门（D1）。** 新测试对每个公共工具的请求 Schema 抽出主判别枚举（mode、action、operation、decision 与 intent 的 kind），要求每个值在描述里原样出现；另核 640 字节上限、只读工具写 "Reads only"、变更工具不写且逐字点名自己的写效果（审查之后改成逐工具登记短语的闭集）；并把 Test 技能里的判定、失败分类与归属词汇逐值对着报告 Schema（反引号里的小写记号必须是 Schema 的枚举值或步骤号，少数普通命令名走显式放行名单）。这道门第一次跑就挡下三份描述（`wakeflow_continue_demand` 缺 preview、`wakeflow_publish_requirement` 缺 publish 与 recover、`wakeflow_inspect_target_result_review` 缺 "Reads only"），已按切片代码改写。
- **证据里不写 hook 记录 id（D2）。** Controller 的证据参考、Test 的执行参考与 Target 的报告要求各加一句：证据与报告正文不写 hook 记录 id、会话 id 或别的裸 UUID，Wakeflow 对象只用带类型前缀的 id（导入会拒绝，证据捕获会要求确认）。

**第二批（O2、D9、D8）与审查。**

- **归档之后的未合并分支（O2）。** §13.128 发现：pod 的 Demand 归档之后到 pod 关闭之前，`unmergedAccepted` 只从活动 Demand 派生，status 对还等合并或放弃的分支沉默。现在全作用域观察多一个 `archives` 域：候选是看板上已归档（完成）与因撤销而 withdrawn 的需求包对应的 Demand，去掉活动 Demand；按归档时间新到旧，只读 podId 仍在配置里的 worktree pod 的归档（primary pod 的归档不读，也不占扫描预算），逐个打开 `<archiveRef>/payload` 取结果评审快照。归档包里的事件存储是可移植模式（0755 / 0644），读者为此多一种 `archived` 节点政策，活动存储仍按私有政策读。事实层把两边的已接受分支合并，条目带 `source: active | archived`，同一 Demand 又活动时只保留活动那条；读不出的归档计数进 `domains.archives`（`archives:unreadable-<n>`）而不让 status 失败，扫描上限丢下的计入 `truncated.archives`；投影作用域不读归档，投影字节不变。复用了仓储的结果历史审计（改成静态方法 `auditTargetResultHistoryOfCommits`，18 行差异）。控制器参考写明：归档之后到关闭 pod 之前，status 以 `source: archived` 继续列出这些分支。
- **reconfigure 新增产品仓库（D9，§13.124 待裁决的"先新增"）。** 此前任何拓扑改动整体报 `reconfigure-layout-change-unsupported`，"往运行中的工作区加一个产品仓库"做不到。现在 `reconfigureTopologyChange` 逐项比较：存储与支撑面照旧不可改；删除或改动已有仓库、已有窗口各报 `reconfigure-repository-removal-unsupported` / `-change-unsupported` 与 `reconfigure-window-removal-unsupported` / `-change-unsupported`；新增窗口只允许是新仓库在 primary pod 的恰好一个产品窗口，否则 `reconfigure-window-addition-unsupported`。新仓库的根要存在、是 Git 主检出、可读，且与已配置仓库的物理身份不重合（`reconfigure-repository-root-missing` / `-not-git` / `-root-unreadable` / `-root-worktree` / `-root-duplicate`）。每个 pod 恰好一个产品窗口对应每个仓库由配置文法保证，所以有 worktree pod 时新增仓库在解析时就被拒绝——拒绝现在带 `details.configReason: topology` 与指到 pod 条目的路径。apply 仍是一次维护事务：配置、新窗口的运行投影与托管块一起落地，新窗口处于未登记状态，status 给出登记的下一步，verify 全部通过，之后 reconcile 零步骤。控制器参考、`/wakeflow-init` 命令与两份 README 同步。
- **私有模式的安全漂移（D8，§13.124 待裁决）。** 私有目录 0700、私有文件 0600；此前整棵树被 `chmod -R go+rX` 之后，核心布局检查只报 `local-root-node-policy` 之类的 conflict，只能手工修，门也不说是哪个节点。现在分三层：foundation 的 `private-mode-convergence` 按当前用户分类一个节点——当前用户拥有、属主位完整、group / other 不可写、无 setuid / setgid / sticky 的宽模式是"安全漂移"，其余（符号链接、特殊节点、别人拥有、group 或 other 可写、属主位缺失、特殊位）是 unsafe；收敛一次一个节点：路径观察与计划时是同一 inode、同一模式 → `O_NOFOLLOW` 打开 → 句柄仍是那个节点 → `fchmod` → 复验，已经私有即 `current`，句柄 stat 失败也收敛成 `resource-changed`。workspace 的 `wakeflow-private-mode-census` 用有界树扫描（10 万条、深 24）普查 `.wakeflow-local` 与 `.wakeflow-active` 的根与每个条目，给出 `current | safe-drift | unsafe | unavailable`、漂移节点（父先于子）、不安全路径与普查摘要；收敛逐个处理漂移节点，普查之后变了或变得不安全的只计数、跳过，从不强改。维护工具：reconcile 的 preview 先普查，只有安全漂移时计划就是 `WakeflowPrivateModeConvergencePlan`（漂移的目录数、文件数与至多三个区域），摘要是普查摘要，apply 重新普查比对摘要后逐节点收回，不进维护事务（它只收窄当前用户自己节点的权限，可幂等重跑），`next` 指回 reconcile；不安全节点使 preview 以 `private-mode-unsafe:<区域>` 阻塞，整个收敛都不做；fresh-initialize 与 reconfigure 在安全漂移时以 `private-mode-drift` 阻塞。verify 的 local-layout 门先普查，按数量与区域点名：`private-mode-drift-27:.wakeflow-active,.wakeflow-local`（区域是至多三段路径的最小覆盖，至多三个，门码字符集外的字符换成 `-`）。回归：foundation 的分类与收敛（同一 inode、换了 inode、换了模式、0777、符号链接不跟随）；普查的 current / safe-drift / unsafe / 缺树跳过 / 普查后变化计数；能力层的 `chmod -R go+rX` 全流程（verify 点名 → reconfigure 拒绝 → preview 收敛计划 → 中途再漂移时旧摘要被拒 → apply 收回 27 个节点 → reconcile 回到零步骤、门通过）与 0777 目录（只报告、不收敛，用户收回后其余漂移照常收敛）。
- **对抗审查与修复。** 三个只读审查（O2、D9、第一批）共报 15 条（3 条 major），逐条对照代码核实后全部修复，没有驳回的：
  - 续做与回读（P1-1、P1-4）：`nudge` 只在 API Error 行是输入框上方最后一条对话行时才粘贴——之后有回复、已渲染的用户消息（包括上次推过的 "Continue."）或任何输出都是 `not-needed`，所以连推两次、正常结束后的旧错误行、回复里提到 "API Error" 都不再误推；回读只认屏幕上最后一个 `[Pasted text …]` 指示且它在输入框里、行数吻合，转录区里的旧指示不算。
  - worktree 准备（P1-2、P1-3、P1-5）：分支 `worktree-<name>` 已在而检出不在时不再复用旧分支（它未必在本地 HEAD 上），以 `worktree-branch-exists` 拒绝、零写，由 Controller 问用户；检出路径已在时只有本仓库 `git worktree list --porcelain` 登记的那条才算 `reused`，否则 `worktree-path-occupied`；任何写入之前核对 `git rev-parse --show-toplevel` 就是落点（否则 `worktree-root-not-toplevel`，不会落到上层仓库），git 调用去掉继承的 `GIT_DIR` 等七个环境变量；检出建好之后开窗或等待失败，失败结果仍带 `worktreePrepared` 与分支，Controller 知道留下了一个检出。
  - 归档观察（O2-1…O2-4）：撤销的 Demand 同样封存归档，候选另取 withdrawn 且原因是 `demand-cancelled:<demandId>` 的需求包，以撤销时间排序；清单读取至多 1024 个（新到旧），先按 pod 过滤，再读至多 64 个评审快照，primary 的归档不再占快照预算，两道上限丢下的都计入 `truncated.archives`；同一 Demand 又活动时事实层直接去掉归档那条，不依赖看板状态；补了 status 端到端（有 worktree pod、撤销后 `source: archived` 过 Schema）、归档损坏（status 成功、`archives:unreadable-1`）与新到旧上限的测试。
  - reconfigure（D9-1…D9-4）：新增仓库必须恰好带一个新的 primary pod 产品窗口；根的 `.git` 是文件（链接 worktree）报 `reconfigure-repository-root-worktree`，根与已配置仓库的物理身份相等、包含或被包含（含符号链接别名）报 `reconfigure-repository-root-duplicate`；有 worktree pod 时的文法拒绝带 `details.configReason: topology` 与指到 pod 条目的路径；补了 managed-block 仓库在 Codex（AGENTS.md）与 Claude Code（CLAUDE.md）两个宿主的托管块落地、已有窗口投影仍 current 的测试。
  - 其余：维护进行中（busy）的事务条目不再标可恢复（P1-7）；工具描述测试里"写工具说出它写什么"的词干匹配分不清名词与否定，改为逐工具登记写效果短语的闭集（P1-6）。

**门。** `npm test` 全绿：typecheck、architecture（717 个模块）、Biome lint（724 个文件，四条既有 warning 不是错误）与 format、knip、TypeScript 测试 1058/1058（含二十个端到端场景）、Schema drift（`schema:build` 之后）、`build:check`（`build:artifacts:committed` 之后字节一致）；`npm run smoke:artifacts` 通过（20 个工具、fresh completed、reconcile no-op、status idle、verify ok、pod create preview ready、hook landed）；`git diff --check` 干净（未跟踪的新文件另查了行尾空白与末尾空行）。第一批完成时另跑过一次全门（1041/1041 加 smoke）。新增测试文件：工具描述门、私有模式的 foundation / 普查 / 能力层三份、reconfigure 新增仓库、维护进行中的残留；既有文件里补了助手（折叠粘贴、nudge、worktree 准备的七种结局）、处置建议、维护残留、归档观察的用例。

**现场。** 仍是 WakeflowTestWorkspace 的八个 main 窗口，经一份新制品副本（manifest `d8e0f584…`）从外部驱动（`live-130.mjs`）。**基线：** status idle，`maintenance{protocol: idle, residues: []}`，`domains.archives` observed（配置里没有 worktree pod，没有要扫的归档），`unmergedAccepted` 空；verify 13/15，失败的是 `host-settings-assets`（部署的助手是旧的）与 `runtime-artifact`（八个窗口 stale），都是换制品的预期。**O3：** 放一个假 intent 文件，status 列出 `{kind: unknown, recoverable: false}`，local-layout 报 `maintenance-protocol-recovery-required`；删掉后回到 idle。**D8：** 对两棵私有树 `chmod -R go+rX`（35 个目录成 0755、392 个文件成 0644），verify 的 local-layout 报 `private-mode-drift-427:.wakeflow-active,.wakeflow-local`（hook 通道、设置资产、活动投影、窗口身份随之失败或 unavailable），reconfigure preview 以 `private-mode-drift` 阻塞；reconcile 第一次 preview 是收敛计划（35 个目录、392 个文件、两个区域），apply 收回 427 个节点（`operationId: null`，`changed: 0`，`next` 指回维护），第二次 reconcile 是一个普通维护步骤（部署新助手）；之后全部回到 0700 / 0600，verify 14/15 只剩窗口 stale。**刷新：** 八个窗口换到新制品——五个有过对话的 resume + relocate（Controller %121 仍是会话 `80580357…`、Design %122、Test %123、Alembic %124、AlembicPlugin %131），三个从没对话的 launch + replace（AlembicAgent %126、AlembicCore %128、AlembicDashboard %130），都 mark；status idle、每个窗口 `artifact: current`，verify 15/15，看板不变（archived 6 / withdrawn 3）。**H3：** 对 Controller、Design、AlembicPlugin 三个空闲窗口跑部署后的 `nudge`，都是 `not-needed`、`observedError: null`，没有粘贴。**未执行（未验证）：** `nudge` 的 `nudged` 路径（真实的 API Error 无法按需制造）；H1 折叠粘贴的回读（本轮没有跑 Demand 投递）；H4 worktree 自备、`worktree-branch-exists` / `-path-occupied` / `-root-not-toplevel` 拒绝、O1 加锁检出的处置建议与 O2 归档后仍列出的未合并分支（都要一个真实 pod，本轮没有开）；D9 现场新增仓库（删除被设计拒绝，现场加一个仓库会给测试工作区永久多出第六个产品窗口，所以只由两个宿主的能力测试覆盖）；清单第 13 项的 continue_demand 重开等。

**残留。** 上面"未执行"的现场项仍只由测试覆盖，下一次开真实 pod 或跑 Demand 时顺带核实。reconfigure 与 fresh-initialize 因 `private-mode-drift` 阻塞时 `next.suggestedTool` 是 null，要靠 Controller 参考里"先 reconcile"一句——让 `next` 直接指向 reconcile 更紧，留作小改进。私有模式普查只覆盖 `.wakeflow-local` 与 `.wakeflow-active` 两棵树；ledger 与支撑面按设计是可移植模式，不在其中。清单第 14–18 项维持记录不做：登录过期只有用户能 `/login`、开发布局下的 `/mcp` 重连、Codex 没有真实会话、ADR-0006 / ADR-0011 放弃的旧功能、投影覆盖提示只在内核测试里。本轮起先由 Fable 5.1 的多代理工作流执行，它们因模型响应慢被看门狗反复重启、额度用尽后由 Opus 5.5 接手；之后两次多代理工作流（D8 普查加三个审查、三个修复）各用 Opus 5.5 的 agent，按文件分工、构建锁每次最多等 60 秒，都完成，只有零星一两次重启。

## 13.131 整体审查：逐文件 review 全部手写文件、核实后修复、代码逻辑梳理与细节优化（2026-09-25）

**背景与范围。** §13.130 提交之后，用户要求"使用 ultra 模式逐个 review 当前 wakeflow 项目的每一个文件，分析发现问题并修复，进行代码逻辑梳理优化和细节优化"。范围是全部手写文件：`src/`（生成的 `src/contracts/generated/` 除外，由 `schema:build` 从 Schema 派生）、`tests/`、`tooling/` 与 `assets/`（技能、命令、README 的单一来源），共 743 个文件、约 23.5 万行；`plugins/` 是生成物，`docs/` 只随代码改动同步。

**方法。** 按模块把文件分成 85 桶（每桶约 4000 行，源码与其测试同桶），每桶一个只读审查者逐文件读完，按正确性、隐私与确定性、合同一致性、简化、注释与细节、测试质量六类报问题，每条给出文件行、依据与最小修法；有报告的桶再交一个独立的怀疑者对照代码逐条核实（confirmed / revised / rejected，拿不准即驳回）。两者都是 Opus 5.5，读写分离，审查阶段零写零构建。修复阶段按"共享文件即同组"把发现聚成互不相交的文件组：18 个并行修复者各自只改自己的文件，Demand / 投递 / 评审核心的 60 条共享 41 个文件，由一条四步顺序链处理；每个修复者只跑自己的聚焦测试（构建锁每次最多等 60 秒），需要别人文件的改动交回主会话。

**数字。** 85 桶全部完成，读完 746 个文件；报 510 条，核实后保留 387 条（major 28、minor 144、nit 215；confirmed 291、revised 96），驳回 123 条。按类：测试质量 85、简化 74、注释 63、正确性 50、合同 48、健壮性 31、一致性 29、隐私 5、确定性 2。第一轮修复 384 条、延后 3 条；修复者交回 71 条跨文件事项、77 条取舍说明。

**第一轮修复（提交 `2457c961`）。** 行为修正：
- 投递：rearm 之后目标的导入认同一 deliveryId 任一已记录结果上的 claimDigest（此前 rearm 后的 prompt 仍带第 1 代声明摘要，导入必然被拒）；三次 rearm 用尽之后两种目标都能重新 prepare，实现目标保留被拒信封的返工 / 补救依据（此前重新 prepare 会静默丢掉），测试目标重发同一次尝试（此前是死路）；`record_delivery_outcome` 与评审决定的幂等重放在目标往下走之后仍然成立（结果里的 phase 由这次结果派生，不再读当前聚合）。
- Demand：continue 核对归档 Demand 的 pod 仍在且开着（`pod-unknown:<podId>` / `pod-closing:<podId>`）；research Demand 不能 continue（`demand-type:research`，此前会被路由到永远规划不了的实现阶段）；apply 发现 pod 被占时与 preview 一样报 `pod-busy` 并带占用的 demandId。
- 观察：verify 在 `server-outdated` / `windows-stale` 时的 next 用 status 的同一词汇（`runtime-artifact-outdated` / `window-artifact-stale`），不再指向维护；关闭待完成的 pod 不再给 `done`。
- Foundation：rename / link / unlink 在提交之后发现父目录漂移报 `commit-uncertain` 而不是提交前的 `parent-changed`；git 忽略观察接受只命中否定模式的批次。
- 内核：字节上限失败的 reason 改为 kebab-case（此前 WakeflowError 构造直接抛 TypeError）；脱敏在对象键里发现私有值时不再把它回显到错误路径（测试原本把这个泄漏固定了下来）。
- 证据：同一行两个隐私发现不再让确认的捕获以 `manifest` 失败；清单容量按实际写出的渲染字节计。
- Claude 宿主：用户的 `.claude/settings.json` 不是 0644、是链接或超过 1 MiB 时报 `settings-blocked:<kind>:<root>:source-policy` 阻塞，而不是整次维护 preview 以 unexpected 失败；助手 `close` 在 pane 不再带 Wakeflow 标记时什么都不杀、报 `closeResult: unknown`。
- 技能文本：Claude 专有的助手句子（resume、已落地守卫）移进两个新占位符 `windowResume`、`resendGuard`，Codex 制品里此前拼出的断句消失；bug 与 supplement 需求包也教写验收标准；Codex 的 worktree 文本与运行时的执行说明一致。CLAUDE.md 的占位符数改为九个。
- Schema：`planDigest` 在 apply 必填、preview 禁止（运行时早已如此）；一批约束与产出代码对齐；删去没被引用的 `$defs`（wire Schema 自包含测试改为只比两边都携带的共享定义）。
其余是简化（重复逻辑、死分支、过度嵌套）、陈旧或错误注释、更清楚的错误映射（中止一律 `io-failure/aborted`、私有文件损坏报 `invalid-request` 而不是 unexpected）与测试加强（断言与标题一致、补分支、零写快照、中断恢复）。主会话补了三处跨文件的测试期望与一处重复的事件 id 助手。门：1093/1093、smoke 通过。

**第二、三轮（跨文件事项与延后项）。** 第一轮的修复者没有所有权去写的回归测试由 13 个并行任务补齐：定位器损坏的三种形状、证据提交之后才中止、宿主设置只含空权限时是 drift、相对 gitdir 的 worktree 不是 prunable、单行化的回调摘要、超过 8192 字符的测试步骤、Unicode 章节锚点、维护日志拒绝空的 desired 摘要、不安全的门缺 maintenance/ 时是 conflict、预备恢复拒绝执行中的日志、投递信封的 prompt 摘要篡改、Demand 权威引用不符、活动守卫中止、pod 被占的 `pod-busy`、归档快照的非流错误、流索引的 byType 与提交序号、工作声明的 foreign 释放与 pod 回执的杂项条目、continue 的 pod 与 research 阻塞。顺带收紧：`wakeflow-target-result-import-result` 的 bindingId 用与其他 Schema 相同的模式；五处 8192 字符上限按码位计（与 JSON Schema 的 maxLength 同一口径）；投递在取得工作声明之后重读绑定，摘要变了就以 `binding-changed` 拒绝并释放声明；投递的 `next` 与证据一样映射结果评审快照的错误。补测试时又挖出两个真实缺陷：规划以 Unicode 章节锚点拒绝时，阻塞项进公开 details 触发 TypeError、以 unexpected 逃出——三个服务各有一份一样的 `rejectWith`（投递那份还没有 8 项上限），合并为内核的 `failWithBlockers`，非标识阻塞项只留阻塞码；不以单个 LF 结尾的定位器文件以 foundation 的 StrictTextFileError 逃出，改为 `invalid-request/locator-record`。
**宿主差异回到宿主 profile。** CLAUDE.md 要求共享代码不以 hostId 推断宿主行为。工作区宿主资源 profile 多一个 `tmuxAsset` 面（Claude 声明 `tmux.mjs`，目录与矩阵从此知道助手资产）与一个顶层 `launch` 模板（`tmux-session{controllerEffort, defaultEffort, permissionMode, sessionName}` | `host-thread`）；端点的执行说明从模板取 Claude 的启动默认值，不再按 hostId 分支；输出逐字节不变，Claude 的矩阵摘要随新声明变化（被打断的旧维护日志会走既有的陈旧日志路径）。
**续接的 real-environment Demand。** 审查的一条 major：real-environment Demand 完成后 continue，上一轮的 test 目标仍算作未终结 / 当前代际，续接后永远规划不了新任务包。第二轮先在归约器里算出"续接时已有的 test 目标"写进状态，独立核实指出两个问题：完成前的路由（`demand-post-acceptance-route`）仍数着旧目标，续接的 Demand 依然走不完；归约器多产出一个字段，已持久化的续接流重放后状态摘要与存下的 `resultingStateDigest` 不再一致、会读不出来。第三轮改成把边界记进事件：continue 的决定按续接前的状态算出 `historicalTestTargetIds`，写进 `lifecycle.demand-continued` 事件（v1 事件数据上的可选字段，空时省略——仓库里 Demand 事件家族都还是 v1，v1 数据此前也就地放宽过；加 v2 与升版器得到的仍是"字段缺席"的同一形状），归约器只从事件取、并核对它恰好等于续接前的 test 目标集合；旧形状的事件重放出与从前逐字节相同的状态与摘要。路由、规划阻塞（`tasking/decide.ts` 直接用 `currentTestTargetsOf`，删掉服务里复制的代际视图）与投影进度只数当前代际。场景 `card-04/complete-and-continue` 现在把 real-environment 路径走完两遍：完成 → continue → 规划实现 → 投递 → 验收（next 为测试规划）→ 规划测试 → 投递 → 导入 → 验收 → 完成，再 continue、取消、取消后 continue 被拒。已知限制：本次之前写下的续接事件没有边界，那样续接的 Demand 仍把旧 test 目标算作当前。
**宿主检查收尾与 Unicode 锚点。** 余下两处共享代码里的 hostId 分支改为读宿主 profile：投递处置"发送返回即落地"由 `sendReturnProvesLanding(profile)`（`launch.kind === "host-thread"`）给出，处置输入不再带 hostId；worktree 处置的备选句按 `surfaces.worktree.launch` 取，字符串逐字节不变。Claude profile 的 `tmux.mjs` 与资产模块的常量由测试钉住。补了接受半边的测试：需求包带自定义 `## 性能约束` 章节时，引用该锚点的规划被接受并记录。两次独立核实都是 ok（只有 minor）。

**门。** `npm test` 全绿：typecheck、architecture（726 个模块）、Biome lint（733 个文件，四条既有 warning 不是错误）与 format、knip、TypeScript 测试 1121/1121（含二十个端到端场景）、Schema drift、`build:check`（`build:artifacts:committed` 之后字节一致）；`npm run smoke:artifacts` 通过（20 个工具、fresh completed、reconcile no-op、status idle、verify ok、pod create preview ready、hook landed）；`git diff --check` 干净。第一轮检查点提交时是 1093/1093 加 smoke。

**残留与待裁决。** **待裁决：** bug 与 supplement 需求包没有验收标准时，发布照样通过、规划时才走不下去；技能文本已教写验收标准，要不要让发布的 preview 直接要求（改 ADR-0011 D3 的必需章节表）留给用户。**已知限制：** 本次之前写下的 real-environment 续接事件没有历史边界（实际没有这样的流：continue 从未在现场用过）。**记录不做：** worktree 处置的两句备选仍是共享代码里按 `surfaces.worktree.launch` 取的表（完全移进宿主要给 profile 加字段）；端点服务按 `model.hosts["claude-code"]` 读配置段（配置形状按宿主分键，不是行为分支）；配置根放置的物理重叠分支在真实文件系统上走不到、没有测试；rename / link / unlink 提交后漂移的 `commit-uncertain` 映射与 `releaseWorkClaimIfHeld` 的并发释放需要故障注入缝，没有为测试给生产代码加钩子；一处"合并三对助手函数"的纯整理跳过；四条既有 Biome warning 未动；新测试文件没有进 `test-durations.json`（可选的排序提示）。123 条被怀疑者驳回的报告不处理。**现场：** 本轮没有现场复验；运行时与技能都换了，测试工作区的窗口与插件缓存仍在 §13.130 的制品上，下次驱动之前要刷新（缓存、resume / relaunch 窗口）。
