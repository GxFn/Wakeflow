...
import { readArchivedDemandResultReviewSnapshot, } from "../review/demand-result-review-snapshot.js";
...
const NOTHING_TO_SCAN = Object.freeze({
...
function newestFirst(left, right) {
    if (left.archivedAt !== right.archivedAt)
        return left.archivedAt < right.archivedAt ? 1 : -1;
    return left.demandId < right.demandId ? -1 : left.demandId > right.demandId ? 1 : 0;
}
async function readArchivedReviewSnapshot(ledgerRoot, archiveRef, signal) {
    const payload = await RootedDirectory.open(path.join(ledgerRoot.absolutePath, ...`${archiveRef}/payload`.split("/")), "$archive/payload", { durability: ledgerRoot.durability });
    try {
        return await readArchivedDemandResultReviewSnapshot(payload, signal === undefined ? {} : { signal });
    }
    finally {
        await payload.close();
    }
...
function countUnreadable(error) {
 ...
 */
export async function observeArchivedDemands(ledgerRoot, candidates, worktreePodIds, signal, limits = DEFAULT_SCAN_LIMITS) {
    if (worktreePodIds.size === 0)
    ...
    for (const candidate of manifestScan) {
        try {
            ...
            scanned += 1;
            const reviewSnapshot = await readArchivedReviewSnapshot(ledgerRoot, located.archiveRef, signal);
            demands.push(Object.freeze({ demandId: candidate.demandId, podId: located.podId, reviewSnapshot }));
        }
        catch (error) {
