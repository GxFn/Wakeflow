...
- **Multiple active demands, one controller**: up to `maxActiveDemands` (default 2) demands run side by side; parallelism exists ONLY at the demand level — never inside a demand. One controller is the single acceptance authority across all of them; each turn declares its demand and re-reads that state root; the later demand touching an occupied repo works through an isolation window; claiming past capacity fails closed.
