...
import { managedEvidenceRecordRootRef, MANAGED_EVIDENCE_PUBLICATION_TRANSACTION_REF, MANAGED_EVIDENCE_ROOT_REF, } from "./managed-evidence-resource-paths.js";
/**
 ...
 */
export const MANAGED_EVIDENCE_RESOURCE_OWNER_ID = "demand-managed-evidence";
...
function demandChildRef(demand, localRef) {
    return parsePortableResourcePath(`${demandFinalRootRef(demand)}/${localRef}`);
}
function commonDeclaration(declarationId, relativePath, nodePolicy, processing) {
...
export function createManagedEvidenceRootResourceDeclaration(demandValue) {
    const demand = demandId(demandValue);
    return commonDeclaration(`demand.managed-evidence.${demand}.root`, demandChildRef(demand, MANAGED_EVIDENCE_ROOT_REF), {
        kind: "directory",
        mode: "0700",
...
export function createManagedEvidenceRecordResourceDeclaration(demandValue, evidenceValue) {
    const demand = demandId(demandValue);
    const evidence = evidenceId(evidenceValue);
    return commonDeclaration(`demand.managed-evidence.${demand}.record.${evidence}`, demandChildRef(demand, managedEvidenceRecordRootRef(evidence)), {
        kind: "tree",
        rootMode: "0700",
        symlinkPolicy: "reject",
...
export function createManagedEvidencePublicationTransactionResourceDeclaration(demandValue) {
    const demand = demandId(demandValue);
    return commonDeclaration(`demand.managed-evidence.${demand}.transaction`, demandChildRef(demand, MANAGED_EVIDENCE_PUBLICATION_TRANSACTION_REF), {
        kind: "file",
        mode: "0600",
...
export function createManagedEvidenceResourceCatalog(demandValue, evidenceValue) {
    const demand = demandId(demandValue);
    const evidence = evidenceId(evidenceValue);
