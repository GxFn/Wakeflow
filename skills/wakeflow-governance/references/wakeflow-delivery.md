...
Use `node scripts/wakeflow-delivery.mjs` from the Wakeflow runtime root.
...
| Prepare state-root dispatch mechanically | `prepare-dispatch-from-state --state-root <path> --target-task-id <task> --group <group> --controller-window <controller> --human-context-ref <path> --require-thread --write --json` | Default route for eligible new work. Reads `wakeflow-state.json` and task-package JSON, then writes window config, dispatch packet / group, and delivery envelope with `stateRef` / `humanContextRef`. It does not read a Markdown plan as authority and stops before host thread send/readback. It fails closed for terminal / paused / blocked / review-ready demands and accepted / completed / blocked target tasks. |
...
| Record target result | `submit-result ... --write --json` | Writes a `TargetResultEnvelope`; it is not an acceptance verdict. When `--group` is provided, the local transport result file is scoped by dispatch group so concurrent controller runs cannot overwrite each other by reusing a target window and task id. |
...
state root or to the Wakeflow runtime root. Missing path refs still gate the
