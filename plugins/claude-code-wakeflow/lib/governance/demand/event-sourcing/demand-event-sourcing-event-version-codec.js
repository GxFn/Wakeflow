...
import { EventSourcingVersionEvolutionRegistry, EventSourcingVersionEvolutionError, } from "../../../foundation/event-sourcing/event-sourcing-version-evolution.js";
import { createRuntimeJsonSchemaValidator } from "../../../foundation/schema/runtime-json-schema.js";
import { parseDemandUncommittedEvent, } from "./demand-event-sourcing-event.js";
...
const validatePublishedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DEMAND_PUBLISHED_EVENT_DATA_V1_SCHEMA, [WAKEFLOW_SHA256_DIGEST_SCHEMA]);
const validateManagedEvidenceRecordedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_MANAGED_EVIDENCE_RECORDED_EVENT_DATA_V1_SCHEMA, [
...
const validateCancelledV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DEMAND_CANCELLED_EVENT_DATA_V1_SCHEMA);
const validateCompletedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DEMAND_COMPLETED_EVENT_DATA_V1_SCHEMA, [
...
const validateEscalatedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DEMAND_ESCALATED_EVENT_DATA_V1_SCHEMA, LIFECYCLE_REFERENCES);
const validateDecisionRecordedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DECISION_RECORDED_EVENT_DATA_V1_SCHEMA, LIFECYCLE_REFERENCES);
const validateContinuedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DEMAND_CONTINUED_EVENT_DATA_V1_SCHEMA, LIFECYCLE_REFERENCES);
const validateTargetTaskPlannedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_TARGET_TASK_PLANNED_EVENT_DATA_V1_SCHEMA, [
...
const validateDeliveryPreparedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DELIVERY_PREPARED_EVENT_DATA_V1_SCHEMA, [
...
const validateDeliveryOutcomeRecordedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DELIVERY_OUTCOME_RECORDED_EVENT_DATA_V1_SCHEMA, [
    WAKEFLOW_DELIVERY_ENVELOPE_SCHEMA,
    WAKEFLOW_DELIVERY_OUTCOME_SCHEMA,
    WAKEFLOW_DELIVERY_REARM_SCHEMA,
    WAKEFLOW_TASK_PACKAGE_SCHEMA,
...
const validateDeliveryRearmedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_DELIVERY_REARMED_EVENT_DATA_V1_SCHEMA, [
    WAKEFLOW_DELIVERY_ENVELOPE_SCHEMA,
    WAKEFLOW_DELIVERY_OUTCOME_SCHEMA,
    WAKEFLOW_DELIVERY_REARM_SCHEMA,
    WAKEFLOW_TASK_PACKAGE_SCHEMA,
...
const validateTargetResultRecordedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_TARGET_RESULT_RECORDED_EVENT_DATA_V1_SCHEMA, [
...
const validateControllerTargetReviewDecidedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_CONTROLLER_TARGET_REVIEW_DECIDED_EVENT_DATA_V1_SCHEMA, [
...
const validateCallbackReissuedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_CALLBACK_REISSUED_EVENT_DATA_V1_SCHEMA, [WAKEFLOW_SHA256_DIGEST_SCHEMA, WAKEFLOW_UTC_INSTANT_SCHEMA]);
const validateProductDefectRemediationAuthorizedV1 = createRuntimeJsonSchemaValidator(WAKEFLOW_PRODUCT_DEFECT_REMEDIATION_AUTHORIZED_EVENT_DATA_V1_SCHEMA, [
    WAKEFLOW_CONTROLLER_PRODUCT_DEFECT_REMEDIATION_AUTHORIZATION_SCHEMA,
    WAKEFLOW_SHA256_DIGEST_SCHEMA,
    WAKEFLOW_UTC_INSTANT_SCHEMA,
]);
function parsePublishedV1(value) {
    const result = validatePublishedV1(value);
    if (!result.ok)
        throw new TypeError("Demand published v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseManagedEvidenceRecordedV1(value) {
...
function parseCancelledV1(value) {
    const result = validateCancelledV1(value);
    if (!result.ok)
        throw new TypeError("Demand cancelled v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseCompletedV1(value) {
    const result = validateCompletedV1(value);
    if (!result.ok)
        throw new TypeError("Demand completed v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseEscalatedV1(value) {
    const result = validateEscalatedV1(value);
    if (!result.ok)
        throw new TypeError("Demand escalated v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseDecisionRecordedV1(value) {
    const result = validateDecisionRecordedV1(value);
    if (!result.ok)
        throw new TypeError("Decision recorded v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseContinuedV1(value) {
    const result = validateContinuedV1(value);
    if (!result.ok)
        throw new TypeError("Demand continued v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseTargetTaskPlannedV1(value) {
    const result = validateTargetTaskPlannedV1(value);
    if (!result.ok)
        throw new TypeError("Target task planned v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseDeliveryPreparedV1(value) {
    const result = validateDeliveryPreparedV1(value);
    if (!result.ok)
        throw new TypeError("Delivery prepared v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseDeliveryOutcomeRecordedV1(value) {
...
function parseDeliveryRearmedV1(value) {
    const result = validateDeliveryRearmedV1(value);
    if (!result.ok)
        throw new TypeError("Delivery rearmed v1 data is invalid.");
    return parseJsonValue(result.value, "$data");
}
function parseTargetResultRecordedV1(value) {
...
function parseControllerTargetReviewDecidedV1(value) {
...
function parseCallbackReissuedV1(value) {
...
function parseProductDefectRemediationAuthorizedV1(value) {
    const result = validateProductDefectRemediationAuthorizedV1(value);
    if (!result.ok) {
        throw new TypeError("Product defect remediation authorized v1 data is invalid.");
    }
    return parseJsonValue(result.value, "$data");
}
const PUBLISHED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["publication.demand-published"],
    codecs: [{ version: 1, parse: parsePublishedV1 }],
    steps: [],
});
const MANAGED_EVIDENCE_RECORDED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["evidence.managed-evidence-recorded"],
    codecs: [{ version: 1, parse: parseManagedEvidenceRecordedV1 }],
    steps: [],
});
const CANCELLED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["lifecycle.demand-cancelled"],
    codecs: [{ version: 1, parse: parseCancelledV1 }],
    steps: [],
});
const COMPLETED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["lifecycle.demand-completed"],
    codecs: [{ version: 1, parse: parseCompletedV1 }],
    steps: [],
});
const ESCALATED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["lifecycle.demand-escalated"],
    codecs: [{ version: 1, parse: parseEscalatedV1 }],
    steps: [],
});
const DECISION_RECORDED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["lifecycle.decision-recorded"],
    codecs: [{ version: 1, parse: parseDecisionRecordedV1 }],
    steps: [],
});
const CONTINUED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["lifecycle.demand-continued"],
    codecs: [{ version: 1, parse: parseContinuedV1 }],
    steps: [],
});
const TARGET_TASK_PLANNED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["tasking.target-task-planned"],
...
const DELIVERY_PREPARED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["delivery.delivery-prepared"],
    codecs: [{ version: 1, parse: parseDeliveryPreparedV1 }],
    steps: [],
});
const DELIVERY_OUTCOME_RECORDED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["delivery.delivery-outcome-recorded"],
...
const DELIVERY_REARMED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["delivery.delivery-rearmed"],
    codecs: [{ version: 1, parse: parseDeliveryRearmedV1 }],
    steps: [],
});
const TARGET_RESULT_RECORDED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["result.target-result-recorded"],
    codecs: [{ version: 1, parse: parseTargetResultRecordedV1 }],
    steps: [],
});
const CONTROLLER_TARGET_REVIEW_DECIDED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["review.target-result-decided"],
...
const CALLBACK_REISSUED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["result.callback-reissued"],
    codecs: [{ version: 1, parse: parseCallbackReissuedV1 }],
    steps: [],
});
const PRODUCT_DEFECT_REMEDIATION_AUTHORIZED_REGISTRY = new EventSourcingVersionEvolutionRegistry({
    currentVersion: DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS["review.product-defect-remediation-authorized"],
    codecs: [
        {
            version: 1,
...
const EVENT_VERSION_REGISTRIES = Object.freeze({
    ...
    "review.product-defect-remediation-authorized": PRODUCT_DEFECT_REMEDIATION_AUTHORIZED_REGISTRY,
...
export const DEMAND_EVENT_SOURCING_SUPPORTED_EVENT_VERSIONS = Object.freeze({
    "delivery.delivery-outcome-recorded": DELIVERY_OUTCOME_RECORDED_REGISTRY.supportedVersions,
    ...
    "evidence.managed-evidence-recorded": MANAGED_EVIDENCE_RECORDED_REGISTRY.supportedVersions,
    ...
    "result.target-result-recorded": TARGET_RESULT_RECORDED_REGISTRY.supportedVersions,
    "review.product-defect-remediation-authorized": PRODUCT_DEFECT_REMEDIATION_AUTHORIZED_REGISTRY.supportedVersions,
    "review.target-result-decided": CONTROLLER_TARGET_REVIEW_DECIDED_REGISTRY.supportedVersions,
    "tasking.target-task-planned": TARGET_TASK_PLANNED_REGISTRY.supportedVersions,
});
function isDemandEventSourcingCurrentEventType(value) {
...
function parseEventDataObject(value, path) {
...
export function decodeDemandEventSourcingPersistedEvent(envelope) {
    if (!isDemandEventSourcingCurrentEventType(envelope.eventType)) {
        throw new EventSourcingVersionEvolutionError("unsupported-version", "$/eventType");
    }
...
export function encodeCurrentDemandEventVersion(value) {
    const event = parseDemandUncommittedEvent(value);
    const registry = EVENT_VERSION_REGISTRIES[event.eventType];
    const encoded = registry.evolve(DEMAND_EVENT_SOURCING_CURRENT_EVENT_VERSIONS[event.eventType], event.data);
