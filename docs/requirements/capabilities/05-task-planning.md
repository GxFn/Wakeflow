...
**现 TS 状态**：`plan_test_card`、`prepare_test_delivery`、`record_controller_test_review_decision`、`authorize_product_defect_remediation` 已公开；测试卡、测试任务、测试投递、dispatch 投影为 23 个文件；缺陷返工授权后可产生产品返工与新的 Test 代际，即旧缺口已补；isolated 的测试规划有 blocker。
