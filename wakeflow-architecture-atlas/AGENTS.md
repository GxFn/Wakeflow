...
- 证据表的测试列与代码列同等对待：要么写 `tests/…#symbol` 锚点（符号必须真的被该测试用到），
  要么写 `间接覆盖：…` 或 `未覆盖：…` 并给出理由。只写测试文件名不算覆盖；文档在 frontmatter
  声明 `testEvidence: anchored` 后由 `npm run check:structure` 强制执行。
