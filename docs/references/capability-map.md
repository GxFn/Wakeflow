...
| 4 | `wakeflow_register_window` | 2 | `wakeflow_register_window_binding` | 重切 | 已实现；窗口在配置里带 `podId`，worktree pod 的产品窗口握手带 worktree 回执（2026-09-10 pod 切片 9，gate-log §13.92）；§13.125：`relocate` 保留绑定、换定位器代际（Claude 助手 `resume` 之后），`launch` / `resume` 拒绝活着的 pane，等不到 hook 且新 pane 已死或消失时报 `resume-exited` / `launch-exited` |
