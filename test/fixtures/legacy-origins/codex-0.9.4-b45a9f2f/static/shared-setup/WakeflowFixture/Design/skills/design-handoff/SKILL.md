...
Every delivery must carry one complete proportional `demandAuthority` whose role-labeled
references match the selected demand type. `wakeflow_deliver` validates this contract before
appending the row; incomplete Design work stays upstream.

Set `Auto Claim` only after readiness is established:

- **`autoClaim: true`** — authorizes the controller to claim the already-ready item
  unattended. It does not make the item ready and does not weaken any authority role.
- **`autoClaim: false`** (default) — keeps the same readiness contract but requires a
  controller/user confirmation before claiming.

Still clarifying, comparing options, or designing means do not deliver yet. Deliver one item;
never edit or re-status a delivered row.
