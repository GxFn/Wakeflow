...
export function deriveDurableId<Kind extends WakeflowDurableIdKind>(
  kind: Kind,
  namespace: string,
  ...
  return createWakeflowDurableId(
    kind,
    parseUuidV4(deterministicUuidV4(`${kind}:${namespace}`, ...parts), "$uuid"),
  );
}

/** 追加命令的 commitId 由 Demand 与客户端幂等键派生，重试自然命中同一提交。 */
export function deriveDemandCommitId(
  demandId: string,
