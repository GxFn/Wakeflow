...
test("sha256-hasher depends only on crypto, ByteCount, and SHA-256 lexicals", () => {
  const source = readFileSync(
    path.join(process.cwd(), "src/foundation/crypto/sha256-hasher.ts"),
    "utf8",
  );
  ...
  equal(source.includes("TextEncoder"), false);
