...
| `.workspace-local/` | ignored local config and real thread ids (Claude Code session ids) |
...
when policy allows, send with the Claude Code host transport (the session
message tool in desktop-session mode, or a background
`claude -p --resume <sessionId> "<prompt>" --output-format json` run in
headless-resume mode), and record delivery evidence. Internal runtime scripts
remain available to Wakeflow skills and tests without becoming public MCP tools.
