import { buildWakeflowConfigIndexes, parseWakeflowConfig, WakeflowConfigError, WAKEFLOW_ACTIVE_ROOT, WAKEFLOW_LOCAL_ROOT, WAKEFLOW_PRESENTATION_LANGUAGES, } from "../../configuration/wakeflow-config.js";
 ...
 */
export const WAKEFLOW_PROGRAM_INSTRUCTION_BODY_AUTHORITY_KIND = "WakeflowProgramInstructionBodyAuthority";
export const WAKEFLOW_PROGRAM_INSTRUCTION_BODY_AUTHORITY_SCHEMA_VERSION = 1;
export const WAKEFLOW_PROGRAM_INSTRUCTION_COMPONENT = "program-instruction";
export const WAKEFLOW_PROGRAM_INSTRUCTION_OWNER = "host-instruction-integration";
...
export class WakeflowProgramInstructionBodyAuthorityError extends Error {
    name = "WakeflowProgramInstructionBodyAuthorityError";
    code = "wakeflow-program-instruction-body-authority";
    reason;
    path;
...
const PRESENTATION_LANGUAGE_SET = new Set(WAKEFLOW_PRESENTATION_LANGUAGES);
const HOST_ID_SET = new Set(WAKEFLOW_WORKSPACE_HOST_IDS);
const CONFIG_FILE_REF = "wakeflow.config.json";
...
function markdownDataLiteral(value, path) {
    try {
...
function englishBody(config, profile, controllerWindowId) {
    const description = config.program.description === undefined
        ? []
        : [
            `- Program description: ${markdownDataLiteral(config.program.description, "$/program/description")}`,
        ];
    return `${[
...
function simplifiedChineseBody(config, profile, controllerWindowId) {
    const description = config.program.description === undefined
        ? []
        : [
            `- 程序说明：${markdownDataLiteral(config.program.description, "$/program/description")}`,
        ];
    return `${[
        "## Wakeflow 程序指令",
        "",
...
function authorityDigestBasis(programId, controllerWindowId, hostId, instructionFileName, language, bodyDigest) {
    return {
        kind: "WakeflowProgramInstructionBodyAuthorityDigestBasis",
        schemaVersion: WAKEFLOW_PROGRAM_INSTRUCTION_BODY_AUTHORITY_SCHEMA_VERSION,
        programId,
        controllerWindowId,
        hostId,
        instructionFileName,
...
export function createWakeflowProgramInstructionBodyAuthority(configValue, profileValue) {
    ...
    const authorityDigest = computeCanonicalJsonSha256Digest(authorityDigestBasis(config.program.programId, controllerWindowId, profile.hostId, profile.instructionFileName, config.presentation.language, bodyDigest));
...
function parseAuthorityId(value, kind, path) {
    try {
...
function parseInstructionFileName(value) {
    try {
        const resourcePath = parsePortableResourcePath(value, "$authority.instructionFileName");
...
export function parseWakeflowProgramInstructionBodyAuthority(value) {
    ...
    const programId = parseAuthorityId(record.programId, "program", "$authority.programId");
    const controllerWindowId = parseAuthorityId(record.controllerWindowId, "window", "$authority.controllerWindowId");
    ...
    try {
        ...
        authorityDigest = parseSha256Digest(record.authorityDigest, "$authority.authorityDigest");
    }
    catch (error) {
    ...
    if (computeSha256Digest(encodeUtf8(record.body), "$authority.body")
        !== bodyDigest
        || computeCanonicalJsonSha256Digest(authorityDigestBasis(programId, controllerWindowId, hostId, instructionFileName, language, bodyDigest)) !== authorityDigest) {
        fail("authority", "$authority");
    }
    const target = plainRecord(record.envelopeTarget, "$authority.envelopeTarget");
