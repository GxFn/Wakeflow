...
| `.workspace-local/wakeflow-delivery/thread-registry/<window>.json` | local runtime | Real Codex thread registration for one window. | no |
| `.workspace-local/wakeflow-delivery/window-config/<window>.json` | local runtime | Derived child-window dispatch config, safe to regenerate from workspace config + thread registry. | no |
| `.workspace-local/wakeflow-delivery/dispatch-groups/<group>.json` | total control | First-class dispatch group protocol: state ref, expected targets, controller window, and return policy. | no |
| `.workspace-local/wakeflow-delivery/dispatch-packets/*.json` | total control | Controller-created work packet. | no |
| `.workspace-local/wakeflow-delivery/delivery-envelopes/*.json` | total control / delivery adapter | Mechanical delivery plan that references a dispatch packet. | no |
| `.workspace-local/wakeflow-delivery/delivery-runs/*.json` | delivery adapter | Actual direct-thread send attempt, readback, and failure evidence. | no |
| `.workspace-local/wakeflow-delivery/target-results/*.json` | target window | Target result envelope returned to total control. | no |
| `.workspace-local/wakeflow-delivery/keep-live/state.json` | delivery support | Current unattended keep-live process/state. | no |
| `.workspace-local/wakeflow-delivery/stop.json` | total control | Explicit stop marker for unattended automation. | no |
...
global `controllerWindow` points at a different controller.
