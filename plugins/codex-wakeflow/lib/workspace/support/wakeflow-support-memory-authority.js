import { buildWakeflowConfigIndexes, computeWakeflowConfigDigest, parseWakeflowConfig, WakeflowConfigError, WAKEFLOW_PRESENTATION_LANGUAGES, } from "../../configuration/wakeflow-config.js";
 ...
 */
export const WAKEFLOW_SUPPORT_MEMORY_AUTHORITY_KIND = "WakeflowSupportMemoryAuthority";
export const WAKEFLOW_SUPPORT_MEMORY_AUTHORITY_SCHEMA_VERSION = 1;
...
export class WakeflowSupportMemoryAuthorityError extends Error {
    name = "WakeflowSupportMemoryAuthorityError";
    code = "wakeflow-support-memory-authority";
    reason;
    path;
...
const LANGUAGE_SET = new Set(WAKEFLOW_PRESENTATION_LANGUAGES);
const ROLE_SET = new Set(["design", "test"]);
...
function dataLiteral(value, path) {
    try {
...
function declarationId(surfaceId, hostId) {
    return `support.${surfaceId}.instruction.${hostId}`;
}
function optionalDescription(surface, language) {
...
function englishBody(config, surface, windowId, profile) {
...
function simplifiedChineseBody(config, surface, windowId, profile) {
    const design = surface.capability === "design";
    const roleName = design ? "Design" : "Test";
    return `${[
...
function assertBody(body, reason) {
...
function digestBasis(configDigest, programId, surfaceId, windowId, role, hostId, instructionFileName, language, bodyDigest) {
    return {
...
function parseSurfaceId(value) {
    try {
        ...
        if (error instanceof WakeflowDurableIdError)
            fail("input", "$surfaceId");
        throw error;
    }
}
/** 为一个严格 wakeflow-managed Design/Test surface 生成 whole-file memory 权威。 */
export function createWakeflowSupportMemoryAuthority(configValue, profileValue, surfaceIdValue) {
    ...
    const authorityDigest = computeCanonicalJsonSha256Digest(digestBasis(configDigest, config.program.programId, surface.surfaceId, window.windowId, role, profile.hostId, profile.instructionFileName, language, bodyDigest));
...
function authorityId(value, kind, path) {
    try {
...
function instructionFileName(value) {
    try {
        const path = parsePortableResourcePath(value, "$authority.instructionFileName");
...
export function parseWakeflowSupportMemoryAuthority(value) {
    ...
    if (computeSha256Digest(encodeUtf8(record.body), "$authority.body")
        !== bodyDigest
        || computeCanonicalJsonSha256Digest(digestBasis(configDigest, programId, surfaceId, windowId, role, hostId, fileName, language, bodyDigest)) !== authorityDigest) {
        fail("authority", "$authority");
    }
    return Object.freeze({
        kind: WAKEFLOW_SUPPORT_MEMORY_AUTHORITY_KIND,
