 ...
 */
const CLAIM_KIND = "WakeflowWorkClaim";
...
function parseWorkClaim(value, path = "$claim") {
    let json;
    try {
    ...
    const record = exactObject(json, CLAIM_FIELDS, path);
    if (record.kind !== CLAIM_KIND || record.schemaVersion !== CLAIM_SCHEMA_VERSION) {
        fail("invalid-request", "claim-kind", `${path}/kind`);
    }
    const holderRecord = exactObject(record.holder, HOLDER_FIELDS, `${path}/holder`);
    ...
    const basis = claimBasis({
        ...
        holder: {
            ...
            targetTaskId: parseId(holderRecord.targetTaskId, "target-task", `${path}/holder/targetTaskId`),
...
/** 从草稿创建声明记录并封摘要。 */
export function createWorkClaim(draft) {
...
export function deriveWorkClaimId(namespace, demandId, idempotencyKey) {
    return deriveDurableId("work-claim", namespace, demandId, idempotencyKey);
}
/** 用声明标识里的 UUID 派生同源的其他身份，例如结果记录标识。 */
function workClaimUuid(claimId) {
    return claimId.slice("work-claim_".length);
}
export function derivedIdFromWorkClaim(kind, claimId) {
    return createWakeflowDurableId(kind, parseUuidV4(workClaimUuid(claimId), "$claimId"));
}
function renderWorkClaim(claim) {
    return renderDeterministicJsonDocument(claim, "$claim");
}
function assertRoot(root) {
    if (!(root instanceof RootedDirectory))
        fail("invalid-request", "claim-root", "$root");
}
function assertNode(node, kind, path) {
    if (node.kind !== kind ||
...
export async function inspectWorkClaim(root, windowIdValue, options = {}) {
    assertRoot(root);
    ...
    try {
        ...
        if (error instanceof RootedDirectoryError && error.reason === "resource-not-found") {
            return Object.freeze({ status: "absent", claim: null, node: null });
        }
    ...
    return Object.freeze({ status: "claimed", claim, node });
}
/**
 * 以独占创建取得声明：同字节重放为 `current`；被其他声明占用即
 * `precondition-failed/window-claimed`，details 列出持有者。
 */
export async function takeWorkClaim(root, claimValue, options = {}) {
    assertRoot(root);
    ...
    try {
        ...
        return Object.freeze({ disposition: "created", claim });
    }
    catch (error) {
...
export async function releaseWorkClaim(root, expectedValue, options = {}) {
    assertRoot(root);
