...
- Development work uses the plugin execution-craft skill `wakeflow-target-craft` (test-first, systematic debugging, self-review by severity, scope discipline, verify-before-done) so it produces structured review inputs whose presence, mapping, and declared paths can be machine-checked. The controller still independently validates truth and acceptance. The craft skill loads via the Wakeflow plugin alongside `wakeflow-target`; this window does NOT use the Design or Test windows' built-in skills.
