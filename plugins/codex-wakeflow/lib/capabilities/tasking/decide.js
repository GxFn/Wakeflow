...
export function deriveSectionAnchorBlockers(sectionAnchors, recordSectionAnchors) {
    ...
    return Object.freeze(sectionAnchors
        .filter((anchor) => !known.has(anchor))
        .map((anchor) => `section-anchor-unknown:${anchor}`));
}
/** 需求包要求用户过目任务清单时，请求必须带确认时间；否则确认时间不得出现。 */
export function derivePlanReview(input) {
    if (input.taskPlanReview === "user") {
        return input.confirmedAt === null
            ? Object.freeze({ planReview: null, blocker: "task-plan-review-required" })
            : Object.freeze({
                planReview: Object.freeze({ reviewer: "user", confirmedAt: input.confirmedAt }),
                blocker: null,
            });
    }
    return input.confirmedAt === null
        ? Object.freeze({
            planReview: Object.freeze({ reviewer: "controller" }),
            blocker: null,
        })
        : Object.freeze({ planReview: null, blocker: "task-plan-review-not-requested" });
}
/** 仓库里的谱系头：有未接受且未被替代的目标就必须替代它；只剩已接受目标就必须续接。 */
export function deriveLineageExpectation(state, repositoryId) {
    const sameRepository = state.targetTasks.filter((target) => target.workType !== "test" && target.repositoryId === repositoryId);
    const open = sameRepository.find((target) => target.phase !== "accepted" && target.phase !== "superseded");
    if (open !== undefined) {
        return Object.freeze({
            kind: "replacement",
...
export function deriveLineageBlockers(lineage, expectation) {
...
export function deriveTopologyBlockers(input) {
    ...
    if (repository === undefined)
        blockers.push("repository-unknown");
    if (window === undefined)
        blockers.push("window-unknown");
    else if (window.role !== "product")
        blockers.push(`window-role:${window.role}`);
...
export function deriveTestStepReferenceBlockers(input) {
    const itemIds = new Set(input.criteria.map((criterion) => criterion.itemId));
    const blockers = [];
...
function implementationReadinessBlockers(state) {
    const implementation = state.targetTasks.filter((target) => target.workType !== "test" && target.phase !== "superseded");
    if (implementation.length === 0)
        return Object.freeze(["implementation-targets-missing"]);
...
function testLineageBlockers(state, lineage) {
    ...
    if (pending !== undefined) {
        ...
        if (lineage === null)
            blockers.push(`lineage-retest-required:${previous}`);
 ...
 */
export function deriveTestPlanningBlockers(input) {
    const { state, lineage } = input;
    const blockers = [];
    if (input.testingMode !== "real-environment")
        blockers.push(`testing-mode:${input.testingMode}`);
    if (state.lifecycle !== "active")
        blockers.push(`demand-lifecycle:${state.lifecycle}`);
    if (state.awaitingDecision !== undefined)
...
export function deriveImplementationBaselines(state) {
    return Object.freeze(state.targetTasks
        .filter((target) => target.workType !== "test" && target.phase === "accepted")
