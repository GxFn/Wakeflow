...
- The Claude edition ships `commands/` (7 slash commands) and `artifact-checks`
  ...
  block.
- All edition manifests are version-locked at **0.8.13** in **five places**: both
  ...
  two different version fields and only the `plugins[0]` one tracks the 0.8.13
...
> dispatch (e.g. `hostProfile.hostId` used as an object key). The five 0.8.13
