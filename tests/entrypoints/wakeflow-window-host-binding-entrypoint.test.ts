...
async function fixture(t: TestContext): Promise<string> {
  const root = realpathSync(mkdtempSync(path.join(os.tmpdir(), "wakeflow-window-binding-public-")));
...
test("Maintenance launch intent 经 Agent result 注册为私有 Binding 与脱敏投影", {
  timeout: 30_000,
}, async (t) => {
  ...
  const projectionFile = path.join(root, ...publicValue.projection.resourceRef.split("/"));
  ...
  equal(replayedValue.binding.source.observedAt, publicValue.binding.source.observedAt);
  ...
  match(readFileSync(recoveryProjectionFile, "utf8"), /"status": "registered"/u);
  ...
  equal(new Set(concurrentValues.map((value) => value.binding.bindingId)).size, 1);
  match(readFileSync(concurrentProjectionFile, "utf8"), /"status": "registered"/u);
  ...
  equal(rollbackValue.binding.registeredAt < rollbackValue.binding.source.observedAt, true);
  equal(existsSync(path.join(root, ...rollbackValue.binding.bindingRef.split("/"))), true);
