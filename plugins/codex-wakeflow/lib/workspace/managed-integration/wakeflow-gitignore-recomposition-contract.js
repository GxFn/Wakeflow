...
import { createWakeflowGitignoreBodyAuthority, WakeflowGitignoreBodyAuthorityError, } from "./wakeflow-gitignore-body-authority.js";
/**
...
export const WAKEFLOW_GITIGNORE_RECOMPOSITION_LOCK_TIMEOUT_MILLISECONDS = 10_000;
const ERROR_MESSAGES = {
    input: "Wakeflow Gitignore recomposition input is invalid.",
    "unsupported-platform": "Wakeflow Gitignore recomposition requires reliable local POSIX ownership semantics.",
...
export class WakeflowGitignoreRecompositionError extends Error {
    name = "WakeflowGitignoreRecompositionError";
    code = "wakeflow-gitignore-recomposition";
    reason;
    path;
...
export function failWakeflowGitignoreRecomposition(reason, path) {
    throw new WakeflowGitignoreRecompositionError(reason, path);
}
export function assertWakeflowGitignoreRecompositionRoot(value) {
...
function parseProfiles(value) {
    let values;
    try {
        values = parseDenseArray(value, WAKEFLOW_WORKSPACE_HOST_IDS.length, "$request.hostProfiles");
    }
    catch (error) {
        if (error instanceof PassiveOwnDataError) {
    ...
    const parsed = values.map((profile, index) => {
        try {
            ...
            if (error instanceof WakeflowWorkspaceHostResourceProfileError) {
                failWakeflowGitignoreRecomposition("input", `$request.hostProfiles/${index}`);
            }
            throw error;
        }
    });
...
export function parseWakeflowGitignoreRecompositionRequest(value) {
    ...
    try {
        ...
        if (error instanceof Sha256Error) {
            failWakeflowGitignoreRecomposition("input", "$request.expectedMatrixDigest");
        }
        throw error;
    }
    ...
    const currentProfile = hostProfiles.find((profile) => (profile.hostId === matrix.hostId));
...
export function parseWakeflowGitignoreRecompositionOptions(value) {
...
export function parseWakeflowGitignoreRecompositionRecoveryOptions(value) {
    const record = plainRecord(value === undefined ? {} : value, "$options");
...
export function assertWakeflowGitignoreRecompositionNotAborted(signal) {
    if (signal?.aborted === true) {
        failWakeflowGitignoreRecomposition("aborted", "$signal");
    }
}
export function currentWakeflowGitignoreRecompositionUserId() {
    if (process.platform === "win32" || typeof process.geteuid !== "function") {
        failWakeflowGitignoreRecomposition("unsupported-platform", "$root");
    }
    return BigInt(process.geteuid());
}
export async function assertCurrentUserWakeflowGitignoreRecompositionRoot(root, expectedUserId) {
    try {
...
export function wakeflowGitignoreInspectionRequest(request, signal) {
    return Object.freeze({
...
export function wakeflowGitignoreRecompositionLockOptions(options) {
    return {
...
export function mapWakeflowGitignoreRecompositionLockError(error, committed) {
    if (committed) {
