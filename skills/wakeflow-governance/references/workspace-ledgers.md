...
Use this reference when creating, moving, syncing, or archiving Wakeflow documents, status mirrors, indexes, templates, Design handoff ledgers, test exchange entries, or workspace skill assets.
...
- Legacy current plan / window dispatch documents remain readable only
...
- `.workspace-active/workspace/current/` holds current status, active TODO, test exchange projections, and active Wakeflow plans / state roots.
...
- Single-repo execution backfill should still be linked back from the active Wakeflow plan or workspace index.
...
- Real test project docs remain project docs; product validation plans, scans, acceptance, and reproduction records should be kept in Wakeflow workspace docs or the configured Test repository `docs/`.
...
- `skills/<skill-name>/` stores actively developed skills.
...
- Use execution date `YYYY-MM-DD` for dated Wakeflow docs.
...
- Render developer progress projection:
  `node scripts/wakeflow-render-progress.mjs --state-root <state-root> --write`
- Verify Wakeflow:
