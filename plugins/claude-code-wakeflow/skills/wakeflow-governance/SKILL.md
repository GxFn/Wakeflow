...
- **Multiple demands = pod 0 + isolation pods**: up to `maxActiveDemands` (default 2) demands run side by side and remain mutually unaware. The default fleet is pod 0: persistent controller/Test/work windows on main checkouts. Only demand 2..N gets its OWN controller (`Controller__<pod>`, stamped into the state root), OWN Test, tmux session, and ONE isolation worktree set. Parallelism exists ONLY at the demand level — never inside a demand; every isolation-pod window, Test included, works in that pod's worktree set, never on a main checkout. Branch merge-back is human-reviewed and decentralized (pending-merges ledger is the memory); claiming past capacity fails closed.
