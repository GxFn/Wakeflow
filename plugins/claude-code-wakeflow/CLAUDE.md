...
- MCP state mutations are authorized against the host-supplied caller handle
  and host-scoped session registry. An initialized workspace rejects an unknown
...
- Parallelism exists ONLY at the demand level, as demand pods: up to
  ...
  own controller, Test, tmux session, and ONE worktree set; every window in
  that isolation pod works and verifies inside that set, never on a main
