...
/**
 * Closed MCP input contract for one execution-endpoint operation on a logical window: inspect (launch intent and current binding, claim, and locator state), register (bind the handle the Agent observed), replace (bind a new handle with CAS on the old binding), relocate (keep the binding and record a resumed session's new tmux pane, with CAS on the binding), decommission (release the binding with closure evidence), or release-claim (force-release an expired or orphaned work claim with liveness evidence).
 */
export type WakeflowWindowHostBindingRegistrationRequestV1 = (InspectRequest | RegisterRequest | ReplaceRequest | RelocateRequest | DecommissionRequest | ReleaseClaimRequest)
/**
...
/**
 * What the Agent observed after executing one launch intent: the opaque host handle, on Claude Code the tmux coordinates of the pane running the session, and for the product window of a worktree pod the git worktree facts of that session.
 */
...
/**
 * Git facts the Agent observed inside the product session of a worktree pod: the verbatim output of `git worktree list --porcelain` and of `git rev-parse --git-common-dir` run in the session cwd. Wakeflow parses them, selects the checkout equal to the session-start cwd, and cross-checks the .git pointer files; nothing here is echoed back.
 */
