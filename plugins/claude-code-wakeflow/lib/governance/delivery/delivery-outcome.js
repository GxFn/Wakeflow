 ...
 */
const OUTCOME_KIND = "WakeflowDeliveryOutcome";
...
export class DeliveryOutcomeError extends Error {
    name = "DeliveryOutcomeError";
    code = "wakeflow-delivery-outcome";
    reason;
    path;
...
const validateWire = createRuntimeJsonSchemaValidator(WAKEFLOW_DELIVERY_OUTCOME_SCHEMA, [
...
export function deliveryClaimHandling(disposition) {
    return disposition === "rejected-before-send" ? "release-authorized" : "retain";
}
function outcomeBasis(value) {
...
export function parseDeliveryOutcome(value) {
    let json;
    try {
    ...
    if (computeCanonicalJsonSha256Digest(basis) !== outcomeDigest)
        fail("digest", "$/outcomeDigest");
    return Object.freeze({ ...basis, outcomeDigest });
}
/** 从草稿封一份处置记录；声明处理由处置派生。 */
export function createDeliveryOutcome(draft) {
    const basis = outcomeBasis({
        ...draft,
        kind: OUTCOME_KIND,
        schemaVersion: OUTCOME_SCHEMA_VERSION,
