...
| --- | --- | --- | --- | --- | --- |
| 1 | `wakeflow_status` | 9 | `wakeflow_status`（带 demandId 即附路由或归档回执） | 重切 | 2026-09-18 L1 observation 切片 10（gate-log §13.94、§13.96）：治理层一次观察多域，每域独立隔离失败；pod 段、待认领摘要、已接受未合并列表、`policy` 生效值、去重排序上限 64 的 `nextActions`；`wakeflow_inspect_demand_route` 并入；hostOperations 删除（ADR-0012 D2）；场景 `card-09/status-and-verify`；§13.127：顶层 `runtime{artifactManifestDigest, artifactOnDisk}`、每窗口 `artifact: current | stale | unknown`、nextActions `window-artifact-stale` / `runtime-artifact-outdated`；§13.129：顶层 `maintenance{status, protocol}`，维护协议非 idle 即 `overall: maintenance` 并指向维护；零写、确定与隐私回归；§13.130：`unmergedAccepted` 另从仍在配置里的 worktree pod 的归档（含撤销）Demand 派生，条目带 `source: active | archived`，`domains.archives` 与 `truncated.archives`；`maintenance.residues[]` 点名事务残留（维护进行中不标可恢复） |
| 2 | `wakeflow_maintain_workspace` | 1 | `wakeflow_maintain_workspace` | 重切 | 已实现 fresh、reconfigure、reconcile 三动作；config 增加 `pods[]` 与窗口 `podId`（2026-09-10 pod 切片 9，fresh 生成 `main`，reconfigure 拒改 `pods`）；TSD-16 从 v1 起版已做（2026-09-19，gate-log §13.102）；managed-block 仓库与 external-owned 支撑面的托管块（对齐台账 G1）2026-09-21 落地（gate-log §13.106）；§13.124：工作区根或配置根与已装载制品重叠即 blocked（`*-overlaps-artifact`）；§13.130：reconfigure 可新增产品仓库（连同 primary pod 的一个产品窗口；根须是 Git 主检出、不与已配置仓库重合），删改已有条目仍拒；reconcile 先做私有模式普查，安全漂移收回 0700 / 0600，不安全节点只报告（`private-mode-unsafe:<区域>`），其余意图以 `private-mode-drift` 拒绝 |
...
| 4 | `wakeflow_register_window` | 2 | `wakeflow_register_window_binding` | 重切 | 已实现；窗口在配置里带 `podId`，worktree pod 的产品窗口握手带 worktree 回执（2026-09-10 pod 切片 9，gate-log §13.92）；§13.125：`relocate` 保留绑定、换定位器代际（Claude 助手 `resume` 之后），`launch` / `resume` 拒绝活着的 pane，等不到 hook 且新 pane 已死或消失时报 `resume-exited` / `launch-exited`；§13.128：worktree 观察豁免请求隐私扫描，`worktree-occupied` 守卫，变更后重算活动投影；§13.130：Claude 助手 `nudge`（被切断的一轮续做一次）、`launch` 为 local-head worktree 意图自备检出、回读认折叠粘贴指示 |
...
| 8 | `wakeflow_record_delivery` | 6 | `record_delivery_outcome`（hook 记录惰性核对）、`rearm_delivery` | 已落地 | 2026-09-10：处置由 `user-prompt-submit` 记录或 Codex 发送返回派生；indeterminate 再调用重查；rearm 上限 3（gate-log §13.84）；§13.122：Claude 助手 `deliver` 送前 pane authority（duplicate-pane / wrong-process / locator-stale）与 hook 记录的 `landing` 观察 |
...
| 30 | `wakeflow_prune_runtime` | 8 | 并入 pod 关闭 | 已落地 | close 第二段要求检出已由 Agent 处置，Wakeflow 只删自己的回执目录；对账只报告（观察切片）；§13.130：登记时加锁的检出，建议命令先 `git worktree unlock` 再 remove |
| 31 | `wakeflow_verify` | 9 | `wakeflow_verify` | 重切 | 2026-09-18 L1 observation 切片 10：工作区 14 门按名字排序（含 host-hook-channel、pod-execution-location、host-settings-assets、active-projection，2026-09-21 补回 window-runtime-projection，gate-log §13.111），`ok` 要求全部 pass，`unavailable` 与 `fail` 分开计数；带 demandId 复用 demand 切片的门；场景 `card-09/status-and-verify`；§13.127：第 15 门 `runtime-artifact`（`server-outdated` / `windows-stale:<n>` / not-applicable）；§13.129：工具描述改为十五门清单，被打断的维护 apply 经 local-layout 的 `maintenance-protocol-<状态>` 报出；§13.130：local-layout 门按区域点名私有模式漂移（`private-mode-drift-<n>:<区域>` / `private-mode-unsafe-<n>:<区域>`） |
