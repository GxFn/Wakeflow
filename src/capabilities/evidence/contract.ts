import {
  WAKEFLOW_RECORD_EVIDENCE_REQUEST_SCHEMA,
  type WakeflowRecordEvidenceRequestV1,
} from "../../contracts/generated/entrypoints/wakeflow-record-evidence-request.generated.js";
import {
  WAKEFLOW_RECORD_EVIDENCE_RESULT_SCHEMA,
  type WakeflowRecordEvidenceResultV1,
} from "../../contracts/generated/entrypoints/wakeflow-record-evidence-result.generated.js";
import {
  JsonValueError,
  parseJsonValue,
  type JsonValue,
...
export const WAKEFLOW_RECORD_EVIDENCE_PUBLIC_TOOL_NAME = "wakeflow_record_evidence" as const;
export const WAKEFLOW_RECORD_EVIDENCE_PUBLIC_SCHEMA_VERSION = 1 as const;

export type RecordEvidenceRequest = Readonly<WakeflowRecordEvidenceRequestV1>;
export type RecordEvidenceResult = Readonly<WakeflowRecordEvidenceResultV1>;

const validateRequest = createRuntimeJsonSchemaValidator<WakeflowRecordEvidenceRequestV1>(
  WAKEFLOW_RECORD_EVIDENCE_REQUEST_SCHEMA,
);
const validateResult = createRuntimeJsonSchemaValidator<WakeflowRecordEvidenceResultV1>(
  WAKEFLOW_RECORD_EVIDENCE_RESULT_SCHEMA,
);
...
export function parseRecordEvidenceRequest(value: unknown): RecordEvidenceRequest {
  const result = validateRequest(requestJson(value));
  if (!result.ok) fail("invalid-request", "schema", `$request${result.path.slice(1)}`);
  return result.value;
}

export function admitRecordEvidenceResult(value: unknown): RecordEvidenceResult {
