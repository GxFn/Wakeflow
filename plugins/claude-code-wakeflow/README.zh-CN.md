...
flowchart TD
  ...
  Delivery --> Host["Claude Code 投递<br/>desktop session message 或 headless resume"]
...
窗口传输是 Claude Code 版本的关键差异。Wakeflow 支持两种投递模式，
两种模式共享同一套 envelope、证据和 review 合约。

**desktop-session 模式。** 每个 Wakeflow 窗口（总控、仓库窗口、Design、Test）
都是一个 Claude Code desktop session。总控用 desktop session message tool
把 envelope prompt 投递到目标 session。用户想保持窗口可见、亲眼看工作流转时，
这是自然的选择。

**headless-resume 模式。** 每个窗口是一个持久的 CLI session。总控通过
后台任务 resume 目标来完成投递：
...
JSON 运行结果就是这次投递的 readback 证据。被 resume 的 session 可能 fork 出
新的 `session_id`；每次运行后必须把新 id 重新注册进本地 thread registry，
否则下一次投递会唤醒过期 session。

`workspace.config.json` 可以显式固定模式：
...
允许的值是 `"desktop-session"` 和 `"headless-resume"`。未固定模式时，
默认行为优先使用打开的 desktop session，并在不可用时回退到 headless resume。

**headless 模式的无人值守权限。** Headless 目标无法回答权限询问。无人值守运行前，
请在每个仓库的 `.claude/settings.json` 中配置按仓库的 allowlist，
或在清楚后果的前提下为可信 session 选择 `--permission-mode acceptEdits`。
Wakeflow 永远不替用户做这个选择：这是按仓库的主动决策，不是默认值。

## 跨宿主统一词汇

Wakeflow 在不同宿主版本之间保持同一套机器词汇。registry、payload 字段和 CLI flag
里的 "thread id" 就是 Claude Code session id；没有任何字段按宿主改名。
...
7. Claude Code 按返回的 launch plan 创建各窗口 session（按当前窗口模式创建
   desktop session 或持久 CLI session），并把每个真实 session id 只传一次给
   Wakeflow 本地注册命令。thread registry 是唯一 session-id 权威；
...
| `workspace.config.json` | 受管窗口、仓库路径、角色、投递模式和默认语言。 |
...
- 真实 session id 只存在 `.workspace-local/wakeflow-delivery/thread-registry/`。
- Window config 从 `workspace.config.json` 和 thread-registry presence 派生，不是第二份 session-id 权威。
- Delivery prompts 保持轻量、可读。
- Host 通过 desktop session message tool 或 headless resume 运行发送 prompt；
  Wakeflow 记录发送和 readback 证据。
- headless-resume 模式下，JSON 运行结果就是 readback 证据，fork 出的新 session id
  必须在下一次投递前重新注册。
...
通过当前窗口模式发送 prompt、记录 delivery run。
...
通过当前窗口模式发送，再记录 delivery evidence。总控 review 也保持拆分：
