...
import { admitDemandAuthority, computeDemandAuthorityDigest, parseDemandAuthorityDocument, DemandAuthorityError, } from "../model/demand-authority.js";
import { computeDemandIdentityDigest, parseDemandIdentityDocument, DemandIdentityError, } from "../model/demand-identity.js";
import { DemandEventSourcingRepository, DemandEventSourcingRepositoryError, } from "./demand-event-sourcing-repository.js";
import { DemandEventSourcingRootInventoryError, inspectDemandEventSourcingRootInventory, } from "./demand-event-sourcing-root-inventory.js";
import { DEMAND_EVENT_SOURCING_AUTHORITY_REF, DEMAND_EVENT_SOURCING_IDENTITY_REF, } from "./demand-event-sourcing-paths.js";
...
export class DemandEventSourcingRootAuthorityError extends Error {
    name = "DemandEventSourcingRootAuthorityError";
    code = "wakeflow-demand-event-sourcing-root-authority";
    reason;
    path;
...
function sameOptionalNode(left, right) {
...
function sameManagedEvidencePublication(left, right) {
    ...
    return (left.transactionDigest === right.transactionDigest &&
        ...
        (leftProgress === undefined || rightProgress === undefined
            ...
            : leftProgress.candidateRootPath === rightProgress.candidateRootPath &&
                ...
                sameStringArray(leftProgress.missingDirectories, rightProgress.missingDirectories) &&
                sameStringArray(leftProgress.missingFiles, rightProgress.missingFiles)));
}
function sameManagedEvidenceInventory(left, right) {
    return (sameManagedEvidencePublication(left.publication, right.publication) &&
...
function sameInventory(left, right) {
    return (left.commitCount === right.commitCount &&
        ...
        sameManagedEvidenceInventory(left.managedEvidence, right.managedEvidence) &&
        ...
        sameFileNodeSnapshot(left.nodes.appendCandidates, right.nodes.appendCandidates) &&
...
function sameManagedEvidenceSelector(record, selector) {
    return (record.evidenceId === selector.evidenceId &&
        record.manifestDigest === selector.manifestDigest &&
        record.payloadArtifactDigest === selector.payloadArtifactDigest);
}
function assertClosedManagedEvidenceRecords(records, selectors, identity, authorityDigest) {
    if (records.length !== selectors.length ||
...
function assertManagedEvidenceClosure(inventory, identity, authorityDigest, aggregate, phase) {
    ...
    if (phase === "healthy") {
        if (publication !== null)
            fail("closure", "$managed-evidence");
        assertClosedManagedEvidenceRecords(records, selectors, identity, authorityDigest);
        return;
    }
    if (publication === null)
        fail("closure", "$managed-evidence");
    const manifest = publication.transaction.manifest;
    if (manifest.evidenceId !== publication.evidenceId ||
        manifest.programId !== identity.programId ||
        manifest.demandId !== identity.demandId ||
        manifest.demandAuthorityDigest !== authorityDigest) {
        fail("closure", "$managed-evidence");
    }
    const targetSelector = selectors.find((selector) => selector.evidenceId === manifest.evidenceId);
    const targetRecord = records.find((record) => record.evidenceId === manifest.evidenceId);
    ...
    assertClosedManagedEvidenceRecords(records, targetSelector === undefined
        ? selectors
        : selectors.filter((selector) => selector.evidenceId !== manifest.evidenceId), identity, authorityDigest);
}
async function readRecord(root, ref, maximumBytes, expectedNode, signal, owner) {
    try {
...
function parseOptions(value) {
    ...
    if (Object.keys(record).some((key) => key !== "audit" && key !== "signal") ||
...
async function loadRootAuthority(root, ledgerStore, options, phase = "healthy") {
    ...
    try {
        admittedAuthority = await admitDemandAuthority(identity, authority, ledgerStore, signal === undefined ? undefined : { signal });
    }
    catch (error) {
        if (error instanceof DemandAuthorityError ||
            error instanceof LedgerAuthorityStoreError) {
            if ((error instanceof DemandAuthorityError && error.reason === "aborted") ||
                (error instanceof LedgerAuthorityStoreError &&
    ...
    let aggregate;
    let loadMode;
    let replayedCommitCount;
    try {
        if (audit) {
            const audited = await repository.audit(signal === undefined ? undefined : { signal });
            aggregate = audited.aggregate;
            replayedCommitCount = audited.replayedCommitCount;
            loadMode = "audit";
        }
        else {
            const loaded = await repository.load(signal === undefined ? undefined : { signal });
            if (loaded === null)
                fail("stream", "$commits");
    ...
    let firstCommit;
    try {
        firstCommit = await eventStore.readCommitAt(1, signal === undefined ? undefined : { signal });
    }
    catch (error) {
    ...
    if (aggregate.demandId !== identity.demandId ||
    ...
    assertManagedEvidenceClosure(inventory, identity, authorityDigest, aggregate, phase);
    let finalInventory;
    try {
        finalInventory = await inspectDemandEventSourcingRootInventory(root, {
            phase,
            ...(signal === undefined ? {} : { signal }),
        });
    }
...
export async function loadDemandEventSourcingRootAuthority(root, ledgerStore, options) {
 ...
 */
export async function loadDemandEventSourcingRootAuthorityDuringManagedEvidencePublication(root, ledgerStore, options) {
    return loadRootAuthority(root, ledgerStore, options, "managed-evidence-publication");
}
