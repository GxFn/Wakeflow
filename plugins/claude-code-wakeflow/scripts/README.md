...
workspace controllers use the named MCP tools, commands, and Skills; they do
not select internal modules or infer command flags from this directory.
...
- `wakeflow-bootstrap.mjs` is the explicit-migration backend reached only by
  ...
  an MCP tool, command, or normal-runtime fallback.
- `wakeflow-smoke.mjs` verifies the packaged v3 artifact in disposable
  Git-backed workspaces without contacting a real Claude session.
...
`scripts/lib/wakeflow-claude-host.mjs` is the thin host facade for current
Claude lifecycle, activation, transport, Pod, and close operations. It delegates
to typed v3 owners and does not restore a parallel state or transport system.
Machine-verifiable close requires the exact close operation followed by a
bounded absence probe. When that proof is unavailable, the operation remains a
manual host gate rather than claiming physical revocation.

## Supported Routes

| Need | Route |
| --- | --- |
| Normal installed-workspace operation | Use the exact Wakeflow MCP tool or packaged command that maps to it. |
