...
- The Claude edition ships `commands/` (7 slash commands) and `artifact-checks`
  ...
  block.
- All edition manifests are version-locked at **0.9.3** in **five places**: both
  ...
  two different version fields and only the `plugins[0]` one tracks the 0.9.3
...
> dispatch (e.g. `hostProfile.hostId` used as an object key). The five 0.9.3
...
| `wakeflow_record_delivery` | `wakeflow-delivery` | `record-delivery-run` — MCP requires `--delivery-file`,`--status`, explicit `--transport-status accepted\|rejected-before-send\|ambiguous`, explicit `--readback-status confirmed\|pending\|unavailable`, explicit `--readback-attempts`, evidence/error/host fields and `--delivery-run-id`; the raw CLI alone still reads legacy `--readback-ok` confirmed writes for compatibility; `--compact` unless `verbose` |
...
| `record-delivery-run status=sent` | requires explicit `transportStatus=accepted`, explicit readback status/attempt count, and non-empty `--evidence`; `markStateRootDeliverySent` advances target state and refreshes the lock, while a controller-return with pending/unavailable readback is projected as `sent-unconfirmed` rather than reached |
...
- **Controller-return delivery status for group**: `not-applicable` →
  `not-built` → `pending-host-send` → `sent-unconfirmed` | `sent`. Both accepted
  states block duplicate returns, but only `sent` means confirmed destination
  visibility.
