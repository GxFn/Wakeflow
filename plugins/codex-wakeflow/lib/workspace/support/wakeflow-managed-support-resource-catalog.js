...
import { parseWakeflowWorkspaceResourceDeclaration, WakeflowWorkspaceResourceDeclarationError, } from "../workspace-resource-declaration.js";
/**
 ...
 */
export const WAKEFLOW_MANAGED_SUPPORT_RESOURCE_CATALOG_KIND = "WakeflowManagedSupportResourceCatalog";
...
export class WakeflowManagedSupportResourceCatalogError extends Error {
    name = "WakeflowManagedSupportResourceCatalogError";
    code = "wakeflow-managed-support-resource-catalog";
    reason;
    path;
...
function memoryDeclaration(surface, profile) {
    return declaration({
        kind: "WakeflowWorkspaceResourceDeclaration",
        declarationId: `support.${surface.surfaceId}.instruction.${profile.hostId}`,
        family: "support",
...
export function createWakeflowManagedSupportResourceCatalog(configValue, profileValue) {
    ...
    for (const surface of config.topology.supportSurfaces) {
        ...
        declarations.push(rootDeclaration(surface), memoryDeclaration(surface, profile));
    }
