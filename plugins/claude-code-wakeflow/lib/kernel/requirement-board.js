import { WAKEFLOW_REQUIREMENT_CLAIM_STATE_SCHEMA, } from "../contracts/generated/governance/board/requirement-claim-state.generated.js";
...
import { REQUIREMENT_BOARD_INDEX_REF, REQUIREMENT_BOARD_ROOT_REF, requirementClaimStateRef, } from "./layout.js";
/**
...
const STATE_FILE_PATTERN = /^(requirement_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.json$/u;
...
const BOARD_LOCKS_ROOT_REF = parsePortableResourcePath(`${REQUIREMENT_BOARD_ROOT_REF}/${LOCKS_DIRECTORY_NAME}`, "$layout");
function claimStateLockRef(requirementId) {
    return parsePortableResourcePath(`${BOARD_LOCKS_ROOT_REF}/${requirementId}.lock`, "$layout");
}
/** 每个需求包一把互斥锁：期望检查与替换在锁内连成一步，两个认领者恰有一个成功。 */
async function withClaimStateLock(root, requirementId, signal, operation) {
    try {
...
const validateState = createRuntimeJsonSchemaValidator(WAKEFLOW_REQUIREMENT_CLAIM_STATE_SCHEMA, [
    WAKEFLOW_SHA256_DIGEST_SCHEMA,
    WAKEFLOW_UTC_INSTANT_SCHEMA,
    WAKEFLOW_PORTABLE_RESOURCE_PATH_SCHEMA,
]);
function assertRelations(state) {
...
/** 准入一份认领状态：Schema 加关系。 */
function parseRequirementClaimState(value) {
...
function advance(current, at, patch) {
    return parseRequirementClaimState({
        ...current,
        ...patch,
...
function requireStatus(current, allowed, transition) {
    if (!allowed.includes(current.status)) {
        fail("precondition-failed", `claim-${transition}-from-${current.status}`, "$claimState.status");
    }
}
/** parked → pending。 */
export function activateRequirementClaim(current, at) {
...
export function withdrawRequirementClaim(current, reason, at) {
...
export function claimRequirementPackage(current, demandId, at) {
...
export function archiveRequirementClaim(current, at) {
...
export function reclaimRequirementPackage(current, demandId, at) {
...
async function readSource(root, ref, signal) {
    try {
        const read = await readDeterministicJsonFile(root, ref, {
            maximumBytes: STATE_MAXIMUM_BYTES,
            ...signalOptions(signal),
        });
        const state = parseRequirementClaimState(parseDeterministicJsonDocument(read.text, "$claimState"));
...
export async function readRequirementClaimState(root, requirementId, signal) {
    return readSource(root, requirementClaimStateRef(requirementId), signal);
}
async function readBoardEntry(root, entry, signal) {
    const match = STATE_FILE_PATTERN.exec(entry.name);
    if (match === null) {
        const expected = entry.name === "index.md" ||
...
export async function listRequirementClaimStates(root, signal) {
    let listing;
    try {
    ...
    for (const entry of entries) {
        ...
        if (outcome === "skip")
            skipped += 1;
        else if (outcome !== "index")
            states.push(outcome);
    }
    return Object.freeze({ states: Object.freeze(states), skipped });
}
/** 确保看板目录存在（0700）。 */
export async function materializeRequirementBoardRoot(root, signal) {
    try {
...
export async function createRequirementClaimStateFile(root, state, signal) {
    if (state.revision !== 1)
        fail("invalid-request", "claim-state-revision", "$claimState.revision");
    ...
    return withClaimStateLock(root, state.requirementId, signal, async () => {
        ...
        try {
            await createFileAtomically(root, ref, encodeUtf8(renderRequirementClaimState(state), "$claimState"), { mode: FILE_MODE, ...signalOptions(signal) });
        }
        catch (error) {
...
export async function replaceRequirementClaimStateFile(root, expected, next, signal) {
    ...
    await withClaimStateLock(root, next.requirementId, signal, async () => {
        ...
        try {
            await replaceFileAtomically(root, ref, encodeUtf8(renderRequirementClaimState(next), "$claimState"), {
                mode: FILE_MODE,
                expected: {
...
export function compareRequirementClaimStates(left, right) {
...
export function renderRequirementBoardIndex(states) {
    const rows = [...states]
        .filter((state) => state.status !== "withdrawn" && state.status !== "archived")
        .sort(compareRequirementClaimStates)
        .map((state) => `| ${state.priority} | ${state.status} | \`${state.requirementId}\` | ${escapeCell(state.title)} | ${state.demandType} | ${state.publishedAt} | ${state.claim === null ? "—" : `\`${state.claim.demandId}\``} |`);
    const lines = [
        "# Requirement Board",
        "",
...
const REQUIREMENT_BOARD_EMPTY_INDEX_DIGEST = computeSha256Digest(encodeUtf8(renderRequirementBoardIndex([]), "$boardIndex"), "$boardIndex");
/** 看板初始化权威的摘要：布局引用加空索引摘要；维护计划用它做步骤目标。 */
export const REQUIREMENT_BOARD_INITIALIZATION_AUTHORITY_DIGEST = computeCanonicalJsonSha256Digest({
...
export async function publishRequirementBoardIndex(root, states, signal) {
    await materializeRequirementBoardRoot(root, signal);
...
function isIndexRace(error) {
    return (error instanceof WakeflowError &&
 ...
 */
export async function refreshRequirementBoardIndex(root, signal) {
