...
1. Confirm this session is a controller window and read ITS demand's state root — with multiple active demands, a pod controller (`Controller__<pod>`) drives only the demand whose state root stamps it as `controllerWindow`; the default controller drives the rest. If `$ARGUMENTS` names a target task id, use it; otherwise pick the next eligible task from `wakeflow_next_work` and confirm it advances the active demand.
