...
- **Multiple demands = demand pods**: up to `maxActiveDemands` (default 2) demands run side by side, each with its OWN controller (stamped into the state root at create so returns route home) and its own window set, mutually unaware. Parallelism exists ONLY at the demand level — never inside a demand (each repo = one window = one combined task package). Branch merge-back is human-reviewed and decentralized; claiming past capacity fails closed. (Pod session tooling is Claude Code-edition; Codex pods = per-demand thread sets, symmetric landing pending.)
