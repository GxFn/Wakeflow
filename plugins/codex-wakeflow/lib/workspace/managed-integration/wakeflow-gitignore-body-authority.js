import { WAKEFLOW_ACTIVE_ROOT, WAKEFLOW_LOCAL_ROOT, } from "../../configuration/wakeflow-config.js";
 ...
 */
export const WAKEFLOW_GITIGNORE_BODY_AUTHORITY_KIND = "WakeflowGitignoreBodyAuthority";
export const WAKEFLOW_GITIGNORE_BODY_AUTHORITY_SCHEMA_VERSION = 1;
export const WAKEFLOW_GITIGNORE_COMPONENT = "workspace-ignore";
export const WAKEFLOW_GITIGNORE_OWNER = "workspace-ignore-integration";
const ERROR_MESSAGES = {
    input: "Wakeflow Gitignore body authority input is invalid.",
    "profile-set": "Wakeflow Gitignore body authority requires one complete host profile set.",
    path: "Wakeflow Gitignore body authority cannot encode a private resource path.",
...
export class WakeflowGitignoreBodyAuthorityError extends Error {
    name = "WakeflowGitignoreBodyAuthorityError";
    code = "wakeflow-gitignore-body-authority";
    reason;
    path;
...
function escapeLiteralPath(value) {
    return value.replace(GITIGNORE_MAGIC_PATTERN, (character) => (`\\${character}`));
}
function rootRule(resourcePath, directory) {
    const escaped = escapeLiteralPath(resourcePath);
    if (escaped.length === 0 || escaped.endsWith("\\"))
        fail("path", "$path");
    return `/${escaped}${directory ? "/" : ""}`;
}
/** 把一条根锚定的字面资源路径渲染为 Gitignore 规则；支撑面的 ignore 权威复用同一词法。 */
export function renderWakeflowGitignoreRootRule(resourcePath, directory) {
    return rootRule(resourcePath, directory);
}
/** 准入一个完整、无重复的宿主画像集合，按协议宿主顺序返回。 */
export function parseWakeflowGitignoreHostProfileSet(value) {
    return parseProfiles(value);
}
function isLiteralRootRule(value) {
    ...
    for (let index = 0; index < encodedPath.length; index += 1) {
        ...
        if (GITIGNORE_MAGIC_CHARACTERS.has(character))
            return false;
...
function coveredByPrivateRoot(resourcePath) {
...
function parseProfiles(value) {
    let values;
    try {
        values = parseDenseArray(value, WAKEFLOW_WORKSPACE_HOST_IDS.length, "$profiles");
    }
    catch (error) {
    ...
    for (const [index, value] of values.entries()) {
        ...
        if (byHostId.has(profile.hostId))
            fail("profile-set", "$profiles");
...
export function createWakeflowGitignoreBodyAuthority(profileValues) {
    ...
    for (const [index, profile] of profiles.entries()) {
        ...
        if (localPath === ACTIVE_ROOT_REF || localPath === LOCAL_ROOT_REF) {
            fail("path", `$/profiles/${index}/surfaces/settingsIntegration/localPath`);
        }
        ...
        for (const declaration of matrix.declarations) {
            ...
            if (declaration.placement.root.kind !== "workspace"
                || declaration.tracking.disposition !== "ignored"
                || relativePath === null
                || coveredByPrivateRoot(relativePath)) {
                continue;
            }
            rules.add(rootRule(relativePath, declaration.nodePolicy.kind !== "file"));
        }
    }
...
export function parseWakeflowGitignoreBodyAuthority(value) {
    ...
    let ruleValues;
    try {
        hostIdValues = parseDenseArray(record.hostIds, WAKEFLOW_WORKSPACE_HOST_IDS.length, "$authority.hostIds");
    ...
    if (hostIdValues.length !== WAKEFLOW_WORKSPACE_HOST_IDS.length
        || hostIdValues.some((hostId, index) => (hostId !== WAKEFLOW_WORKSPACE_HOST_IDS[index]))
        || ruleValues.length === 0
        || ruleValues.some((rule) => (typeof rule !== "string"
            || !rule.isWellFormed()
            || rule.normalize("NFC") !== rule
            || !isLiteralRootRule(rule)))) {
        fail("authority", "$authority");
    }
    const rules = Object.freeze(ruleValues);
    if (new Set(rules).size !== rules.length
        || rules.some((rule, index) => (index > 0 && lexicalCompare(rules[index - 1] ?? "", rule) >= 0))
        || record.body !== `${rules.join("\n")}\n`) {
        fail("authority", "$authority");
    }
    let bodyDigest;
    let suppliedAuthorityDigest;
    try {
        bodyDigest = parseSha256Digest(record.bodyDigest, "$authority.bodyDigest");
        suppliedAuthorityDigest = parseSha256Digest(record.authorityDigest, "$authority.authorityDigest");
    }
    catch (error) {
    ...
    const hostIds = Object.freeze(hostIdValues);
...
function outsideSegments(value) {
    let record;
    try {
 ...
 */
export function classifyWakeflowGitignoreExactOutsideRules(authorityValue, outsideValue) {
    ...
    const exactDuplicateRules = Object.freeze(authority.rules.filter((rule) => (lines.has(rule))));
    const exactNegatedRules = Object.freeze(authority.rules.filter((rule) => (lines.has(`!${rule}`))));
    return Object.freeze({
