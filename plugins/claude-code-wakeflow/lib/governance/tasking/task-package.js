...
import { LedgerAuthorityStoreError, parseLedgerAuthorityMemberReference, } from "../ledger/ledger-authority-store.js";
/**
 ...
 * 在追加业务事件前验证。
 */
const TASK_PACKAGE_ARTIFACT_KIND = "wakeflow-task-package";
...
export class TaskPackageError extends Error {
    name = "TaskPackageError";
    code = "wakeflow-task-package";
    reason;
    path;
...
const validateWire = createRuntimeJsonSchemaValidator(WAKEFLOW_TASK_PACKAGE_SCHEMA, [
...
const CONTENT_DRAFT_VALIDATION_DIGEST = parseSha256Digest(`sha256:${"0".repeat(64)}`, "$contentDraftValidationDigest");
const CONTENT_DRAFT_PROGRAM_ID = parseWakeflowDurableIdOfKind("program_00000000-0000-4000-8000-000000000000", "program", "$contentDraftProgramId");
const CONTENT_DRAFT_DEMAND_ID = parseWakeflowDurableIdOfKind("demand_00000000-0000-4000-8000-000000000001", "demand", "$contentDraftDemandId");
const CONTENT_DRAFT_TASK_PACKAGE_ID = parseWakeflowDurableIdOfKind("task-package_00000000-0000-4000-8000-000000000002", "task-package", "$contentDraftTaskPackageId");
const CONTENT_DRAFT_TARGET_TASK_ID = parseWakeflowDurableIdOfKind("target-task_00000000-0000-4000-8000-000000000003", "target-task", "$contentDraftTargetTaskId");
const AUTHORED_CONTENT_DRAFT_REFERENCE = parseLedgerAuthorityMemberReference({
    ...
    recordRef: "requirements/requirement_00000000-0000-4000-8000-000000000010/record.json",
    recordDigest: CONTENT_DRAFT_VALIDATION_DIGEST,
    memberPath: "requirement.md",
    memberRef: "requirements/requirement_00000000-0000-4000-8000-000000000010/requirement.md",
...
function parseTextList(values, path) {
    const parsed = [];
    const seen = new Set();
    for (let index = 0; index < values.length; index += 1) {
        const text = parseCanonicalText(values[index], `${path}/${index}`);
        if (seen.has(text))
...
function parseNonEmptyTextList(values, path) {
...
function parseSelectedAuthorityRefs(values) {
    const parsed = [];
    ...
    const first = parsed[0];
    if (first === undefined)
        fail("schema", "$/selectedAuthorityRefs");
    return Object.freeze([first, ...parsed.slice(1)]);
}
function parseAcceptanceAnchors(values) {
    const parsed = [];
    const anchorIds = new Set();
    for (let index = 0; index < values.length; index += 1) {
        const value = values[index];
        if (value === undefined)
            fail("schema", `$/acceptanceAnchors/${index}`);
...
function parseLineage(value, targetTaskId) {
    if (value === undefined || value === null)
        return null;
    if (value.kind === "replacement") {
        const replacesTargetTaskId = parseId(value.replacesTargetTaskId, "target-task", "$/lineage/replacesTargetTaskId");
        if (replacesTargetTaskId === targetTaskId)
            fail("relation", "$/lineage");
        return Object.freeze({ kind: "replacement", replacesTargetTaskId });
    }
    const continuesTargetTaskId = parseId(value.continuesTargetTaskId, "target-task", "$/lineage/continuesTargetTaskId");
    if (continuesTargetTaskId === targetTaskId)
        fail("relation", "$/lineage");
    return Object.freeze({ kind: "continuation", continuesTargetTaskId });
}
function parsePlanReview(value) {
    if (value === undefined)
        fail("schema", "$/planReview");
    if (value.reviewer === "controller") {
        return Object.freeze({ reviewer: "controller" });
    }
    let confirmedAt;
    try {
        ...
        if (error instanceof UtcInstantError)
            fail("time", "$/planReview/confirmedAt");
        throw error;
    }
    return Object.freeze({ reviewer: "user", confirmedAt });
}
function parseSectionAnchors(value) {
    if (value === undefined)
        fail("schema", "$/sectionAnchors");
    const seen = new Set();
    for (const [index, anchor] of value.entries()) {
        if (seen.has(anchor))
            fail("relation", `$/sectionAnchors/${index}`);
        seen.add(anchor);
    }
    return Object.freeze([...value]);
}
const STEP_ID_PREFIX = "ts-";
function parseRequirementRef(value, path) {
...
function parseTestContract(value) {
    if (value === undefined)
    ...
    for (const [index, step] of value.steps.entries()) {
        ...
        if (step.stepId !== `${STEP_ID_PREFIX}${index + 1}`)
            fail("relation", `${path}/stepId`);
        const requirementRef = parseRequirementRef(step.requirementRef, `${path}/requirementRef`);
        if (itemIds.has(requirementRef.itemId))
            fail("relation", `${path}/requirementRef/itemId`);
    ...
    if (firstStep === undefined)
        fail("schema", "$/testContract/steps");
    const stepTuple = Object.freeze([firstStep, ...steps.slice(1)]);
    let environment;
    try {
    ...
    for (const [index, skill] of value.allowedSkills.entries()) {
        if (skills.has(skill))
            fail("relation", `$/testContract/allowedSkills/${index}`);
        skills.add(skill);
    }
    return Object.freeze({
...
function parseImplementationBaselines(value) {
    if (value === undefined)
        fail("schema", "$/implementationBaselines");
    const parsed = [];
    ...
    for (const [index, baseline] of value.entries()) {
        ...
        parsed.push(Object.freeze({
            ...
            targetReviewDecisionId: parseId(baseline.targetReviewDecisionId, "target-review-decision", `${path}/targetReviewDecisionId`),
...
function parseTestLineage(value, targetTaskId) {
    if (value === undefined || value === null)
        return null;
    if (value.kind !== "retest")
        fail("schema", "$/lineage");
    const retestsTargetTaskId = parseId(value.retestsTargetTaskId, "target-task", "$/lineage/retestsTargetTaskId");
    if (retestsTargetTaskId === targetTaskId)
        fail("relation", "$/lineage");
    return Object.freeze({
        kind: "retest",
        retestsTargetTaskId,
        productDefectRemediationId: parseId(value.productDefectRemediationId, "product-defect-remediation", "$/lineage/productDefectRemediationId"),
        authorizationDigest: parseDigest(value.authorizationDigest, "$/lineage/authorizationDigest"),
    });
}
function normalizeWire(wire) {
    const confirmedContext = parseNonEmptyTextList(wire.confirmedContext, "$/confirmedContext");
    const inScope = parseNonEmptyTextList(wire.boundaries.inScope, "$/boundaries/inScope");
    const completionExpectations = parseNonEmptyTextList(wire.completionExpectations, "$/completionExpectations");
    const common = {
        ...
        taskPackageId: parseId(wire.taskPackageId, "task-package", "$/taskPackageId"),
        ...
        confirmedContext,
        selectedAuthorityRefs: parseSelectedAuthorityRefs(wire.selectedAuthorityRefs),
        boundaries: Object.freeze({
            inScope,
            outOfScope: parseTextList(wire.boundaries.outOfScope, "$/boundaries/outOfScope"),
            forbidden: parseTextList(wire.boundaries.forbidden, "$/boundaries/forbidden"),
        }),
        completionExpectations,
    };
    const windowId = parseId(wire.assignment.windowId, "window", "$/assignment/windowId");
    ...
    if (wire.workType === "implementation") {
        ...
        if (firstAnchor === undefined)
            fail("schema", "$/acceptanceAnchors");
        const parsedAnchors = Object.freeze([firstAnchor, ...acceptanceAnchors.slice(1)]);
        const implementation = {
            ...common,
            assignment: Object.freeze({
                repositoryId: parseId(wire.assignment.repositoryId, "repository", "$/assignment/repositoryId"),
                windowId,
            }),
            workType: "implementation",
            commitExpectation: wire.commitExpectation,
            acceptanceAnchors: parsedAnchors,
            lineage: parseLineage(wire.lineage !== undefined && wire.lineage !== null && wire.lineage.kind === "retest"
                ? fail("schema", "$/lineage")
                : wire.lineage, common.targetTaskId),
            planReview: parsePlanReview(wire.planReview),
            sectionAnchors: parseSectionAnchors(wire.sectionAnchors),
        };
        return Object.freeze(implementation);
    }
    if (wire.testContract === undefined ||
...
export function parseTaskPackage(value) {
    let json;
    try {
...
export function createTaskPackage(draft, options = {}) {
    let record;
    try {
    ...
    const expectedFields = record.workType === "test" ? TEST_DRAFT_FIELDS : DRAFT_FIELDS;
 ...
 */
export function parseTaskPackageContentDraft(value) {
    let record;
    try {
    ...
    const keys = Object.keys(record).sort();
    if (keys.length !== CONTENT_DRAFT_FIELDS.length ||
...
export function parseTaskPackageAuthoredContentDraft(value) {
    let record;
    try {
    ...
    const keys = Object.keys(record).sort();
    if (keys.length !== AUTHORED_CONTENT_DRAFT_FIELDS.length ||
...
export function renderTaskPackage(value) {
    return renderDeterministicJsonDocument(parseTaskPackage(value), "$taskPackage");
}
/** 只接受与领域确定性表示逐字节相同的 TaskPackage 文档。 */
export function parseTaskPackageDocument(text) {
    let json;
    try {
...
export function computeTaskPackageDigest(value) {
