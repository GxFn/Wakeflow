...
/**
 * wakeflow_create_demand 的请求：认领一个 pending 需求包即创建 Demand。preview 零写，apply 带 planDigest 重算同一计划，recover 带 operationId（即 demandId）向前恢复。podId 缺省为 primary pod；同一 pod 同一时刻只推进一个 Demand（ADR-0010 D3）。Demand 类型、测试决策、权威成员、时间与标识都由 Wakeflow 派生。
 */
...
/**
 * Target pod; omitted means the primary pod. The pod must be open (not closing) and must not have an active Demand.
 */
podId?: string
