...
import { planWholeFileContentTransition, WholeFileContentTransitionError, } from "../../foundation/filesystem/whole-file-content-transition.js";
...
import { createWakeflowSupportMemoryAuthority, WakeflowSupportMemoryAuthorityError, } from "./wakeflow-support-memory-authority.js";
import { WAKEFLOW_MANAGED_SUPPORT_ROOT_MODE, } from "./wakeflow-managed-support-root-materialization.js";
/**
 ...
 */
export const WAKEFLOW_SUPPORT_MEMORY_MAXIMUM_BYTES = parseByteCount(256 * 1024, "$supportMemory.maximumBytes");
...
const ERROR_MESSAGES = {
    ...
    "unadmitted-source": "Wakeflow support memory source is not an admitted whole-file render.",
...
export class WakeflowSupportMemoryInspectionError extends Error {
    name = "WakeflowSupportMemoryInspectionError";
    code = "wakeflow-support-memory-inspection";
    reason;
    path;
...
export function parseWakeflowSupportMemoryInspectionRequest(value) {
    let record;
    try {
    ...
    const catalog = createWakeflowManagedSupportResourceCatalog(desiredConfig, profile);
    const expectedCatalogDigest = parseDigest(record.expectedCatalogDigest, "$request.expectedCatalogDigest");
    if (catalog.catalogDigest !== expectedCatalogDigest) {
        fail("catalog", "$request.expectedCatalogDigest");
    }
    if (!catalog.declarations.some((entry) => (entry.declarationId === `support.${surfaceId}.root`))) {
...
async function admitRoots(workspaceRoot, supportRoot, request) {
    let report;
    try {
        report = await validateWakeflowConfigRootPlacements(workspaceRoot, request.desiredConfig);
    }
    catch (error) {
...
function currentAuthority(request) {
    if (request.currentConfig === null)
        return null;
    try {
        return createWakeflowSupportMemoryAuthority(request.currentConfig, request.profile, request.surfaceId);
    }
    catch (error) {
...
function desiredAuthority(request) {
    try {
        return createWakeflowSupportMemoryAuthority(request.desiredConfig, request.profile, request.surfaceId);
    }
    catch (error) {
        if (error instanceof WakeflowSupportMemoryAuthorityError) {
...
async function readSource(supportRoot, request) {
    try {
        const read = await readStableFile(supportRoot, request.profile.instructionFileName, {
...
async function revalidateSource(supportRoot, request, initial) {
    try {
        const current = await readStableFile(supportRoot, request.profile.instructionFileName, {
            maximumBytes: WAKEFLOW_SUPPORT_MEMORY_MAXIMUM_BYTES,
...
export async function inspectWakeflowSupportMemory(workspaceRootValue, supportRootValue, requestValue) {
    ...
    const declaration = request.catalog.declarations.find((entry) => (entry.declarationId
        === `support.${request.surfaceId}.instruction.${request.profile.hostId}`));
    if (declaration === undefined)
        fail("surface", "$request.surfaceId");
    let operation;
    try {
        operation = admitWakeflowResourceOperation(declaration.processing, "deterministic-rewrite");
    }
    catch (error) {
    ...
    let transition;
    try {
        transition = planWholeFileContentTransition(source?.bytes ?? null, {
    ...
    if (transition.desiredByteCount > WAKEFLOW_SUPPORT_MEMORY_MAXIMUM_BYTES
