...
test("repositories 与 supportSurfaces 的 placement 至多一个前导 ..：拒绝一律由 Schema 触发，解析器的同名收紧够不到；一级接受；ledgerRoot 不受此限（§13.97 D2e）", () => {
  for (const [collection, placement] of [
    ...
    // `parseWakeflowConfigV3` 先跑 Schema 再跑 placement 准入，所以实际触发的永远是 "schema"：
    // 解析器里同一条上限是纵深防御，不是第二条可达路径（下面用 Schema 的语法本身证明够不到）。
    expectConfigError(() => parseWakeflowConfigV3(value), "schema");
  }
  const pattern = (
    WAKEFLOW_CONFIG_V3_SCHEMA as unknown as {
  ...
  const oneLevel = createMinimalWakeflowConfigV3();
  topology(oneLevel).supportSurfaces[0]!.path = "../Design";
  equal(parseWakeflowConfigV3(oneLevel).topology.supportSurfaces[0]?.path, "../Design");

  // 只有拓扑放置收紧：账本根仍可以是更深的相对位置。
  const ledger = createMinimalWakeflowConfigV3();
  (ledger.storage as Record<string, unknown>).ledgerRoot = "../../shared/wakeflow-ledger";
  equal(parseWakeflowConfigV3(ledger).storage.ledgerRoot, "../../shared/wakeflow-ledger");
});
