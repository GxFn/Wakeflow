...
Wakeflow Delivery Loop is a delivery layer wrapped around normal total
...
long `rules` line. Those fields stay in `wakeflow-state.json`,
...
1. **Orient**
   - Read workspace `AGENTS.md`, workspace index/status, the current
     state-root `wakeflow-state.json` / `developer-progress.md`, and this
...
```text
node scripts/wakeflow-delivery.mjs review-pack --state-root <stateRoot> --json
```
...
```text
node scripts/wakeflow-delivery.mjs review-pack --group <dispatchGroup> --json
```
...
```text
node scripts/wakeflow-delivery.mjs review-results --group <dispatchGroup> --json
```
...
```text
node scripts/wakeflow-state.mjs add-task-package --state-root <stateRoot> --task-package-id <taskPackageId> --summary "<summary>" --source-ref "<sourceRef>" --target-window <window> --target-task-id <taskId> --write --json
```

   - Append the human-readable task package entry only as a timestamped log:

```text
node scripts/wakeflow-progress-log.mjs --state-root <stateRoot> --type task-package --task-package-id <taskPackageId> --summary "<summary>" --source-ref "<sourceRef>" --write --json
```
...
```text
node scripts/wakeflow-render-progress.mjs --state-root <stateRoot> --write --json
```
   - Ensure each target window has a local thread registration. Register a real
     thread id only in local runtime:

```text
node scripts/wakeflow-delivery.mjs register-thread --window <window> --thread-id <realThreadId> --role target --responsibility-root <repo-or-workspace-path> --write --json
```
...
```text
node scripts/wakeflow-delivery.mjs build-window-config --window <window> --require-thread --write --json
```
...
```text
node scripts/wakeflow-delivery.mjs prepare-dispatch-from-state --state-root <stateRoot> --target-task-id <taskId> --group <dispatchGroup> --controller-window <currentControllerWindow> --return-policy group-ready --human-context-ref <stateRoot>/developer-progress.md --require-thread --write --json
```
...
```text
node scripts/wakeflow-delivery.mjs start-keep-live --automation-run-id <dispatchGroup-or-runId> --write --json
```
...
```text
node scripts/wakeflow-delivery.mjs record-delivery-run --delivery-file <deliveryFile> --status sent --readback-ok true --evidence "<host send/readback evidence>" --write --json
```
...
4. **Stop**
   ...
   - After a demand is completed, archived, or switched to an idle current
     active plan, scan formal Design handoffs and TODO candidates before
...
```text
node scripts/wakeflow-delivery.mjs stop-loop --automation-run-id <dispatchGroup-or-runId> --reason "<reason>" --write --json
```
