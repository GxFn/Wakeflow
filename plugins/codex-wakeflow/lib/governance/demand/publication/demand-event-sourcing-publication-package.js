...
import { claimRequirementPackage, computeRequirementClaimStateDigest, readRequirementClaimState, refreshRequirementBoardIndex, replaceRequirementClaimStateFile, } from "../../../kernel/requirement-board.js";
import { failDemandEventSourcingPublication as fail, } from "./demand-event-sourcing-publication-contract.js";
/**
 ...
 * 分类不变。
 */
function mapBoardError(error, path) {
...
export async function inspectPackageForDemandPublication(root, transaction, signal) {
    try {
...
function sameLineage(source, transaction) {
...
export function exactClaimedPackage(source, transaction) {
    if (source === null
...
export function assertPendingPackage(source, transaction) {
    if (source === null)
        fail("package-not-found", "$board");
    if (!sameLineage(source, transaction)
...
export async function claimPackageForDemandPublication(root, transaction, signal) {
    ...
    if (claimed === null) {
        ...
        try {
            const next = claimRequirementPackage(pending.state, transaction.demandId, transaction.initialCommand.recordedAt);
            await replaceRequirementClaimStateFile(root, { digest: pending.digest, read: pending.read }, next, signal);
