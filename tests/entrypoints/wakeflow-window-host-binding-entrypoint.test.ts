...
} from "../../src/entrypoints/codex-wakeflow-mcp.js";
import {
...
async function fixture(t: TestContext): Promise<string> {
  const root = realpathSync(mkdtempSync(path.join(
    os.tmpdir(),
    "wakeflow-window-binding-public-",
  )));
...
test("Maintenance launch intent 经 Agent result 注册为私有 Binding 与脱敏投影", {
  ...
  equal(concurrentResults.every((result) => result.isError === undefined), true);
  const concurrentValues = concurrentResults.map((result) => (
  ...
  equal(
    concurrentValues.map((value) => value.disposition).sort().join(","),
    "registered,replayed",
  );
  equal(
