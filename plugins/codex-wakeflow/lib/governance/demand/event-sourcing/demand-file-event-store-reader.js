...
import { computeDemandEventStreamCommitDigest, parseDemandEventStreamCommitDocument, DemandEventStreamCommitError, renderDemandEventStreamCommit, } from "./demand-event-stream-commit.js";
...
import { demandEventStreamCommitRef, parseDemandEventStreamCommitFileName, DemandEventSourcingPathError, DEMAND_EVENT_STREAM_COMMITS_ROOT_REF, } from "./demand-event-sourcing-paths.js";
import { DEMAND_FILE_EVENT_STORE_MAXIMUM_COMMIT_BYTES, DEMAND_FILE_EVENT_STORE_MAXIMUM_COMMITS, DEMAND_FILE_EVENT_STORE_MAXIMUM_TOTAL_BYTES, DemandFileEventStoreError, assertDemandFileEventStoreDirectory, assertDemandFileEventStoreFile, failDemandFileEventStore as fail, } from "./demand-file-event-store-contract.js";
/** Demand 文件事件存储的稳定提交清单与读取边界。 */
const COMMIT_READ_CONCURRENCY = 8;
function assertDirectoryPolicy(node, path, policy) {
    if (policy === "private") {
...
function assertFilePolicy(node, path, policy, admittedLinkCounts = [1n]) {
    if (policy === "private") {
...
export async function readDemandFileEventDirectory(root, ref, maximumEntries, signal, expectedNode, policy = "private") {
    try {
...
export async function readDemandFileEventCommit(root, resourcePath, expectedNode, signal, path, admittedLinkCounts = [1n], policy = "private") {
...
function assertCommitChain(commits) {
    let previous = null;
    const commitIds = new Set();
    const eventIds = new Set();
    for (const [index, commit] of commits.entries()) {
        if (commit.commitSequence !== index + 1
            || commitIds.has(commit.commitId)
            || (previous === null && (commit.expectedStreamRevision !== 0
                || commit.previousCommitDigest !== null))
            || (previous !== null && (commit.demandId !== previous.demandId
...
function demandFileEventCursorFrom(commit) {
    if (commit === undefined)
        return null;
    return Object.freeze({
...
function parseCursor(value) {
    ...
    try {
        lastCommitDigest = parseSha256Digest(record.lastCommitDigest, "$/lastCommitDigest");
    }
    catch (error) {
        if (error instanceof Sha256Error)
            fail("input", "$/lastCommitDigest");
        throw error;
    }
    try {
        return Object.freeze({
...
async function readCommitEntries(root, entries, indexOffset, signal, policy = "private") {
    const limit = pLimit(COMMIT_READ_CONCURRENCY);
    const settled = await Promise.allSettled(entries.map((entry, index) => (limit(() => readDemandFileEventCommit(root, entry.resourcePath, entry.node, signal, `$commits/${indexOffset + index}`, [1n], policy)))));
    const loaded = [];
...
function assertInventoryNames(read, policy = "private") {
    let totalBytes = 0;
...
export async function readAllDemandFileEventCommits(root, signal, policy = "private") {
    const before = await readDemandFileEventDirectory(root, DEMAND_EVENT_STREAM_COMMITS_ROOT_REF, DEMAND_FILE_EVENT_STORE_MAXIMUM_COMMITS, signal, undefined, policy);
    assertInventoryNames(before, policy);
    const loaded = await readCommitEntries(root, before.entries, 0, signal, policy);
    const after = await readDemandFileEventDirectory(root, DEMAND_EVENT_STREAM_COMMITS_ROOT_REF, DEMAND_FILE_EVENT_STORE_MAXIMUM_COMMITS, signal, before.directoryNode, policy);
    if (!sameDirectoryRead(before, after))
        fail("stream-changed", "$commits");
...
export async function readDemandFileEventCommitsAfter(root, cursorValue, signal) {
    const cursor = parseCursor(cursorValue);
    // 尾部读取按序号逐文件探测，不枚举整个提交目录：成本只与尾部长度有关。
    const anchorLoaded = await readDemandFileEventCommitOrNull(root, demandEventStreamCommitRef(cursor.commitSequence), signal);
    ...
    const commits = [];
    let previous = anchor;
    for (let sequence = cursor.commitSequence + 1; sequence <= DEMAND_FILE_EVENT_STORE_MAXIMUM_COMMITS; sequence += 1) {
        const loaded = await readDemandFileEventCommitOrNull(root, demandEventStreamCommitRef(parseDemandEventCommitSequence(sequence, "$tail")), signal);
        if (loaded === null)
            break;
        const commit = loaded.commit;
        if (commit.commitSequence !== sequence
...
export async function readDemandFileEventCommitOrNull(root, ref, signal, admittedLinkCounts = [1n]) {
    let resource;
    try {
    ...
    return readDemandFileEventCommit(root, ref, resource.node, signal, "$commit", admittedLinkCounts);
}
export async function readDemandFileEventCommitAt(root, sequenceValue, signal) {
    let sequence;
    try {
    ...
    const loaded = await readDemandFileEventCommitOrNull(root, demandEventStreamCommitRef(sequence), signal);
    if (loaded === null)
        return null;
...
export function assertDemandFileEventAppendAdmissionAgainstPrefix(prefixCommits, prepared) {
    ...
    const currentBytes = prefix.commits.reduce((total, current) => total + encodeUtf8(renderDemandEventStreamCommit(current)).byteLength, 0);
    const commitBytes = encodeUtf8(renderDemandEventStreamCommit(commit)).byteLength;
    if (prefix.commits.length >= DEMAND_FILE_EVENT_STORE_MAXIMUM_COMMITS
    ...
    if (commit.commitSequence === 1) {
        ...
        if (previousLastEvent === undefined)
            fail("stream-invalid", "$commits/last");
    ...
    if (commit.idempotency !== undefined
        && prefix.commits.some((current) => current.idempotency?.key === commit.idempotency?.key)) {
    ...
    for (const [index, event] of commit.events.entries()) {
        if (eventIds.has(event.eventId)) {
            fail("append-identity-conflict", `$commit/events/${index}/eventId`);
        }
    }
}
