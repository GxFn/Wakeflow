 ...
 */
function deriveAnchorEvidenceBlockers(view, anchorEvidence) {
    if (view.managedEvidenceIds.length === 0)
        return ["anchor-evidence:no-managed-evidence"];
    if (anchorEvidence === undefined)
        return [];
    if (anchorEvidence === null || anchorEvidence.length === 0)
        return ["anchor-evidence:missing"];
    const blockers = [];
    const claimed = new Set(anchorEvidence.map((entry) => entry.anchorId));
    for (const anchorId of view.acceptanceAnchorIds) {
        if (!claimed.has(anchorId))
            blockers.push(`anchor-evidence:uncovered:${anchorId}`);
    }
    ...
    for (const entry of anchorEvidence) {
        ...
        for (const evidenceId of entry.evidenceIds) {
            if (!known.has(evidenceId))
                blockers.push(`anchor-evidence:unknown-evidence:${evidenceId}`);
        }
    }
    return blockers;
}
 ...
 */
export function deriveImplementationDecisionBlockers(decision, view, independentChecks, anchorEvidence) {
    const blockers = [];
    if (decision === "accept") {
