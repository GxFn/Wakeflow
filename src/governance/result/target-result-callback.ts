...
export interface TargetResultCallbackRecord {
...
export interface TargetResultCallbackReissue {
...
/** 导入时对一条证据定位符的解析收据：定位符指向哪份受管证据、多少字节（§13.87 D3）。 */
export interface TargetResultEvidenceResolution {
...
export type TargetResultCallbackErrorReason =
  | "input"
  | "identifier"
  | "digest"
  | "path"
...
} as const satisfies Readonly<Record<TargetResultCallbackErrorReason, string>>;

export class TargetResultCallbackError extends Error {
  override readonly name = "TargetResultCallbackError";
  readonly code = "wakeflow-target-result-callback" as const;
  readonly reason: TargetResultCallbackErrorReason;
  readonly path: string;

  constructor(reason: TargetResultCallbackErrorReason, path: string) {
...
const RESOLUTION_FIELDS = Object.freeze(["bytes", "digest", "evidenceId", "ref"] as const);
const MAXIMUM_EVIDENCE_RESOLUTIONS = 64;

function fail(reason: TargetResultCallbackErrorReason, path: string): never {
...
function id<Kind extends "target-delivery" | "target-result" | "window" | "evidence">(
  value: unknown,
  kind: Kind,
  path: string,
...
function bindingId(value: unknown, path: string): WakeflowWindowHostBindingId {
  try {
    ...
    if (error instanceof WakeflowWindowHostBindingIdError) fail("identifier", path);
    throw error;
  }
}

function generation(value: unknown, minimum: number, path: string): number {
  if (
    typeof value !== "number" ||
...
export function deriveTargetResultCallbackId(
  targetResultId: WakeflowDurableId<"target-result">,
): WakeflowDurableId<"target-delivery"> {
  return deriveDurableId("target-delivery", "target-result-callback", targetResultId);
}

export function parseTargetResultCallbackRecord(
  value: unknown,
  path = "$callback",
...
export function parseTargetResultCallbackReissue(
  value: unknown,
  path = "$reissue",
): Readonly<TargetResultCallbackReissue> {
  ...
  if (nextGeneration !== previousGeneration + 1) fail("relation", `${path}/generation`);
  return Object.freeze({
...
export function parseTargetResultEvidenceResolutions(
  value: unknown,
  path = "$evidenceResolution",
): readonly Readonly<TargetResultEvidenceResolution>[] {
  if (!Array.isArray(value) || value.length > MAXIMUM_EVIDENCE_RESOLUTIONS) fail("input", path);
  ...
  const resolutions = value.map((entry, index) => {
    ...
    try {
      ...
      if (error instanceof PortableResourcePathError) fail("path", `${entryPath}/ref`);
      throw error;
    }
    if (refs.has(ref)) fail("relation", `${entryPath}/ref`);
    refs.add(ref);
    if (
...
export type TargetResultCallbackStatus = "pending" | "landed" | "silent" | "acknowledged";

export interface CallbackLandingRecordView {
  readonly recordId: string;
  readonly promptDigest: Sha256Digest | null;
  readonly recordedAt: UtcInstant;
}

export interface CallbackStatusInput {
  readonly issuedAt: UtcInstant;
  readonly promptDigest: Sha256Digest;
  /** Controller 会话在 `issuedAt` 之后的 `user-prompt-submit` 记录。 */
  readonly landingRecords: readonly Readonly<CallbackLandingRecordView>[];
  readonly acknowledged: boolean;
  readonly now: UtcInstant;
  readonly silenceMilliseconds: number;
}

export type CallbackStatusView = Readonly<{
  readonly status: TargetResultCallbackStatus;
  readonly landedRecordId: string | null;
  readonly landedAt: UtcInstant | null;
}>;

/** 回调状态由读侧惰性派生：决定已记录即 acknowledged，否则按落地记录与静默阈值判断。 */
export function deriveTargetResultCallbackStatus(
  input: Readonly<CallbackStatusInput>,
): CallbackStatusView {
  const landed = input.landingRecords.find(
    (record) =>
      record.promptDigest === input.promptDigest &&
  ...
  return Object.freeze({
    status: silent ? ("silent" as const) : ("pending" as const),
    landedRecordId: null,
    landedAt: null,
  });
}
