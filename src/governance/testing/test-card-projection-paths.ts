...
export type TestCardProjectionPathErrorReason = "identifier" | "file-name";

const ERROR_MESSAGES = {
  identifier: "Test dispatch projection identity is invalid.",
  "file-name": "Test dispatch projection filename is invalid.",
} as const satisfies Readonly<
  Record<TestCardProjectionPathErrorReason, string>
>;

/** Test目标读取投影路径无法形成唯一词法映射时的稳定错误。 */
export class TestCardProjectionPathError extends Error {
  override readonly name = "TestCardProjectionPathError";
  readonly code = "wakeflow-test-card-projection-path" as const;
  readonly reason: TestCardProjectionPathErrorReason;
  readonly path: string;

  constructor(reason: TestCardProjectionPathErrorReason, path: string) {
...
function fail(
  reason: TestCardProjectionPathErrorReason,
  path: string,
): never {
  throw new TestCardProjectionPathError(reason, path);
}

function id<Kind extends "test-card">(
  value: unknown,
  kind: Kind,
  path: string,
