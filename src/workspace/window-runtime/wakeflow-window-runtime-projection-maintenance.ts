...
export async function planWakeflowWindowRuntimeProjectionMaintenance(
  ...
  let expected = await resolveWakeflowWindowRuntimeProjectionExpectedEntries(
    rootValue,
    inputs,
    request.signal,
  );
  ...
  if (runtimeMissing) {
    expected = await resolveWakeflowWindowRuntimeProjectionExpectedEntries(
      rootValue,
      inputs,
      request.signal,
      { projectionRootRequired: false },
    );
  }
  ...
  for (const item of inspected) {
    if (runtimeMissing && !item.entry.registered) continue;
...
export interface WakeflowWindowRuntimeSkeletonReceipt {
  readonly created: boolean;
  readonly directoriesCreated: number;
  readonly projectionsPublished: number;
}
 ...
 */
export async function ensureWakeflowWindowRuntimeSkeleton(
  rootValue: RootedDirectory,
  request: EnsureWakeflowWindowRuntimeSkeletonRequest,
): Promise<Readonly<WakeflowWindowRuntimeSkeletonReceipt>> {
  if (request.signal?.aborted === true) failWindowRuntimeProjection("aborted", "$signal");
  ...
  for (const resourceRef of [
    ...
    try {
      ...
      if (error instanceof DurableDirectoryMaterializationError) {
        failWindowRuntimeProjection(
          error.reason === "aborted" ? "aborted" : "effect",
          `$skeleton/${resourceRef}`,
        );
      }
      throw error;
    }
    if (!present) directoriesCreated += 1;
  }
  ...
  for (const entry of unregistered.entries) {
    ...
    try {
      const inspection = await inspectWakeflowWindowRuntimeProjectionDocument(
        rootValue,
        target,
        signal,
      );
...
export interface RetireWakeflowWindowRuntimeProjectionsRequest {
  readonly resourceProfile: unknown;
  readonly windowIds: readonly unknown[];
  readonly signal?: AbortSignal;
}

export interface WakeflowWindowRuntimeProjectionRetirementReceipt {
  /** 精确删除的投影文件数。 */
 ...
 */
export async function retireWakeflowWindowRuntimeProjections(
  rootValue: RootedDirectory,
  request: RetireWakeflowWindowRuntimeProjectionsRequest,
): Promise<Readonly<WakeflowWindowRuntimeProjectionRetirementReceipt>> {
  if (request.signal?.aborted === true) failWindowRuntimeProjection("aborted", "$signal");
  let resourceProfile;
  try {
    resourceProfile = parseWakeflowWorkspaceHostResourceProfile(request.resourceProfile);
