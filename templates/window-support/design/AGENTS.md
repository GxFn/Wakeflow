...
本目录是 Wakeflow 内置的需求设计窗口入口。若用户配置了外部 Design 仓库，外部仓库自己的 `AGENTS.md` 和本 workspace 写入的 scope 管理块优先生效；本文件用于没有外部需求设计仓库时承接 Design 能力。
...
如果无法读取总控文档，只能进入 `detached-design-mode`：可以继续写草案，但必须标注“未接入总控当前状态，等待 Wakeflow 导入复核”。
...
- 保存用户决策、假设、开放问题和交接说明。
- 为 Wakeflow 准备随时可接收的 signal / handoff 草案；最终 TODO 入账、阶段确认、task package 派发、测试协调、验收、归档和提交仍归 Wakeflow。

## 不可变边界

- 不修改任何产品源码仓库。
- 不运行产品构建、冷启动、真实项目测试、包刷新、发布命令或部署命令。
- 不直接向产品窗口、Test 或其它执行窗口派发实现任务。
- 不修改 Wakeflow 当前状态、TODO 列表、state-root、测试 card 或测试交流投影；Design 只写设计草案、signal、handoff 或 handoff board。
