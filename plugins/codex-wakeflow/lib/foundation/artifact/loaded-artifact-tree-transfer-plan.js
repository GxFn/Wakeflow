...
import { planDirectoryTreeCandidateFromFileDescriptors, parseDirectoryTreeCandidatePlan, DurableDirectoryTreeCandidateError, } from "../filesystem/durable-directory-tree-candidate.js";
...
import { LOADED_ARTIFACT_TREE_IDENTITY_LIMITS, validateLoadedArtifactTreeManifest, LoadedArtifactTreeIdentityError, } from "./loaded-artifact-tree-identity.js";
/**
 ...
 */
export const LOADED_ARTIFACT_TREE_TRANSFER_PLAN_KIND = "wakeflow-loaded-artifact-tree-transfer-plan";
export const LOADED_ARTIFACT_TREE_TRANSFER_PLAN_SCHEMA_VERSION = 1;
...
export class LoadedArtifactTreeTransferPlanError extends Error {
    name = "LoadedArtifactTreeTransferPlanError";
    code = "wakeflow-loaded-artifact-tree-transfer-plan";
    reason;
    path;
...
function parseModes(value) {
    ...
    const modes = Object.freeze({
        ...
        executableFileMode: parseMode(record.executableFileMode, "$options.executableFileMode"),
        regularFileMode: parseMode(record.regularFileMode, "$options.regularFileMode"),
    });
    if ((modes.directoryMode & 0o700) !== 0o700
...
function parseCandidateRootPath(value) {
    try {
...
function planFiles(manifest, modes) {
...
function createDirectoryPlan(files, modes) {
    try {
...
function planDigestBasis(artifactDigest, candidateRootPath, directoryPlan, copies) {
...
function parseDirectoryPlan(value) {
    try {
        ...
        if (error instanceof DurableDirectoryTreeCandidateError) {
            if (error.reason === "capacity")
                fail("capacity", "$plan.directoryPlan");
            fail("plan", "$plan.directoryPlan");
        }
        throw error;
    }
}
function parseCopyPath(value, path) {
    try {
...
function parseCopies(value, maximumLength) {
    let values;
    try {
    ...
    return Object.freeze(values.map((entry, index) => {
        ...
        return Object.freeze({
            sourceResourcePath: parseCopyPath(record.sourceResourcePath, `$plan.copies.${index}.sourceResourcePath`),
            candidateResourcePath: parseCopyPath(record.candidateResourcePath, `$plan.copies.${index}.candidateResourcePath`),
        });
    }));
}
/** 严格重验 transfer plan 的 manifest、目录计划、copy 映射和完整摘要。 */
export function parseLoadedArtifactTreeTransferPlan(value) {
    let record;
    try {
    ...
    const artifactDigest = parseDigest(record.artifactDigest, "$plan.artifactDigest");
    const planDigest = parseDigest(record.planDigest, "$plan.planDigest");
    if (copies.length !== manifest.files.length
        || directoryPlan.files.length !== manifest.files.length
        || directoryPlan.totalBytes !== manifest.totalBytes
        || artifactDigest !== computeCanonicalJsonSha256Digest(manifest, "$plan.manifest")) {
        fail("plan", "$plan");
    }
    for (const [index, manifestFile] of manifest.files.entries()) {
        ...
        if (plannedFile === undefined
            ...
            || copy.candidateResourcePath !== joinDirectoryTreeCandidatePath(candidateRootPath, manifestFile.ref)) {
            fail("plan", `$plan.copies.${index}`);
        }
    }
    const basis = planDigestBasis(artifactDigest, candidateRootPath, directoryPlan, copies);
 ...
 */
export function planLoadedArtifactTreeTransfer(manifestValue, candidateRootPathValue, optionsValue) {
    ...
    const copies = Object.freeze(manifest.files.map((file) => Object.freeze({
        ...
        candidateResourcePath: joinDirectoryTreeCandidatePath(candidateRootPath, file.ref),
    })));
    const artifactDigest = computeCanonicalJsonSha256Digest(manifest, "$manifest");
    const basis = planDigestBasis(artifactDigest, candidateRootPath, directoryPlan, copies);
