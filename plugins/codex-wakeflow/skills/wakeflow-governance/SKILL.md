...
- **Multiple active demands, one controller**: up to `maxActiveDemands` (default 2) demands run side by side; parallelism exists ONLY at the demand level — never inside a demand (each repo = one window = one combined task package). One controller is the single acceptance authority across all of them; each turn declares its demand and re-reads that state root; claiming past capacity fails closed.
