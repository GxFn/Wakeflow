...
> 上位需求：[TypeScript 单一源码、双宿主制品与轻量测试需求](../requirements/typescript-dual-artifact-build.md)
> 当前实施计划：[TypeScript 全新项目能力重构开发计划](../plan/typescript-reimplementation-plan.md)
> 资源标准：[TypeScript 资源处理归一标准与收敛矩阵](../standards/resource-handling-standard.md)
> 最新旧产品全闭包审查：[旧 JavaScript 产品全场景闭包审查](../references/legacy-js-scenario-closure-audit.md)
...
| [TypeScript 需求](../requirements/typescript-dual-artifact-build.md) | TS 单一手写源码、双生成制品、Schema 单向生成、轻量测试 | 新项目最终验收权威 |
| [TypeScript 开发计划](../plan/typescript-reimplementation-plan.md) | E0～E4 阶段边界和当前进度 | 保留阶段门；后续切片顺序允许按本轮事实修订 |
| [资源处理标准](../standards/resource-handling-standard.md) | 资源 role、mutation recipe、RH-1/RH-2 和候选 RH-3～RH-6 | 新物理格式与机制收敛方向；早期后续编号不是绝对命令 |
