...
/**
 * wakeflow_pod 的请求（ADR-0010 D6）：preview 零写推导创建或关闭计划，apply 带 planDigest 重算同一计划后执行一次配置事务，recover 按 podId 对账回执。intent.kind=create 创建一个 worktree pod（窗口集与 worktree 意图由 Wakeflow 派生）；intent.kind=close 两段关闭：第一段记下分支处置并进入 closing，第二段在全部窗口退役且检出已处置后从配置删除 pod。只有 main 的 Controller 调用；调用方身份不在线上。
 */
export type WakeflowPodRequestV1 = (EffectRequest | RecoverRequest)
/**
...
/**
 * Short host-safe pod name; unique among live pods and never `main`. The worktree name is wakeflow-<name>.
 */
export type PodName = string
/**
 * Client key the podId derives from; the same key replays the same pod as already-created.
 */
...
intent: (CreateIntent | CloseIntent)
}
...
podId: PodId
/**
 * One disposition per registered worktree; required on the first close phase, ignored once the pod is closing.
 *
 * @maxItems 64
 */
branches: BranchDisposition[]
}
export interface BranchDisposition {
repositoryId: RepositoryId
/**
 * How the developer disposed of this worktree branch; merging happens outside Wakeflow (ADR-0010 D5).
 */
