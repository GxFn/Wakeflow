...
import { readStableRootResourceTree, StableResourceTreeReadError, } from "../filesystem/stable-resource-tree-read.js";
/**
 ...
 */
export const LOADED_ARTIFACT_TREE_MANIFEST_VERSION = 1;
export const LOADED_ARTIFACT_TREE_MANIFEST_KIND = "wakeflow-loaded-artifact-tree";
...
export class LoadedArtifactTreeIdentityError extends Error {
    name = "LoadedArtifactTreeIdentityError";
    code = "wakeflow-loaded-artifact-tree-identity";
    reason;
    path;
...
function assertExactFields(record, fields, path, reason) {
...
function parseNonNegativeInteger(value, path, reason) {
...
function parseLimits(value) {
    if (value === undefined)
        return LOADED_ARTIFACT_TREE_IDENTITY_LIMITS;
    let record;
    try {
...
function parseIdentityOptions(value) {
    let record;
    try {
...
function parseManifestByteCount(value, path) {
    try {
        return parseByteCount(value, path);
    }
    catch (error) {
        if (error instanceof ByteCountError)
            fail("manifest-shape", path);
        throw error;
    }
}
function parseManifestRef(value, path) {
    try {
...
function parseManifestDigest(value, path) {
    try {
...
function portablePathDepth(ref) {
    let depth = 1;
    for (let index = 0; index < ref.length; index += 1) {
        if (ref.charCodeAt(index) === 0x2f)
            depth += 1;
    }
    return depth;
}
function parseManifestFile(value, index, limits) {
...
function assertManifestTopology(files, limits) {
    const fileRefs = new Set(files.map((file) => file.ref));
    const resources = new Map();
    const admit = (ref, kind, path) => {
        ...
        if (existing === undefined) {
            ...
            if (resources.size > limits.maxEntries)
                fail("entry-limit", path);
        }
    };
...
function parseManifest(value, limits) {
    let record;
    try {
    ...
    if (record.artifactKind !== LOADED_ARTIFACT_TREE_MANIFEST_KIND
    ...
    if (values.length === 0)
        fail("empty-tree", "$manifest.files");
    const files = [];
    let previousRef;
    ...
    for (const [index, value] of values.entries()) {
        ...
        try {
            ...
            if (error instanceof ByteCountError)
                fail("manifest-totals", "$manifest.totalBytes");
            throw error;
        }
    ...
    const fileCount = parseNonNegativeInteger(record.fileCount, "$manifest.fileCount", "manifest-shape");
    const declaredTotalBytes = parseNonNegativeInteger(record.totalBytes, "$manifest.totalBytes", "manifest-shape");
...
export function validateLoadedArtifactTreeManifest(value, options) {
...
function mapStableTreeReadError(error) {
    if (error.reason === "entry-limit")
        fail("entry-limit", "$tree");
    if (error.reason === "depth-limit")
        fail("depth-limit", "$tree");
    if (error.reason === "file-count")
        fail("file-count", "$tree.files");
    if (error.reason === "file-bytes")
        fail("file-bytes", "$tree.files");
    if (error.reason === "total-bytes")
        fail("total-bytes", "$tree.totalBytes");
...
async function readArtifactTree(root, options) {
    try {
        return await readStableRootResourceTree(root, {
            ...
            maximumFileBytes: parseByteCount(options.limits.maxFileBytes, "$limits.maxFileBytes"),
            maximumTotalBytes: parseByteCount(options.limits.maxTotalBytes, "$limits.maxTotalBytes"),
...
async function inspectArtifactManifest(root, options) {
    ...
    for (const entry of tree.entries) {
        ...
        if (entry.node.kind === "directory")
            continue;
        if (entry.node.kind === "symbolic-link")
            fail("symlink", "$tree.entries");
        if (entry.node.kind !== "file")
            fail("special-node", "$tree.entries");
    }
    const files = tree.files.map((file) => Object.freeze({
...
function manifestJsonValue(manifest) {
    try {
        return parseJsonValue(manifest, "$manifest");
    }
    catch (error) {
        if (error instanceof JsonValueError)
            fail("manifest-shape", error.path);
        throw error;
    }
}
export async function inspectLoadedArtifactTree(root, options) {
    assertRoot(root);
