...
| [0010](./0010-worktree-isolated-execution-and-converged-flow.md) | Pod 隔离执行与流程收敛：main 是 `primary` 的 pod、一 pod 一时一 Demand、worktree 归产品窗口并由宿主原生能力创建、合并在 Wakeflow 之外、只有 main 开关 pod、一条直线的流程 | `accepted`，首版任务粒度的 D1 到 D3 被用户的 pod 模型取代 | 2026-09-04 | 配置 `pods[]`、Demand 身份 `podId`、窗口绑定、投递目标、结果 schema、状态投影、TSD-15、L1 切片 |
