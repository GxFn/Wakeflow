  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        craftEvidence: { type: "array", items: { type: "object" }, description: "Typed execution-craft evidence for the package's evidence contract: [{ kind, ref|value|commit, verify }]. A completed result must cover the contract's required kinds. Optional." },
      },
    },
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        sourceRef: { type: "string" },
        strategySource: { type: "string", description: "Where the test approach came from (the Design-stage testing decision / test-strategy). Optional; when absent the card is flagged as an approach not decided at Design, surfaced as a reminder (never a gate)." },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        demandKey: { type: "string", description: "<topic>-YYYY-MM-DD; the demand key and state-root slug. Defaults to todoId when omitted." },
