...
export function controllerReturnDuplicateSelector({ returnPolicy = {}, triggerTarget = "", triggerTaskId = "" } = {}) {
  const mode = normalizeReturnPolicyMode(returnPolicy.mode || returnPolicy);
  return mode === "per-target" ? { triggerTarget, triggerTaskId } : {};
}

export function controllerReturnDuplicateScopeText({ returnPolicy = {}, triggerTarget = "", triggerTaskId = "" } = {}) {
  const mode = normalizeReturnPolicyMode(returnPolicy.mode || returnPolicy);
  return mode === "per-target" ? ` for ${triggerTarget} / ${triggerTaskId}` : "";
}
...
export function buildControllerCallbackPlan({
  ...
  const units = mode === "group-ready"
    ...
    : returnable.map((item) => {
        const existingDeliveries = controllerReturnDeliveries
