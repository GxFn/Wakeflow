import { parseDenseArray, parsePlainRecord, PassiveOwnDataError, } from "./passive-own-data.js";
/**
...
function readPassiveSnapshot(operation) {
    try {
        return operation();
    }
    catch (error) {
...
function defineJsonProperty(snapshot, key, value) {
...
function parseArrayValue(sourceValue, path, ancestors, depth) {
    const source = readPassiveSnapshot(() => parseDenseArray(sourceValue, JAVASCRIPT_MAXIMUM_ARRAY_LENGTH, path));
    ancestors.add(sourceValue);
    try {
        const snapshot = [];
        for (let index = 0; index < source.length; index += 1) {
            snapshot.push(parseValue(source[index], appendPropertyPath(path, String(index)), ancestors, depth + 1));
        }
...
function parseObjectValue(sourceValue, path, ancestors, depth) {
    ...
    try {
        ...
        for (const key of Object.keys(source)) {
            if (!key.isWellFormed())
                fail("lone-surrogate", path);
            const childPath = appendPropertyPath(path, key);
            defineJsonProperty(snapshot, key, parseValue(source[key], childPath, ancestors, depth + 1));
        }
        return Object.freeze(snapshot);
    }
    finally {
...
function parseValue(value, path, ancestors, depth) {
    if (depth > JSON_VALUE_MAXIMUM_DEPTH)
        fail("maximum-depth", path);
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            fail("non-finite-number", path);
        if (Object.is(value, -0))
            fail("negative-zero", path);
        return value;
    }
    if (typeof value !== "object")
        fail("unsupported-type", path);
    let isArray;
    try {
 ...
 */
export function hasPassiveJsonSerializationEnvironment() {
 ...
 */
export function parseJsonValue(value, errorPath) {
    return parseValue(value, normalizeErrorPath(errorPath), new Set(), 0);
}
