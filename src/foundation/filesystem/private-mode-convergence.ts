...
const SPECIAL_BITS = 0o7000n;

export type PrivateNodeClass = "current" | "safe-drift" | "unsafe";

export type PrivateModeConvergenceErrorReason =
  | "input"
  | "root-scope"
  | "resource-changed"
  | "resource-unsafe"
...
} as const satisfies Readonly<Record<PrivateModeConvergenceErrorReason, string>>;

export class PrivateModeConvergenceError extends Error {
  override readonly name = "PrivateModeConvergenceError";
  readonly code = "wakeflow-private-mode-convergence" as const;
  readonly reason: PrivateModeConvergenceErrorReason;
  readonly path: string;

  constructor(reason: PrivateModeConvergenceErrorReason, path: string) {
...
function fail(reason: PrivateModeConvergenceErrorReason, path: string): never {
...
export function privateModeOf(node: Readonly<FileNodeSnapshot>): number | null {
  if (node.kind === "directory") return PRIVATE_DIRECTORY_MODE;
  if (node.kind === "file") return PRIVATE_FILE_MODE;
  return null;
}

/** 按当前进程的用户分类一个节点（没有 geteuid 的平台不核对属主）。 */
export function classifyPrivateNode(node: Readonly<FileNodeSnapshot>): PrivateNodeClass {
  const target = privateModeOf(node);
  if (target === null) return "unsafe";
  const user = currentUserId();
  if (user !== null && node.userId !== user) return "unsafe";
  if ((node.rawMode & SPECIAL_BITS) !== 0n) return "unsafe";
  const bits = node.permissionBits;
  if (bits === target) return "current";
  if ((bits & target) !== target) return "unsafe";
  if ((bits & GROUP_OTHER_WRITE) !== 0) return "unsafe";
  return "safe-drift";
}

function snapshotOf(value: unknown, errorPath: string): Readonly<FileNodeSnapshot> {
  try {
    return createFileNodeSnapshot(value, errorPath);
  } catch (error: unknown) {
    if (error instanceof FileNodeSnapshotError) fail("resource-changed", errorPath);
    throw error;
  }
}
...
function openFlags(kind: "file" | "directory", errorPath: string): number {
  const noFollow: unknown = fileSystemConstants.O_NOFOLLOW;
  if (typeof noFollow !== "number") fail("root-scope", errorPath);
  const nonBlocking =
    typeof fileSystemConstants.O_NONBLOCK === "number" ? fileSystemConstants.O_NONBLOCK : 0;
  let directory = 0;
  if (kind === "directory") {
...
async function inspectPath(
  ...
  errorPath: string,
): Promise<Readonly<{ readonly physicalPath: string; readonly node: Readonly<FileNodeSnapshot> }>> {
  try {
 ...
 */
export async function convergePrivateNodeMode(
  root: RootedDirectory,
  resourcePath: PortableResourcePath,
  expected: Readonly<FileNodeSnapshot>,
  options: Readonly<{ readonly signal?: AbortSignal }> = {},
): Promise<"converged" | "current"> {
  ...
  if (observedClass === "current") return "current";
  if (
    !sameFileNodeIdentity(observed.node, expected) ||
    observed.node.permissionBits !== expected.permissionBits ||
    observed.node.kind !== expected.kind
  ) {
    fail("resource-changed", errorPath);
  }
  if (observedClass !== "safe-drift") fail("resource-unsafe", errorPath);
  ...
  try {
    ...
    if (
      opened.kind !== observed.node.kind ||
