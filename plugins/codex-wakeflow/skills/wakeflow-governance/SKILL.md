...
- **Multiple demands = pod 0 + isolation pods**: up to `maxActiveDemands` (default 2) demands run side by side and remain mutually unaware. The default fleet is pod 0: persistent controller/Test/work windows on main checkouts. Only demand 2..N gets its OWN controller (stamped into the state root), OWN Test, and ONE isolation worktree set opened by `wakeflow_pod_open` and closed by `wakeflow_pod_close` (complete-demand -> pod close -> archive). Parallelism exists ONLY at the demand level — never inside a demand (each repo = one window = ONE combined task package); every isolation-pod window, Test included, works in that pod's worktree set, never on a main checkout. Branch merge-back is human-reviewed and decentralized (pending-merges ledger); claiming past capacity fails closed.
