...
async function inspectCurrent(root, request, signal, afterCommit) {
    try {
        return await inspectWakeflowWorkspaceGitignore(root, wakeflowGitignoreInspectionRequest(request, afterCommit ? undefined : signal));
    }
    catch (error) {
        if (afterCommit)
...
async function publishTarget(root, inspection, signal) {
    const target = inspection.target;
    if (inspection.status !== "recompose-required"
        || target === null
    ...
    try {
        if (inspection.source === null) {
            return await createFileAtomically(root, WAKEFLOW_GITIGNORE_REF, target.bytes, {
                mode: WAKEFLOW_GITIGNORE_FILE_MODE,
                ...(signal === undefined ? {} : { signal }),
            });
        }
        return await replaceFileAtomically(root, WAKEFLOW_GITIGNORE_REF, target.bytes, {
            mode: WAKEFLOW_GITIGNORE_FILE_MODE,
...
function assertReadback(before, effect, after, expectedUserId) {
...
async function recomposeUnderLock(root, request, options, expectedUserId, markCommitted) {
    ...
    await assertCurrentUserWakeflowGitignoreRecompositionRoot(root, expectedUserId);
 ...
 */
export async function recomposeWakeflowWorkspaceGitignore(rootValue, requestValue, optionsValue) {
    ...
    await assertCurrentUserWakeflowGitignoreRecompositionRoot(rootValue, expectedUserId);
    let committed = false;
    try {
        return await withRootedExclusiveFileLock(rootValue, WAKEFLOW_GITIGNORE_RECOMPOSITION_LOCK_REF, () => recomposeUnderLock(rootValue, request, options, expectedUserId, () => { committed = true; }), wakeflowGitignoreRecompositionLockOptions(options));
    }
    catch (error) {
