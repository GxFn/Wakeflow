...
2. When the user asks what to do next or status shows eligible work, also call `wakeflow_next_work`; its `activeDemands` + `demandCapacity` fields are the multi-demand dashboard (up to `maxActiveDemands` demands, each with its own state root — pod demands carry their own stamped `controllerWindow`).
