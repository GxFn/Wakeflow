...
function contextWithConfig(context, config) {
  return {
    ...
    config,
    userConfig: config,
    parentRoot: resolveMaybeRelative(context.controlRoot, config.workspaceRoot ?? context.config.workspaceRoot ?? ".."),
    ledgerPaths: workspaceLedgerPaths({ workspaceRoot: context.controlRoot, args, config }),
  };
}
...
function automationStateDir(context) {
  return path.join(context.controlRoot, ".workspace-local/codex-automation-loop");
}
...
function defaultThreadRole(context, windowName) {
  if (windowName === context.config.controlWindow) return "controller";
...
function localWindowRoot(context, windowName) {
  if (windowName === context.config.controlWindow) {
    return {
      path: ".",
      absolutePath: context.controlRoot,
      role: "Control workspace controller",
    };
  }
  ...
  return {
    path: repo.path,
    absolutePath: repositoryAbsPath(context.controlRoot, repo),
    role: repo.role,
  };
}
...
function localWindowRegistrationPayload(context) {
  ...
  for (const windowName of [...windowSpecs].sort()) {
    ...
    results.push({
      ...
      registryFile: hasThread ? slash(path.relative(context.controlRoot, registryPath)) : null,
      windowConfigFile: slash(path.relative(context.controlRoot, configPath)),
      threadRegistered: hasThread,
