...
/**
 * 仓库指针上观察到的分支名，按 git 自己的 ref 文法收：排除控制字符、空格、~ ^ : ? * [ 与反斜杠，排除首字符 '-' 与 '.'、'..'、'@{'、连续斜杠、以 '.' 开头的分段，以及以 '/'、'.' 或 '.lock' 结尾。
 */
...
/**
 * wakeflow_status 的结果：一次观察派生的工作区定向投影——总体状态、配置摘要、看板摘要、活动 Demand、窗口与工作声明、pod、仓库指针事实、hook 通道、已接受但分支仍在的结果、没有自带 status 的域的观察状态、被上限略去的条数、投影目标、生效阈值与下一步；带 demandId 时附该 Demand 的 Controller Route 或归档回执。不含私有路径、句柄与摘要之外的内容。
 */
...
/**
 * 没有自带 status/issue 的域的观察状态：读不出时空列表不是“没有”，issue 说明是哪一步失败。
 */
domains: {
demands: ObservedDomain
claims: ObservedDomain
pods: ObservedDomain
projection: ObservedDomain
windowRuntime: ObservedDomain
archives: ObservedDomain1
}
/**
 * 因越过本结果的数组上限而被略去的条数；全为 0 时列表是完整的。
 */
truncated: {
demands: Count
windows: Count
...
repositories: Count
worktrees: Count
unmergedAccepted: Count
/**
 * 终态 Demand 的候选按终态时间新的在前，超过归档清单上限（1024，primary pod 的归档也计入）或 worktree pod 评审快照上限（64）而没读的条数（§13.130）。
 */
archives: number
}
projection: {
...
hostHookRetentionMilliseconds: Count
}
route: (null | Route)
...
/**
 * 本 MCP 服务进程的制品身份：启动时的 manifest 摘要，以及磁盘上同一处 manifest 现在是否还是它（changed 即本窗口的服务进程要重连）。
 */
runtime: {
artifactManifestDigest: (null | Sha256Digest)
artifactOnDisk: ("same" | "changed" | "unknown")
}
/**
 * 维护协议的状态（§13.129），与维护工具的预览同一份核心布局检查：idle 才没有事情要做；busy 是另一次维护正在进行；recovery-required 是一次被打断的 apply 留下了事务残留，要用 recover 收尾；conflict 是锁不安全；absent 与 bootstrap-prefix 是协议根尚未建好，由对账重建。非 idle 时 overall 记 maintenance；检查本身读不出即 unavailable，protocol 为 unknown。
 */
maintenance: {
status: DomainStatus
protocol: ("idle" | "busy" | "recovery-required" | "conflict" | "absent" | "bootstrap-prefix" | "unknown")
/**
 * transactions 目录里的条目（§13.130），按名字排序，至多 64 条：kind 按命名约定分 intent、journal 与 unknown；recoverable 为 true 时可以用 operationId 调维护 recover（recover 仍会核对内容）；protocol 为 busy 时条目属于正在进行的维护，一律为 false；名字不合约定的条目 recover 找不到，只能由人查看后删除。名字只保留可打印字符。
 *
 * @maxItems 64
 */
residues: {
name: SingleLineText
kind: ("intent" | "journal" | "unknown")
operationId: (null | string)
recoverable: boolean
}[]
residuesOmitted: Count
}
}
...
/**
 * 该窗口的运行投影与当前 Config 加 Binding 重算的比对，取各宿主里最差的一份；缺失或过期由 reconcile 重建，unsafe 只报告。
 */
projection: ("current" | "stale" | "missing" | "unsafe" | "unavailable")
hostId: (null | HostId)
...
/**
 * 绑定会话最近一次 session-start 记录里的制品 manifest 摘要与本进程的比对：stale 的窗口要 resume 才跑当前制品；unknown 是没有可比的记录或本进程没有 manifest。
 */
artifact: ("current" | "stale" | "unknown")
}
...
/**
 * active：来自活动 Demand；archived：来自仍在配置里的 worktree pod 的终态 Demand（完成归档或取消撤回，§13.130），分支在 pod 关闭记下处置之前一直列出，同一 Demand 又活动时只列 active 的一份；primary pod 的归档不列，只占归档清单的读取上限。
 */
...
/**
 * 仍在配置里的 worktree pod 的终态 Demand（完成归档的与取消撤回的，§13.130），只为 unmergedAccepted 读：没有 worktree pod 时什么都不读；看板读不出时 unavailable；有归档读不出时仍 observed，issue 报出条数。
 */
export interface ObservedDomain1 {
status: DomainStatus
issue: NullableCode
}
