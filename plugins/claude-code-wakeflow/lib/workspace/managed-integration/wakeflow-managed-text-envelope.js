...
import { Sha256Hasher, Sha256HasherError, } from "../../foundation/crypto/sha256-hasher.js";
import { parsePlainRecord, PassiveOwnDataError, } from "../../foundation/data/passive-own-data.js";
import { createFileByteRange, parseFileByteOffset, } from "../../foundation/filesystem/file-byte-range.js";
 ...
 * 对该 component 的修改权限。
 */
export const WAKEFLOW_MANAGED_TEXT_MARKER_PREFIX = "<!-- wakeflow:managed-content:v1:";
const MARKER_NAMESPACE_PREFIX_BYTES = Buffer.from("<!-- wakeflow:managed-content:", "ascii");
const LF = 0x0a;
const COMPONENT_PATTERN = /^[a-z][a-z0-9-]{0,127}$/u;
const OWNER_PATTERN = /^[a-z][a-z0-9_-]{0,127}$/u;
const MARKER_PATTERN = /^<!-- wakeflow:managed-content:v1:(begin|end) component=([a-z][a-z0-9-]{0,127}) owner=([a-z][a-z0-9_-]{0,127}) digest=(sha256:[0-9a-f]{64}) sep=([01]) -->$/u;
...
export class WakeflowManagedTextEnvelopeError extends Error {
    name = "WakeflowManagedTextEnvelopeError";
    code = "wakeflow-managed-text-envelope";
    reason;
    path;
...
function digestBytes(bytes, path) {
    try {
...
function digestParts(parts, path) {
    let hasher;
    try {
...
function concatenate(parts, path) {
    try {
...
function range(start, endExclusive, path) {
    return createFileByteRange(parseFileByteOffset(start, `${path}.offset`), parseByteCount(endExclusive - start, `${path}.length`), path);
}
function markerOffsets(source) {
    const offsets = [];
    let offset = 0;
    while (offset <= source.byteLength - MARKER_NAMESPACE_PREFIX_BYTES.byteLength) {
...
function parseMarker(source, start) {
    if (start !== 0 && source[start - 1] !== LF)
        fail("marker", "$marker");
...
function assertBodyProfile(body, path) {
    if (body.length <= 1
        || body.startsWith("\ufeff")
        || body.includes("\r")
        || !body.endsWith("\n")
...
function inputRecord(value, path, expectedKeys) {
    let record;
    try {
...
export function parseWakeflowManagedTextEnvelopeTarget(value) {
...
function inspectSnapshot(source) {
    decodeSource(source);
    ...
    return Object.freeze({
        ...
        suffixOutsideRange: range(blockEnd, source.byteLength, "$suffixOutsideRange"),
    });
}
/** 复制并检查一份完整 UTF-8 字节快照中的唯一 Wakeflow envelope。 */
export function inspectWakeflowManagedTextEnvelope(sourceValue) {
    return inspectSnapshot(snapshotBytes(sourceValue));
}
function renderOwnedBlock(target, ownsSeparator) {
    let bodyBytes;
    try {
    ...
    const marker = (side) => (`${WAKEFLOW_MANAGED_TEXT_MARKER_PREFIX}${side} component=${target.component} owner=${target.owner} digest=${digest} sep=${separator} -->`);
    try {
        return Buffer.from(`${ownsSeparator ? "\n" : ""}${marker("begin")}\n${target.body}${marker("end")}\n`, "utf8");
    }
    catch {
        fail("capacity", "$target.body");
    }
}
function resultBytes(bytes) {
    return Object.freeze({
        bytes,
 ...
 */
export function recomposeWakeflowManagedTextEnvelope(sourceValue, targetValue) {
 ...
 */
export function removeWakeflowManagedTextEnvelope(sourceValue, identityValue) {
    const source = snapshotBytes(sourceValue);
