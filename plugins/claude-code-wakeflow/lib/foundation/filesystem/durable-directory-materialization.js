...
import { mkdir, open as openFileHandle, } from "node:fs/promises";
...
import { createFileNodeSnapshot, sameFileNodeIdentity, } from "./file-node-snapshot.js";
...
export class DurableDirectoryMaterializationError extends Error {
    name = "DurableDirectoryMaterializationError";
    code = "wakeflow-durable-directory-materialization";
    reason;
    path;
...
function requiredDirectoryOpenFlags() {
...
function mapParentHandleError(error, operation, inspectReason) {
...
async function inspectParentTarget(parent, reason) {
    try {
...
async function closeParent(parent) {
    try {
        ...
        if (error instanceof RootedResourceParentHandleError) {
            return new DurableDirectoryMaterializationError(error.reason === "close-failure" ? "close-failure" : "parent-changed", "$resourcePath");
        }
        throw error;
    }
}
async function closeHandle(handle) {
    try {
        ...
        return new DurableDirectoryMaterializationError("close-failure", "$resourcePath");
    }
}
...
async function performAtomicCreate(root, resourcePath, options) {
    ...
    try {
        ...
        if (await inspectParentTarget(parent, "target-inspection-failure") !== null) {
        ...
        try {
            targetHandle = await openFileHandle(parent.resourceAbsolutePath, requiredDirectoryOpenFlags());
        }
        catch {
            fail("commit-uncertain", "$resourcePath");
        }
        const opened = await snapshotHandle(targetHandle, "commit-uncertain", "$resourcePath");
        const pathBefore = await inspectParentTarget(parent, "commit-uncertain");
        if (pathBefore === null
        ...
        const hardened = await snapshotHandle(targetHandle, "commit-uncertain", "$resourcePath");
        if (hardened.kind !== "directory"
        ...
        await assertParentCurrent(parent);
        const afterHandle = await snapshotHandle(targetHandle, "commit-uncertain", "$resourcePath");
        const afterPath = await inspectParentTarget(parent, "commit-uncertain");
...
async function inspectExistingDirectoryOrNull(root, resourcePath) {
    let resource;
    try {
...
function prefixPaths(resourcePath) {
    let segments;
    try {
    ...
    for (let index = 1; index <= segments.length; index += 1) {
        ...
        try {
            prefixes.push(parsePortableResourcePath(current, "$resourcePath"));
        }
        catch (error) {
            if (error instanceof PortableResourcePathError) {
                fail("input", "$resourcePath");
 ...
 */
export async function createDirectoryAtomically(root, resourcePath, options) {
    assertRoot(root);
 ...
 */
export async function materializeDirectoryPath(root, resourcePath, options) {
    assertRoot(root);
    const parsed = parseOptions(options);
    ...
    for (const prefix of prefixes) {
        ...
        try {
            const created = await performAtomicCreate(root, prefix, parsed);
            observations.push(Object.freeze({
                resourcePath: prefix,
    ...
    for (const observation of observations) {
        const current = await inspectExistingDirectoryOrNull(root, observation.resourcePath);
        if (current === null
