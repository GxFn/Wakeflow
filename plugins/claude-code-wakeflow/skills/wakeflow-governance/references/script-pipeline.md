...
| Archive completed Wakeflow docs and shrink historical indexes | MCP: `wakeflow_archive_workspace_docs`, `wakeflow_archive_todo`; backend scripts: `wakeflow-archive-docs.mjs`, `wakeflow-compact-index.mjs`, `wakeflow-archive-todo.mjs`, `wakeflow-archive-summaries.mjs` | Use MCP for normal controller archive flows. Dry-run first; apply only after current status no longer points at the archived item. |
