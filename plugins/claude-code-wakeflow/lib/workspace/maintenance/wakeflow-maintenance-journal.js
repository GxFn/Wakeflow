import { WAKEFLOW_MAINTENANCE_JOURNAL_SCHEMA, } from "../../contracts/generated/workspace/maintenance-journal.generated.js";
...
import { parseWakeflowMaintenanceExecutionPlan, WakeflowMaintenanceExecutionPlanError, } from "./wakeflow-maintenance-execution-plan.js";
...
export class WakeflowMaintenanceJournalError extends Error {
    name = "WakeflowMaintenanceJournalError";
    code = "wakeflow-maintenance-journal";
    reason;
    path;
...
const validateWireJournal = createRuntimeJsonSchemaValidator(WAKEFLOW_MAINTENANCE_JOURNAL_SCHEMA, [WAKEFLOW_SHA256_DIGEST_SCHEMA]);
...
export function createPreparedWakeflowMaintenanceJournal(operationIdValue, intentDigestValue, planValue) {
    let operationId;
    try {
    ...
    const stepIds = Object.freeze(plan.steps.map((entry) => entry.stepId));
...
export function parseWakeflowMaintenanceJournal(value) {
    ...
    let operationId;
    try {
        operationId = parseWakeflowMaintenanceOperationId(validated.value.operationId, "$journal.operationId");
    }
    catch (error) {
        if (error instanceof WakeflowMaintenanceOperationIdError) {
    ...
    const stepIds = Object.freeze([...validated.value.stepIds]);
...
function journalRepresentation(journal) {
    return {
        kind: journal.kind,
...
function sameImmutableJournal(left, right) {
...
export function beginWakeflowMaintenanceJournalStep(value) {
...
export function completeWakeflowMaintenanceJournalStep(value) {
    const journal = parseWakeflowMaintenanceJournal(value);
...
export function terminalizeWakeflowMaintenanceJournal(value) {
    const journal = parseWakeflowMaintenanceJournal(value);
    if (journal.state !== "executing"
        || journal.affectedStepId !== null
        || journal.checkpoint !== journal.stepIds.length) {
    ...
    return Object.freeze({ ...journal, state: "terminal" });
}
/** 判断 proposed 是否恰好是 current 允许的一个单调 journal 后继。 */
export function isWakeflowMaintenanceJournalSuccessor(currentValue, proposedValue) {
    const current = parseWakeflowMaintenanceJournal(currentValue);
    const proposed = parseWakeflowMaintenanceJournal(proposedValue);
    if (!sameImmutableJournal(current, proposed))
        return false;
...
export function computeWakeflowMaintenanceJournalDigest(value) {
    return computeCanonicalJsonSha256Digest(journalRepresentation(parseWakeflowMaintenanceJournal(value)));
}
/** 为一个 operation 生成精确 journal 动态资源声明。 */
export function createWakeflowMaintenanceJournalResourceDeclaration(operationIdValue) {
