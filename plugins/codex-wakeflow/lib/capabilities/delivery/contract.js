import { WAKEFLOW_PREPARE_DELIVERY_REQUEST_SCHEMA, } from "../../contracts/generated/entrypoints/wakeflow-prepare-delivery-request.generated.js";
import { WAKEFLOW_PREPARE_DELIVERY_RESULT_SCHEMA, } from "../../contracts/generated/entrypoints/wakeflow-prepare-delivery-result.generated.js";
import { WAKEFLOW_REARM_DELIVERY_REQUEST_SCHEMA, } from "../../contracts/generated/entrypoints/wakeflow-rearm-delivery-request.generated.js";
import { WAKEFLOW_REARM_DELIVERY_RESULT_SCHEMA, } from "../../contracts/generated/entrypoints/wakeflow-rearm-delivery-result.generated.js";
import { WAKEFLOW_RECORD_DELIVERY_OUTCOME_REQUEST_SCHEMA, } from "../../contracts/generated/entrypoints/wakeflow-record-delivery-outcome-request.generated.js";
import { WAKEFLOW_RECORD_DELIVERY_OUTCOME_RESULT_SCHEMA, } from "../../contracts/generated/entrypoints/wakeflow-record-delivery-outcome-result.generated.js";
 ...
 */
export const WAKEFLOW_PREPARE_DELIVERY_PUBLIC_TOOL_NAME = "wakeflow_prepare_delivery";
export const WAKEFLOW_RECORD_DELIVERY_OUTCOME_PUBLIC_TOOL_NAME = "wakeflow_record_delivery_outcome";
export const WAKEFLOW_REARM_DELIVERY_PUBLIC_TOOL_NAME = "wakeflow_rearm_delivery";
export const WAKEFLOW_DELIVERY_PUBLIC_SCHEMA_VERSION = 1;
const validatePrepareRequest = createRuntimeJsonSchemaValidator(WAKEFLOW_PREPARE_DELIVERY_REQUEST_SCHEMA);
const validatePrepareResult = createRuntimeJsonSchemaValidator(WAKEFLOW_PREPARE_DELIVERY_RESULT_SCHEMA);
const validateOutcomeRequest = createRuntimeJsonSchemaValidator(WAKEFLOW_RECORD_DELIVERY_OUTCOME_REQUEST_SCHEMA);
const validateOutcomeResult = createRuntimeJsonSchemaValidator(WAKEFLOW_RECORD_DELIVERY_OUTCOME_RESULT_SCHEMA);
const validateRearmRequest = createRuntimeJsonSchemaValidator(WAKEFLOW_REARM_DELIVERY_REQUEST_SCHEMA);
const validateRearmResult = createRuntimeJsonSchemaValidator(WAKEFLOW_REARM_DELIVERY_RESULT_SCHEMA);
...
export function parsePrepareDeliveryRequest(value) {
    return parseRequest(validatePrepareRequest, value);
}
export function admitPrepareDeliveryResult(value) {
    return admitResult(validatePrepareResult, value);
}
export function parseRecordDeliveryOutcomeRequest(value) {
    return parseRequest(validateOutcomeRequest, value);
}
export function admitRecordDeliveryOutcomeResult(value) {
    return admitResult(validateOutcomeResult, value);
}
export function parseRearmDeliveryRequest(value) {
    return parseRequest(validateRearmRequest, value);
}
export function admitRearmDeliveryResult(value) {
...
export const REARM_DELIVERY_TOOL_REGISTRATION = Object.freeze({
    ...
    description: "Rearm one delivery. Target mode: a target deliveryId whose current generation is rejected-before-send keeps its envelope and prompt, gets a fresh window work claim and fence, the generation increases by one, and the permit is re-issued; at most three rearms per envelope, then prepare the target again. Callback mode: the callback deliveryId of a result-reported or test-result-reported target has its wake-controller callback permit re-issued against the current Controller binding, with no claim or fence. Never performs the host effect.",
